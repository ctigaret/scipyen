...
def averageEPSCaTs(scandata, epscatname, frame_index = None):
    ...
    if isinstance(frame_index, dt.TriggerProtocol):
...
def analyseEPSCaT(lsdata, frame, indicator_channel_ndx, 
    ...
    elif isinstance(unit, dt.AnalysisUnit):
    ...
    if cursor is None:
        ...
        elif hasattr(lsdata, "__analysis_unit__") and isinstance(lsdata.__analysis_unit__, dt.AnalysisUnit):
            epscatname = lsdata.analysisUnit().name
            
        else:
            epscatname = "EPSCaT"
        
    elif len(cursor.frameIndices) == 0 or frame in cursor.frameIndices:
    ...
    if isinstance(lsdata.scans[0], dt.PictArray):
        ...
        cal = dt.AxisCalibration(lsdata.scans[0].axistags["t"])
...
def analyseFrame(lsdata, frame, unit=None, 
    ...
    #if not isinstance(unit, (dt.AnalysisUnit, type(None))):
...
def computeLSCaT(roiRange, f0Range, ca_data, ref_data=None, detrend=False, 
    ...
    if isinstance(ca_data, dt.PictArray):
        ...
        tcal = dt.AxisCalibration(ca_data.axistags["t"])
...
def fitEPSCaT(data, p0, bounds, fitWindow = None, integration=None):
    ...
    if fitWindow is not None:
        ...
        y_data = y.time_slice(fit_start, fit_end).copy()
...
def reportUnitAnalysis(scandata, analysis_unit=None, protocols=None, frame_index = None, filename=None, return_type="dataframe"):
    ...
    if analysis_unit is None: # when analysis unit is None, take the whole data as an analysis unit
    ...
    elif isinstance(analysis_unit, dt.AnalysisUnit): # analysis unit specified as an AnalysisUnit object
        ...
        for a in analysis_unit:
            ...
            elif isinstance(a, dt.AnalysisUnit):
                if scandata.hasAnalysisUnit(a) or analysis_unit is scandata.analysisUnit():
    ...
    for analysis_unit in analysis_units:
        ...
        if protocols is None or (isinstance(protocols, (tuple, list)) and len(protocols) == 0): # no protocols specified in the call list
            ...
            for p in protocols:
                ...
                elif isinstance(p, dt.TriggerProtocol): # protocol specified as is (TriggerProtocol object)
                    if p not in scandata.triggerProtocols:
                        raise ValueError("protocol %s not found in scandata %s" % \
        ...
        elif isinstance(protocols, dt.TriggerProtocol): #  a single TriggerProtocol object
...
def detectRoisInProfile(profile, order, *args, **kwargs):
    if not isinstance(profile, (dt.DataSignal, np.ndarray)):
...
class LSCaTWindow(ScipyenFrameViewer, __UI_LSCaTWindow__):
    ...
    def _save_viewer_settings_(self):
        ...
        self.settings.setValue("LSCaTAnalysis/Link_Scan_Vertical_Cursors_to_Scene_Point_Cursors", self.actionLink_vertical_scan_cursors_to_scene_point_cursors.isChecked())
        ...
        if len(self.profileviewers):
            ...
            self.settings.setValue("LSCaTAnalysis/ProfileWindow_Size", w.size())
            self.settings.setValue("LSCaTAnalysis/ProfileWindow_Position", w.pos())
            #self.settings.setValue("LSCaTAnalysis/ProfileWindow_Geometry", w.geometry())
            self.settings.setValue("LSCaTAnalysis/ProfileWindow_State", w.saveState())
                
            for dw in w.dockWidgets:
                self.settings.setValue("LSCaTAnalysis/ProfileWindow_%s" % dw[0], dw[1].isVisible())
        ...
        if len(self.ephysviewers):
            ...
            self.settings.setValue("LSCaTAnalysis/EphysWindow_Size", w.size())
            self.settings.setValue("LSCaTAnalysis/EphysWindow_Position", w.pos())
            #self.settings.setValue("LSCaTAnalysis/EphysWindow_Geometry", w.geometry())
            self.settings.setValue("LSCaTAnalysis/EphysWindow_State", w.saveState())
    ...
    def _load_viewer_settings_(self):
        ...
        link_scan_vc_to_scene_pc = self.settings.value("LSCaTAnalysis/Link_Scan_Vertical_Cursors_to_Scene_Point_Cursors", False)
        ...
        plot_long_fits = self.settings.value("LSCaTAnalysis/Plot_long_fits", False)
    ...
    def filterData(self, scene=True, scans=True):#, frames = None):
        ...
        if scene and len(self._data_.scene) > 0:
            ...
            sceneWorker = pgui.ProgressWorker(self.processData, pdlg,
                                      channel = [self._data_.analysisoptions["Channels"]["Reference"], self._data_.analysisoptions["Channels"]["Indicator"]],
                                      scene=True)
    ...
    def slot_importPrairieView(self):
        ...
        if isinstance(ephysData, neo.Block):
            ...
            if len(ephysData.segments) > lsdata.__scans_frames__:
                msgbox_btn = QtWidgets.QMessageBox.question(self, "Importing electrophysiology into %s" % tempDataVarName,
                                                "Block data has more segments (%d) that linescan frames (%d).\n Do you wish to discard surplus segments?" % (len(ephysData.segments), lsdata.__scans_frames__))


                if msgbox_btn in (QtWidgets.QMessageBox.Yes,  QtWidgets.QMessageBox.Ok):
                    for k in range(len(ephysData.segments), lsdata.__scans_frames__):
                        ephysData.segments = ephysData.segments[0:lsdata.__scans_frames__]
                        
            elif len(ephysData.segments) < lsdata.__scans_frames__:
                msgbox_btn = QtWidgets.QMessageBox.question(self, "Importing electrophysiology into %s" % tempDataVarName,
                                                "Block data has fewer segments (%d) that linescan frames (%d).\n Do you wish to append empty segments to electrophysiology?" % (len(ephysData.segments), lsdata.__scans_frames__))
            ...
            if len(self._data_.triggerProtocols):
                for protocol in self._data_.triggerProtocols:
                    ...
                    neoutils.embed_trigger_protocol(rev_p,
                                                    self._data_.__scans_block__,
                                                    clearTriggers=True,
                                                    clearEvents=False)
                    
                    neoutils.embed_trigger_protocol(rev_p,
                                                    self._data_.__scene_block__,
                                                    clearTriggers=True,
                                                    clearEvents=False)
    ...
    def slot_define_analysis_unit(self):
        ...
        if self._selected_analysis_cursor_ is not None:
            self._define_analysis_unit_on_landmark_(self._selected_analysis_unit_)
    ...
    def slot_showReportWindow(self):
        ...
        windowSize = self.settings.value("LSCaTAnalysis/ReportWindow_Size", None)
        
        if windowSize is not None:
            self.reportWindow.resize(windowSize)
            
        windowPos = self.settings.value("LSCaTAnalysis/ReportWindow_Position", None)
        
        if windowPos is not None:
            self.reportWindow.move(windowPos)
            
        windowState = self.settings.value("LSCaTAnalysis/ReportWindow_State", None)
    ...
    def slot_addProtocol(self):
        ...
        newProtocol = dt.TriggerProtocol()
    ...
    def slot_gui_edit_analysis_unit_descriptors(self):
        ...
        if dlg.exec() == QtWidgets.QDialog.Accepted:
            ...
            unit.setDescriptor("Distance_From_Soma", somatic_distance)
            unit.setDescriptor("Branching_Points", nbranches)
            unit.setDescriptor("Branch_Order", branch_order)
            unit.setDescriptor("Dendrite_Width", dendrite_width)
            unit.setDescriptor("Dendrite_Length", ddlength)
            unit.setDescriptor("Spine_Width", spine_width)
            unit.setDescriptor("Spine_Length", spine_length)
            ...
            if unit == self._data_.analysisUnit() or unit.landmark is None:
                if data_wide and propagate_check is not None:
                    if propagate_check.isChecked():
                        for u in self._data_.analysisUnits:
                            u.setDescriptor("Distance_From_Soma", somatic_distance)
    ...
    def slot_graphics_objects_deselected(self):
        ...
        try:
            for w in self.sceneviewers + self.scansviewers:
                if w != win:
                    w.viewerWidget.setSelectedCursor("", False)
                    w.viewerWidget.setSelectedRoi("", False)
    ...
    def _setup_scene_viewers_(self):
        ...
        # Therefore is data.scene is empty then we'll have no scene windows.
        
        if len(self.sceneviewers) > len(self._data_.scene):
            for w in self.sceneviewers[len(self._data_.scene):]: # remove extra viewers
                w.unlinkFromViewers()
                w.clear()
                w.close()
                
            self.sceneviewers = self.sceneviewers[0:len(self._data_.scene)]
        ...
        for k in range(len(self._data_.scene)):
            if k >= len(self.sceneviewers):
                ...
                if self.settings.contains("%s_RoisColor" % winSettingsStrPrefix):
                    roiscolor = self.settings.value("%s_RoisColor" % winSettingsStrPrefix, None)
                    
                    if isinstance(roiscolor, QtGui.QColor) and roiscolor.isValid():
                        win.setRoisColor(roiscolor)
                
                if self.settings.contains("%s_SharedCursorsColor" % winSettingsStrPrefix):
                    cursorscolor = self.settings.value("%s_SharedCursorsColor" % winSettingsStrPrefix, None)
                    
                    if isinstance(cursorscolor, QtGui.QColor) and cursorscolor.isValid():
                        win.setSharedCursorsColor(cursorscolor)
                
                if self.settings.contains("%s_SharedRoisColor" % winSettingsStrPrefix):
                    roiscolor = self.settings.value("%s_SharedRoisColor" % winSettingsStrPrefix, None)
                ...
                win = self.sceneviewers[k]
                win.unlinkFromViewers()
                win.clear()
                
            if win.framesSlider is not None:
                win.framesSlider.setMaximum(self._data_.sceneFrames)
        ...
        if self._data_.hasSignalData("ephys"):
            if len(self.ephysviewers):
                ...
                for dw in win.dockWidgets:
                    ...
                    dock_visibility = self.settings.value("LSCaTAnalysis/EphysWindow_%s" % dw[0], "false")
        ...
        if self._data_.hasSignalData("scene_profiles"):
            if len(self.profileviewers):
                ...
                for dw in win.dockWidgets:
                    ...
                    dock_visibility = self.settings.value("LSCaTAnalysis/ProfileWindow_%s" % dw[0], "false")
        ...
        if self._data_.hasSignalData("scans_data"):
            if len(self.scansblockviewers):
                ...
                for dw in win.dockWidgets:
                    ...
                    dock_visibility = self.settings.value("LSCaTAnalysis/ScansDataWindow_%s" % dw[0], "false")
        ...
        if self._data_.hasSignalData("scene_data"):
            if len(self.sceneblockviewers):
                ...
                for dw in win.dockWidgets:
                    ...
                    dock_visibility = self.settings.value("LSCaTAnalysis/SceneDataWindow_%s" % dw[0], "false")
    ...
    def _display_scene_(self):
        ...
        for k in range(len(self._data_.scene)):
            ...
            if len(self._data_.scene) > 1:
                ...
                self.sceneviewers[k].setWindowTitle("%s %s" % ("Scene", dt.AxisCalibration(self._data_.scene[k].axistags["c"]).channelIndicesAndNames()[0][1]))
                
            else: # single possibly multi-channel array
                axcal = dt.AxisCalibration(self._data_.scene[k].axistags["c"])
    ...
    def _display_epscats_(self):
        ...
        if len(self.scansblockviewers) == 0:
            self.scansblockviewers.append(sv.SignalViewer(pWin = self._scipyenWindow_))
            
            winSettingsStrPrefix = "LSCaTAnalysis/EPSCaTWindow"
            ...
            #wlist = [self]
                ...
                #wlist += self.scansviewers
                
            #if len(self.sceneviewers):
                #wlist += self.sceneviewers
                
            #if len(self.ephysviewers):
                #wlist += self.ephysviewers
                
            #if len(self.profileviewers):
                #wlist += self.profileviewers
                
            #self.scansblockviewers[0].linkToViewers(*wlist)
                
            
        if len(self._data_.scansBlock.segments):# and all([len(s.analogsignals) for s in self._data_.scansBlock.segments]):
            self.scansblockviewers[0].view(self._data_.scansBlock)
            self.scansblockviewers[0].currentFrame = self.currentFrame
            
        self.scansblockviewers[0].setWindowTitle("%s %s" % ("EPSCaTs", self._data_.name))
    ...
    def _display_scanline_profiles_(self):
        ...
        if len(self.profileviewers) == 0:
            self.profileviewers.append(sv.SignalViewer(pWin = self._scipyenWindow_))
    ...
    def _display_ephys_(self):
        ...
        if len(self._data_.ephys.segments) != self._data_.scansFrames:
            warnings.warn("Mismatch between number of scans frames and electrophysiology sweeps")
            if len(self.ephysviewers):
        ...
        if not all([len(s.analogsignals) for s in self._data_.ephys.segments]):
            warnings.warn("Not all scan frames have an associated electrophysiology segment")
            if len(self.ephysviewers):
                self.ephysviewers[0].setWindowTitle("%s %s (%s)" % ("Electrophysiology", self._data_.name, self._data_.ephys.name))
            return
        
        if len(self.ephysviewers) == 0:
            self.ephysviewers.append(sv.SignalViewer(pWin = self._scipyenWindow_))
            
            winSettingsStrPrefix = "LSCaTAnalysis/EphysWindow"
            ...
            #if len(self.scansblockviewers):
                #wlist += self.scansblockviewers
                
            #self.ephysviewers[0].linkToViewers(*wlist)
    ...
    def _trigger_events_detection_gui_(self, options, ephys_start, ephys_end, dlg = None, title = "Detect triggers"):
        ...
        if len(presynaptic_name.strip()) == 0:
        ...
        if len(postsynaptic_name.strip()) == 0:
        ...
        if len(photostimulation_name.strip()) == 0:
        ...
        if len(imaging_frame_name.strip()) == 0:
    ...
    def _display_scans_(self):
        ...
        for k in range(len(self._data_.scans)):
            ...
            if len(self._data_.scans) > 1:
                # multiple single-channel arrays
                self.scansviewers[k].setWindowTitle("%s %s" % ("Scan", dt.AxisCalibration(self._data_.scans[k].axistags["c"]).channelIndicesAndNames()[0][1]))
                
            else:
                # single, possibly multi-channel array
                axcal = dt.AxisCalibration(self._data_.scans[k].axistags["c"])
                chnames = [s[1] for s in axcal.channelIndicesAndNames()]
    ...
    def autoSetupLinescanCursorsInFrame(self, frame, displayFrame=True):
        ...
        channel_ndx = neoutils.get_index_of_named_signal(profiles.segments[frame], refChannelName, stype=dt.DataSignal)
        ...
        if len(data) > 0:
            if len(self._data_.sceneRois) > 0:
                if len(data) == 1: 
                    ...
                    profiles = [[dt.DataSignal(getProfile(img, self._data_.scanRegion.objectForFrame(k)), \
                                                sampling_period=dt.getAxisResolution(img.axistags["x"]), \
                                                name="%s" % dt.axisChannelName(subarray.axistags["c"], j), \
                                                index = j) \
                    ...
                    profiles = [[dt.DataSignal(getProfile(subarray.bindAxis(self._data_.sceneFrameAxis, k), self._data_.scanRegion.objectForFrame(k)), \
                                                sampling_period=dt.getAxisResolution(subarray.bindAxis(self._data_.sceneFrameAxis, k).axistags["x"]), \
                                                name="%s" % dt.axisChannelName(subarray.axistags["c"], 0), index = j) \
    ...
    def generateScanRregionProfilesFromScans(self): 
        ...
        if len(data) > 0:
            if len(data) == 1: 
                ...
                profiles = [[dt.DataSignal(np.array(img.mean(axis=1)), \
                                        sampling_period = dt.getAxisResolution(img.axistags["x"]), \
                                        name="%s" % dt.axisChannelName(subarray.axistags["c"], j), \
                                        index = j) \
                                    for j, img in dt.dimEnum(subarray, "c")] \
                ...
                profiles = [[dt.DataSignal(np.array(subarray.bindAxis(self._data_.scansFrameAxis, k).mean(axis=1)), \
                                        sampling_period = dt.getAxisResolution(subarray.bindAxis(self._data_.scansFrameAxis, k).axistags["x"]), \
                                        name="%s" % dt.axisChannelName(subarray.axistags["c"],0), \
                                        index = j) \
                                    for j, subarray in enumerate(data)] \
    ...
    def processData(self, scene=True, channel = None, progressSignal = None, setMaxSignal=None):
        ...
        if len(source) == 1: # case when source data is a VigraArray (possibly, a multi-channel array)
            ...
            source_chn_cal = dt.AxisCalibration(source[0].axistags["c"])
            source_chn_cal.calibrateAxis(result[0].axistags["c"])
            #result_chn_cal = dt.AxisCalibration(result[0].axistags["c"])
            ...
            for k in process_channel_ndx:
                chn_cal = dt.AxisCalibration(source[k].axistags["c"])
    ...
    def setData(self, newdata = None, varname=None):
        ...
        if isinstance(newdata, ScanData):
            ...
            self._setup_viewers_() # this MAY modify data (if scan region sceen profile is enabled)
