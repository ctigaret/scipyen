...
{
    echo "Trying to create/use virtual environment ${virtual_env} in ${ve_path} using ${using_python}"
    ...
    if [ -d $ve_path/$virtual_env ] ; then
        ...
        ${python_executable} -m virtualenv --python ${python_executable} $virtual_env && source $ve_path/$virtual_env/bin/activate
...
function installpipreqs ()
{
    ...
    if [ ! -r ${VIRTUAL_ENV}/.pipdone ] || [[ $reinstall_pips -gt 0 ]] ; then
        ...
        echo "Using ${py_exec} as `whoami` to install PyPI packages"
        
        ${py_exec} -m pip install -r "$installscriptdir"/pip_requirements.txt
#         python3 -m pip install -r "$installscriptdir"/pip_requirements.txt
...
function dopyqt5 ()
{
    ...
    if [ ! -r ${VIRTUAL_ENV}/.pyqt5done ] || [[ $reinstall_pyqt5 -gt 0 ]]; then
        ...
        if [[ `id -u` -eq 0 ]] ; then
            py_exec="$VIRTUAL_ENV/bin/${python_exec}"
            sip_wheel_exec="$VIRTUAL_ENV/bin/sip_wheel"
        else
            py_exec=${python_exec}
...
python_exec="python${using_python}"
...
# # NOTE: 2023-06-25 10:52:11
# # now , switch to the virtual env python executable;
# # if running as root, pass the full path explicitly
# if [[ `id -u` -eq 0 ]] ; then
#     py_exec="$VIRTUAL_ENV/bin/${python_exec}"
# else
#     py_exec=${python_exec}
