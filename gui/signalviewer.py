...
class SignalViewer(ScipyenFrameViewer, Ui_SignalViewerWindow):
    ...
    def __init__(self, 
        ...
        self._data_cursors_ = collections.ChainMap(self.crosshairDataCursors, self.horizontalDataCursors, self.verticalDataCursors)
    ...
    def addCursor(self, cursorType: typing.Union[str, SignalCursor.SignalCursorTypes] = "c", 
        ...
        ## this FIRST creates the cursor (with default coordinates i.e. at the
        ## CENTRE of the axis or scene), THEN updates its coordinates
        #if cursorType in ("c", "crosshair"):
            #crsID = self._addCursor_(cursorType)
            #crsID = self.slot_addCrosshairCursor(label)
            #self.slot_selectCursor(crsID)
            #self.selectedDataCursor.xwindow = xwindow
            #self.selectedDataCursor.ywindow = ywindow
        ...
        #elif cursorType in ("h", "horizontal"):
    ...
    def _slot_plot_axis_x_range_changed(self, x0, x1):
        ...
        lris = [item for item in ax.items if isinstance(item, pg.LinearRegionItem)]
        ...
        print("_slot_plot_axis_x_range_changed axis",
              ax,
              "viewRange x",
              x_min, x_max)
        
        for lri in lris:
            lx0, lx1 = lri.getRegion()
            if lx0>=x_min and lx0<=x_max and lx1 >= x_min and lx1 <= x_max:
                lri.setVisible(True)
                print("_slot_plot_axis_x_range_changed lri region", lri.getRegion())
    ...
    def displayFrame(self):
        ...
        #print("SignalViewer.displayFrame self._current_frame_index_ ", self._current_frame_index_ )
        ...
        if isinstance(self.y, (tuple, list)): 
            ...
            if isinstance(self.y, neo.core.Block):
                ...
                #print("SignalViewer.displayFrame for neo.Block: segment index", 
                      #segmentNdx,
                      #"for %d segments" % len(self.y.segments))
                
                segment = self.y.segments[segmentNdx]
                
                #print("SignalViewer.displayFrame for neo.Block: segment.epochs",
                      #segment.epochs)
        ...
        def _plot_epochs_(*args, **kwargs):
            ...
            for epoch in args:
                ...
                for k in range(len(self.axes)):
                    lris = [pg.LinearRegionItem(values=value, brush=brush, orientation=pg.LinearRegionItem.Vertical, movable=False, bounds = visibleXRange) for value in zip(x0,x1)]
