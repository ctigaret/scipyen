...
class GraphicsImageViewerWidget(QWidget, Ui_GraphicsImageViewerWidget):
    ...
    def _cursorEditor(self, crsId=None):
        ...
        cDict = self._graphicsObjects_[cursor_type]
        ...
        if d.exec() == QtWidgets.QDialog.Accepted:
            ...
            if old_name != cursor.name:
                cDict = self._graphicsObjects_[cursor.objectType]
    ...
    def newPlanarGraphics(self, item:typing.Optional[pgui.PlanarGraphics]=None, movable=True,
                          editable=True, showLabel=True, labelShowsPosition=True,
                          autoSelect=False):
        if self.__scene__.rootImage is None:
            return
        ...
        if isinstance(item, pgui.PlanarGraphics):
            ...
            pgui.GraphicsObject(obj=item, showLabel=showLabel, 
                                      labelShowsPosition = labelShowsPosition,
                                      parentWidget = parentWidget)
            
            self._planargraphics_.append(item)

        elif isinstance(item, type) and pgui.PlanarGraphics in item.__mro__:
            if pgui.Cursor in item.__mro__:
                def_x = np.floor(self.__scene__.rootImage.boundingRect().center().x())
                def_y = np.floor(self.__scene__.rootImage.boundingRect().center().y())
                ...
                #if len(cDict) > 0:
                #if len(self._planargraphics_):
                    ## install mouse event filter for all other cursors
                    #mouseEventFilters = [pgui.MouseEventSink(c) for c in self.scene.]
                    #for ck, c in enumerate(cDict.values()):
                    #for ck, c in enumerate(self._planargraphics_):
    ...
    def slot_removeCursorByName(self, crsId):
        ...
        if crsId in self.__cursors__.keys():
            ...
            #removed_cursor = self._graphicsObjects_[cType].pop(crsId, None)
            self._graphicsObjects_[cType].pop(crsId, None)
   ...
   #### BEGIN properties
    ...
    def graphicsObjects(self) -> typing.Iterator:
        ...
        #return filter(lambda x: isinstance(x, pgui.GraphicsObject), self.scene.items())
    ...
    @property
    def cursors(self) -> typing.Iterator:
    ...
    def roi(self, value:typing.Any, attribute:str="name", 
        ...
        attribute with values that satisfy the predicate.
    ...
    @safeWrapper
    def cursor(self, value:typing.Any, attribute:str="name", 
               predicate:typing.Optional[typing.Callable[...,bool]]=lambda x,y: x == y) -> typing.Iterator:
        """Iterates through Cursors having named attribute that satisfies predicate.
        ...
        Returns:
        ========
        An iterator for Cursor objects having the named attribute with values
        that satisfy the predicate.
        
        """
        return filter_attribute(self.cursors, attribute, value, predicate)
    
    def verticalCursor(self, value:typing.Any, attribute:str="name", 
               predicate:typing.Optional[typing.Callable[...,bool]]=lambda x,y: x == y) -> typing.Iterator:
        return filter_attribute(filter_type(self.planarGraphics, pgui.VerticalCursor), 
                                attribute, value, predicate)
        
    def horizontalCursor(self, value:typing.Any, attribute:str="name", 
               predicate:typing.Optional[typing.Callable[...,bool]]=lambda x,y: x == y) -> typing.Iterator:
        return filter_attribute(filter_type(self.planarGraphics, pgui.HorizontalCursor), 
                                attribute, value, predicate)
        
    def crosshairCursor(self, value:typing.Any, attribute:str="name", 
               predicate:typing.Optional[typing.Callable[...,bool]]=lambda x,y: x == y) -> typing.Iterator:
        return filter_attribute(filter_type(self.planarGraphics, pgui.CrosshairCursor), 
                                attribute, value, predicate)
        
    def pointCursor(self, value:typing.Any, attribute:str="name", 
               predicate:typing.Optional[typing.Callable[...,bool]]=lambda x,y: x == y) -> typing.Iterator:
...
class ImageViewer(ScipyenFrameViewer, Ui_ImageViewerWindow):
    ...
    def addPlanarGraphics(self, item:pgui.PlanarGraphics, pos=None, movable=True, editable=True, 
