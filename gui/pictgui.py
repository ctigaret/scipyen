...
class PlanarGraphics():
    """Ancestor for all planar graphic objects in Scipyen imaging.
    ...
    GraphicsObjectType enum in this module); some PlanarGraphics classes support
    more than one GraphicsObjectType value (e.g. Cursor).
    ...
    * * *
    ...
    def indexOfStateWithFrame(self, state:typing.Optional[DataBag]=None, 
        ...
        """Get the index of state in this object's list of states.
        
        Locates a state regardless of its visibility.
        """
        if state is None:
            state = self.getStateforFrame(frame_index=frame_index)
            
        return index_of(self._states_, state, key = lambda x: x.z_frame)
            
    def indexOfState(self, state:typing.Optional[DataBag]=None, 
                           frame_index:typing.Optional[int]=None) -> typing.Optional[int]:
        """Get the index of a state in this object's list of states, subject to visibility.
        ...
        frame_index: int >= 0 (optional , default is None)
        
        Returns:
        =======
        
        An int: 
            When state is None, this is the index of the state that is visible 
            in frame_index.
            
            Otherwise, this is the index of the specified state, if it exists
            in this object's states list.
            
        None: when the specified state was not found in this object's list of 
            states, or when the specified state is None and the current frame 
            does not have a visible state.
            
        """
        if state is None:
            state = self.getState(frame_index=frame_index)
            
        return index_of(self._states_, state, key = lambda x: x.z_frame)
            
    def getStateforFrame(self, frame_index:typing.Optional[int]=None) -> typing.Optional[object]:
        """Access the state where z_frame == frame_index regardless state visibility.
        
        Parameters
        ==========
        
        frame_index: None or an int;
        
            ATTENTION: Differences with self.getState():
            
                1. When frame_index is None the function tries to return the 
                   state for which z_frame is None (if found)
                
                2. When an int, frame_index can also take negative values.
        
        Returns:
        ========
        A state (DataBag object) if found, or None.
        
        If there is an ubiquitous state, this will always be returned regardless
        of the value of frame_index.
        
        Otherwise, returns the state with z_frame == frame_index, or None if 
        no such state exists.
        
        """
        if not isinstance(frame_index, (int, type(None))):
            raise TypeError("'frame_index' expected to be an int or None; got %s instead" % type(frame_index).__name__)
        
        if len(self._states_) == 1 and self._states_[0].z_frame is None:
            return self._states_[0]
        
        # states MUST be unique wrt their z_frame values
        states = [s for s in self._states_ if s.z_frame == frame_index]
    ...
    def getState(self, frame_index:typing.Optional[int]=None) -> typing.Optional[object]:
        ...
        frame_index: int or None; 
            ...
            ATTENTION: This is in contrast to self.getStateForFrame()
        ...
        * the unique state with ubiquitous visibility (i.e., has z_frame None)
            - is such a state exists, it will be returned regardless of the 
                value of the 'frame_index' parameter
                
        OR:
            
        * the state visible in the frame_index with index specified by 'frame_index'; this 
            state is either:
            
            - the state with z_frame == frame_index (i.e. the single-frame state for 
                this frame_index)
            
            or
            
            - the state explicitly invisible in another frame_index (frame-avoiding), 
                such that:
                
                z_frame != -1 *` frame_index -1
            
        """
        if frame_index is None:
            frame_index = self._currentframe_
            
        if not isinstance(frame_index, int):
            raise TypeError("'frame_index' expected to be an int; got %s instead" % type(frame_index).__name__)
        ...
        if len(self._states_) == 1: # trivial
            if self.__class__.isStateVisible(self._states_[0], frame_index):
                return self._states_[0]
            
        elif len(self._states_) > 1:
            visible_states = [s for s in self._states_ if self.__class__.isStateVisible(s, frame_index)]
