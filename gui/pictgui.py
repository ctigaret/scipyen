    ...
    * * *
    ...
    def __init_from_descriptors__(self, *args, frameindex:typing.Optional[typing.Iterable]=[],
        ...
        if len(args) == 1 and isinstance(args[0], str):
            # constructs a Text planar graphics
            state = DataBag({"text":args[0]},mutable_types=True,allow_none=True)
            ...
            if len(args) == 1:
                if isinstance(args[0], self.__class__): # COPY CONSTRUCTOR
                    ...
                    if all([isinstance(v, numbers.Real) for v in args[0]]):
                        ...
                        # check that the appropriate number of descriptors are
                        # given;
                        # needs frameindex, currentframe, name
                        #
                        # NOTE: 2020-11-30 16:38:45 about frameindex
                        # this is one of the rare cases where we need to specify
                        # frame indices to the primitive's contructor,
                        # because __init_from_descriptors__ simply constructs a 
                        # single state; 
                        # therefore we have the chance here to ascribe 
                        # frame-associations to the constructed state.
                        ...
                        # frameindex should be ignored here, because states' z_frame
                        # attributes already provide the frame indices where they
                        # are visible
...
class GraphicsObject(QtWidgets.QGraphicsObject):
    ...
    def __updateCachedPathFromBackend__(self):
        ...
        if isinstance(self.__backend__, (Ellipse, Rect)):
            self.__cachedPath__ = Path(Move(self.__backend__.x, 
                                            self.__backend__.y),
    ...
    def shape(self):
        ...
        if self.objectType & GraphicsObjectType.allCursorTypes:
            ...
            if self.__backend__ is not None and self.__backend__.hasStateForFrame(self._currentframe_) and self.__backend__() is not None:
                ...
                if self._objectType == GraphicsObjectType.line:
                    p0 = self.mapFromScene(QtCore.QPointF(self.__backend__[0].x, 
                                                          self.__backend__[0].y))
                    
                    p1 = self.mapFromScene(QtCore.QPointF(self.__backend__[1].x, 
                                                          self.__backend__[1].y))
                    ...
                    if self.editMode:
                        ...
                        if self.__cachedPath__ is not None and len(self.__cachedPath__) > 0:
                            if self.objectType & GraphicsObjectType.path:
                                ...
                                p0 = self.mapFromScene(QtCore.QPointF(self.__cachedPath__[0].x, 
                                                                      self.__cachedPath__[0].y))
                                
                                p1 = self.mapFromScene(QtCore.QPointF(self.__cachedPath__[1].x,
                                                                      self.__cachedPath__[1].y))
        ...
        if change == QtWidgets.QGraphicsItem.ItemPositionChange and self.scene():
            ...
            if self.objectType & GraphicsObjectType.allCursorTypes:
                ...
                # crosshair cursors
                    ...
                    self._deltaPos = (newPos - QtCore.QPointF(self.__backend__.x, 
                                                              self.__backend__.y))
