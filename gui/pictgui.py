...
class PlanarGraphics():
    ...
    * * *
    ...
    def __init__(self, *args, graphicstype=None, closed:bool=False, 
        ...
        *args: either:
            ...
            1) sequence of planar descriptors specific to the PlanarGraphics 
                primitive type, but WITHOUT z_frame (specified via frameindex)
                
                The expected order is as specified in __class__._planar_descriptors_
                
            2) iterable of planar descriptors as above
            
            3) a sequence of PlanarGraphics objects - Path only
            
            4) iterable of PlanarGRaphics objects - Path only
            
            5) a PlanarGraphics object (copy constructor); 
        ...
        if graphicstype is not None:
            if not isinstance(graphicstype, GraphicsObjectType): 
                # get a GraphicsObjectType by its name (substring allowed) or value (int)
                if isinstance(graphicstype, str):
                    # get by name (substring allowed)
                    if graphicstype in [t.name for t in GraphicsObjectType]:
                        ...
                        # try substrings
                        grtypes = [t for t in GraphicsObjectType if graphicstype in t.name]
                        
                        if len(grtypes):
                            if len(grtypes) > 1:
                                raise ValueError("Ambiguous graphics type name %s" % graphicstype)
                            
                            graphicstype = grtypes[0]
                    ...
                    if graphicstype in [t.value for t in GraphicsObjectType]:
                        grtypes = [t for t in GraphicsObjectType if t.value == graphicstype]
                        
                        if len(grtypes):
                            if len(grtypes) > 1:
                                raise ValueError("Ambiguous graphics type value %d" % graphicstype)
            ...
            if isinstance(self, Cursor):
                if graphicstype & GraphicsObjectType.allCursorTypes == 0:
                    expected_cursor_types = ["%s (%d)" % (t.name, t.value) for t in GraphicsObjectType if "_cursor" in t.name.lower()]
                    raise ValueError("For Cursor, 'graphicstype' is expected to be one of %s; got %s instead" % (expected_cursor_types,
                                                                                                  graphicstype))
    ...
    def checkStates(self):
        """Ensures consistency of the PlanarGraphcis states.
    ...
    def isStateVisible(cls, state:DataBag, frame:int) -> bool:
        ...
        if state.z_frame is None: # trivial: this is ALWAYS visible
            ...
            if frame >= 0: # check visibility in real frame index
                if state.z_frame >= 0:
    ...
    def setCurrentState(self, frame:typing.Optional[int]=None):
