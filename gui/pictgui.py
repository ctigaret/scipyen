...
class PlanarGraphics(object):
    ...
    used as landmarks (ROIs, and Cursors) on 2D images or frames. 
    
    In this context, a "frame" represents a 2D "slice" of the data array along
    one of its axes. An "image" is a 2D data array with one frame.
    
    Each shape primitive (point, line, arc, ellipse, etc) is defined by a set of
    numeric parameters (planar descriptors, e.g. position coordinates, width and
    height of the bounding rectangle, angle).
    ...
    ("frontend") over a data frame. The GraphicsObject objects "frontends" 
    inherit from the Qt QGraphicsObject defined in the QGraphics framework of 
    the Qt toolkit. 
    
    All 2D geometric primitives defined in this module inherit from PlanarGraphics,
    and are encapsulated in subclasses. A PlanarGraphics object also has a "type"
    instance attribute (see GraphicsObjectType enum in this module); some 
    PlanarGraphics classes support more than one GraphicsObjectType value (e.g. 
    Cursor).
    ...
    The planar descriptors are a set of numeric parameters that define a 2D 
    graphics primitive specific to the PlanarGraphics type and listed in the 
    "_planar_descriptors_" attribute of the PlanarGraphics object. These
    parameters emulate the parameters for constructing graphics primitives in 
    the QGraphics framework of the Qt tookit.
    ...
    Linear descriptor values (position coordinates, and distances) are 
    interpreted as data samples (i.e., pixels) relative to the top left sample of
    the data frame : the pixell with (0,0) coordinates, or the "origin".
    ...
    The concrete values of the planar descriptors are contained in so-called
    state objects. These values can be associated with a particular frame, a 
    subset of frames, or all 2D frames in the data array.
    
    A state is a dictionary-like object (python type: traitcontainers.DataBag)
    where the planar descriptors are accessed (and modified) by theie name, 
    either as attributes, or as dict keys.
    ...
    (I)     ubiquitous = visible in all available frames in the data:
    
            state z_frame is None
    
    (II)    single-frame = visible only in a frame with the index given by
            
            state.z_frame, where state.z_frame >= 0
    
    (III)   frame-avoding = visible in ALL EXCEPT the frame with the index
    
            -1 * state.z_frame - 1, where state.z_frame < 0
    
    Such mechanism allows a PlanarGraphics object to change its position, size, etc 
    across frames while still having the same graphics type (and Python type). 
    
    Moreover, a state can be visible in all but one particular frame, meaning that
    a PlanarGraphics object may be visible only in a subset of the data frames.
    
    States are optionally associated with frames via their "z_frame" attribute.
    
    The z_frame attribute of a state object can be:
        (1) an int that satisfies z_frame >= 0 and z_frame < number_of_frames
        
            In this case the graphics is displayed in the frame with the index 
            equal to z_frame using the concrete descriptor values contained in 
            that state.
            
        (2) an int where z_frame < 0
        
            In this case the graphics is NOT painted (i.e., it is hidden) in the 
            frame with the index (-1) * z_frame -1, but is painted in all other
            frames using the concrete values of the descriptors given by the state.
            
        (3) None
        
            In this case, the graphics is painted in all available frames, using 
            the descriptor values in this state.
            
    Changing planar descriptor values in a state should come with the option to 
        modify the state's association with a frame index:
        If state.z_frame is an int >= 0:
            * changing z_frame to None means the state "takes over"; alll other 
                states are discarded
                
            * changing the SIGN of z_frame means the state "takes over" all other
                states where z_frame >= 0 except for the state where
                z_frame == frame_index
                
        If state.z_frame is an int < 0:
            * changing the SIGN of z_frame >= 0 results in the state being assigned
                to frame_index; the previous state for that frame index, if it
                exists, will be discarded
                
            * changing z_frame to None means it will take over all other states 
                (which will be discarded)
                
        If state.z_frame is None:
            * chaning z_frame to an int >= 0:
                state becomes associated with given frame_index;
                all other frames have NO state.
                
            * chaning z_frame to an int < 0:
                state becomes associated with all frames with index != -1*z_frame -1
                
                frame with index == -1 * z_frame -1 is left without state
                
            
    It follows that a PlanarGraphics object:
    
        (a) may have a single state - which MAY be painted onto a specific data
            frame (i.e. VISIBLE) depending on the z_frame value:
            
            (a.1) when z_frame is None, the geometry is painted with the same 
                descriptor values in ALL available frames
            
            (a.2) when z_frame is an int >=0 above, the geometry 
                is painted ONLY in the data frame with 
                
                    frame_index == z_frame
            
            (a.3) when z_frame is an int < 0, the geometry is painted in ALL 
                available frames EXCEPT the frame with 
            
                    frame_index == (-1) * z_frame -1
                
        (b) can have several states; whether they are painted or not in a data
            frame depends on their z_frame value as in (a)
            
        (c) may be painted (visible) only in a subset of frames: the graphics 
            primitive will be INVISIBLE in those frames with indices NOT found 
            among the z_frame values of the "states" of the object
            
        (d) is INVISIBLE in a frame if any of the following is True:
        
            (d.1) none of its states has a z_frame value equal to the frame's 
                index - this also applies to the single-state with z_frame an int
                
            (d.2) it has a state where z_frame < 0 including
            
                (-1) * the frame's index - 1
                
        (e) is VISIBLE in a frame if any of the following is True:
        
            (e.1) has a single state with state.z_frame None
            
            (e.2) has multiple states and either:
                (e.2.1) one state has z_frame == frame_index
                
                (e.2.2) has at least one state declared invisible in another 
                    frame:
                    
                    z_frame < 0 and z_frame != (-1) * frame_index - 1
                    
        ATTENTION: Exclusive conditions:
        
        1) The PlanarGraphics can have only one state where z_frame is None.
            This is the UNIQUE state of the PlanarGraphics object.
            
        2) When there are several states:
            2.1) all states must have integer z_frame 
            
            2.2) there can be only one state with a negative z_frame, which is:
                * INVISIBLE in frame_index == -1 * z_frame -1
                * VISIBLE in any frame_index != -1 * z_frame -1
                2.2.1) there can be only ONE aditional state with z_frame > 0
                    which satisfies:
                    
                    additional_state.z_frame == -1 * state.z_frame
                    
                    i.e., it is VISIBLE in the frame that the negative z_frame
                    state is invisible.
                    
                2.2.2) this emans that the PlanarGraphics object has at most TWO
                    states in this case
                    
            
        Points (a)-(d) imply that several states may be painted on the same data
        frame.
                
        When a PlanarGraphics object with multiple states (typically,
        constructed in the context of 3D data) is used with 2D data, all but one
        one state become redundant. It is up to the code managing the 
        PlanarGraphics object to decide which state to keep and what to do with 
        the redundant ones.
        
            * decide which state should be retained for the (new) 2D data
            
                PlanarGraphics objects contain a reference to their "current 
                state", defined as the state or states VISIBLE in the context of
                the 3D data used; these can then be retained, and the others
                discarded.
        
            * retain ALL the states in the hope of later use with 3D data of
            the same shape as the data used when the PlanarGraphics object was 
            created (thus likely to generate a large overhead)
            
    It is up to the GUI viewer code to determine when a PlanarGraphics should be
    visible, and up to the code managing the PlanarGraphics to avoid having more
    than one state in the same frame.

    ATTENTION This mechanism has the potential to generate a huge overhead:
    
        * In the particular case where the shape has a different geometry in just
        one frame, the PlanarGraphics object must contain a sequence of states 
        with one state per frame, even if these states hold the same geometry in 
        all but one frame.
        
        * If not properly managed, accumulation of "invisible" states can cause 
        a large overhead.
    ...
    * * *
    ...
    def validateState(cls, value):
        ...
        if all_keys:
            ...
            valid_z_frame = isinstance(value.z_frame, (int, type(None)))
    ...
    def copyConvertState(cls, src):
        ...
        #if isinstance(src, PlanarGraphicsState):
            #ret = src.copy()
            
        #elif isinstance(src, DataBag):
            #ret = PlanarGraphicsState(src)
                
        #else: # covers dict and common subclasses such as collections.OrderedDict, traitlets Bunch, mappings etc
            #ret = PlanarGraphicsState(src)        
        
        # make sure it has a z_frame attribute
        if not hasattr(ret, "z_frame"):
            ret.z_frame = None
            
        #print("\tPlanarGraphics.copyConvertState for %s: ret keys ="% cls.__name__, 
              #[k for k in ret.keys()])
            
        # NOTE: 2020-11-01 14:25:30
        # check that all mandatory descriptors are present in src
        # z_frame was introduced later in development and it can be set to None
        # here
        #if any([k not in keys for k in cls._planar_descriptors_]):
            #raise ValueError("%s objects require the descriptors : %s but the following %s were supplied" % (cls.__name__, cls._planar_descriptors_, keys))
        ...
        #print("\t%s.copyConvertState:\nsrc = %s"% (cls.__name__, type(src).__name__), 
              #src, "\nret = %s"% type(ret).__name__, ret, "\n")
        #print()
    ...
    def __init_from_descriptors__(self, *args, frameindex:typing.Optional[typing.Iterable]=[],
        ...
        # NOTE: 2020-11-01 13:42:42
        # keep z_frame out, as this is not expected in the constructor params
        ...
        # z_frame is zet up below, using frameindex
        #self._states_ = [PlanarGraphicsState(dict(zip(descriptors, args)))]
    ...
    def __init_from_state_sequence__(self, *states, frameindex:typing.Optional[typing.Iterable]=[], 
                                     currentframe:int=0) -> None:
        #print("PlanarGraphics.__init_from_state_sequence__ for %s %s %s\n" 
              #% (self.__class__.__name__, self._graphics_object_type_.name, self._ID_, ))
        
        #print("\t\t", [type(s).__name__ for s in states])
        #print("PlanarGraphics.__init_from_state_sequence__ for %s %s %s" 
              #% (self._ID_, self.__class__.__name__, self._graphics_object_type_.name),
              #states, "\n")
    ...
    def __init__(self, *args, graphicstype=None, closed:bool=False, 
        ...
        *args: either:
            ...
            1) sequence of planar descriptors specific to the PlanarGraphics 
                type, but WITHOUT z_frame (NOTE: this will be specified via
                frameindex)
            
            2) a PlanarGraphics object (copy constructor); 
        ...
        frameindex: iterable (tuple, list, or range); default is empty: 
            contains the indices of data frames that associate states.
        ...
        if not isinstance(graphicstype, (type(None), int, str, GraphicsObjectType)):
            raise TypeError("graphicstype expected to be an int, a str, a pictgui.GraphicsObjectType, or None; got %s instead" % type(graphicstype).__name__)
        ...
        if len(args):
            #print("PlanarGraphics.__init__ %s %s %s\n" % (self._ID_, self.__class__.__name__, self._graphics_object_type_.name))
            
            if len(args) == 1:
                #print("PlanarGraphics.__init__ %s %s from" % (self._ID_, self.__class__.__name__), type(args[0]).__name__, 
                        #args[0])
                ...
                if isinstance(args[0], self.__class__): # COPY CONSTRUCTOR
                    ...
                    # ignores the following parameters:
                    #   graphicstype
                    #   closed
                    #   name
                    #   frameindex
                    ...
                    if all([isinstance(v, numbers.Real) for v in args[0]]):
                        # contruct from a sequence of planar descriptors
                        # check that the appropriate number of descriptors are given
                        # needs frameindex, currentframe, name
                        ...
                        # frameindex is ignored here, because these states SHOULD
                        # themselves provide the frame indices where they are visible
                        # i.e. the z_frame attribute
                        # also applies when args[0] has only one state
                        self.__init_from_state_sequence__(*args[0],
                                                          frameindex=frameindex,
                                                          currentframe=currentframe)
                        
                        
                    else:
                        raise TypeError("Expecting a sequence of planar descriptors or states")
                ...
                self.__init_from_descriptors__(*args)
    ...
    #def __getattr__(self, name):
        ...
        #"""
        ##if name in PlanarGraphics._required_attributes_:
            ##return self.__dict__[name]
            
        ## NOTE: 2020-11-09 16:00:57
        ## this call triggers infinite recursion!
        ##states = self.getStates() # without frame parameter this returns the "current states"
        ##states = object.__getattribute__(self,"_currentstates_")
            ...
            #else:
                #return self.__class__.defaultState()[name]
        ...
        ##except Exception as e:
            ##print("in PlanarGraphics.__getattr__(...):")
            ##traceback.print_exc()
            ##raise e
    ...
    #def __setattr__(self, name, value):
        ...
        #except Exception as e:
            #print("in PlanarGraphics.__setattr__(...):")
            #traceback.print_exc()
            #raise
    ...
    def _check_states_(self):
        """Ensures consistency of states wrt z_frames
        
        The object can have:
        
        1) A single state with ubiquitous frame visibility (z_frame is None),
        
        OR
        
        2) One state ('A') with z_frame < 0 and at most another state ('B) with
        z_frame >=0 such that:
        
            A.z_frame  == -1 * B.z_frame -1
            
        OR:
        
        3) Any number of states with z_frame >= 0 and UNIQUE values (i.e., NO
        TWO states can have the same z_frame value)
        
        There SHOULD BE exactly one one state for any given frame.
        ...
        if len(self._states_):
            ...
            ubiquitous_frame_states = [s for s in self._states_ if s.z_frame is None]
            negative_frame_states   = [s for s in self._states_ if isinstance(s.z_frame, int) and s.z_frame < 0]
            positive_frame_states   = [s for s in self._states_ if isinstance(s.z_frame, int) and s.z_frame >= 0]
            
            # make sure all positive_frame_states have unique z_frame values
            
            if len(positive_frame_states):
                positive_frame_states = unique(positive_frame_states, key=lambda x:x.z_frame)
            
            if len(ubiquitous_frame_states):
                # make sure there is only one state with ubiquitous frame; choose
                # the first from the list
                self._states_[:] = [ubiquitous_frame_states[0]]
                #return
                        
            elif len(negative_frame_states):
                # there can be at most two states: one with negative z_frame
                # (z_frame_a) and another with positive z_frame (z_frame_b) such
                # that:
                #
                # z_frame_b == -1 * z_frame_a -1
                
                # make sure there is only state with negative frame; choose the
                # first from the list
                negative_frame_state = negative_frame_states[0]
                
                avoided_frame = -negative_frame_state.z_frame -1
                
                # find state visible in the avoided frame
                states_for_avoided_frame = [s for s in positive_frame_states is s.z_frame == avoided_frame]
                
                if len(states_for_avoided_frame):
                    # make sure there is only one; choose the first from the list
                    state_visible_in_avoided_frame = states_for_avoided_frame[0]
                    
                    # max two states: one visible in all but frame X, and one
                    # visible only in frame X
                    self._states_[:] = [negative_frame_state, state_visible_in_avoided_frame]
                    
                else:
                    # there is only one state visible in all but frame X
                    self._states_[:] = [negative_frame_state]
                    
                    
            else:
                # no negative frame states exist, no ubiquitous frame states exist
                if len(positive_frame_states) == 0:
                    # no state defined?
                    self._states_[:] = [self.__class__.defaultState()]
                    
                else:
                    self._states_[:] = positive_frame_states
                    
        else: # no states defined - make sure there is one!
            self._states_ = [self.__class__.defaultState()]
            
        if len(self._states_):
            self.sortStates(none_last=False)
    ...
    def setFrameVisibility(self, ndx):
        pass
        # do something like:
        # get current state (i.e. state associated with current frame)
        #
        # if current state has z_frame None: (this should be the ONLY state)
        #   if ndx is None or the str "all" -> return (do nothing)
        #   if ndx is an int >= 0:
        #       set z_frame to ndx -> will have ONE state visible ONLY in ndx
        #   if ndx is an int < 0:
        #       set z_frame to ndx -> will have ONE state visible in all but 
        #       -1 * ndx -1
        #
        #
        # if current state has z_frame an int >= 0:
        #
    ...
    def _apply_frame_index_(self, frameindex:typing.Optional[typing.Iterable]=[],
        ...
        noneph = math.inf if none_last else -math.inf
        ...
        if sort:
            frameindex = sorted(frameindex, key = lambda x: x if x is not None else noneph)
        ...
        if len(frameindex):
            ...
            if len(states) == len(frameindex):
                ...
                if len(states) == 1:
                    ...
                    # below)
                    #
                    #print("PlanarGraphics._apply_frame_index_ in %s: frameindex =" % self.__class__.__name__, 
                          #frameindex)
    ...
    def sortStates(self, none_last=True):
        ...
        if len(self._states_) > 1:
            states = sorted(self._states_, key = lambda x: x.z_frame if (isinstance(x.z_frame, int) and x.z_frame >= 0) else -x.z_frame-1 if (isinstance(x.z_frame, int) and x.z_frame < 0) else noneph)
            self._states_[:] = states[:]
    ...
    def hasStateForFrame(self, frame):
        ...
        return self.getState(frame) is not None
    ...
    def indexOfStateWithFrame(self, state:typing.Optional[DataBag]=None, 
                           frame:typing.Optional[int]=None) -> typing.Optional[int]:
        if state is None:
            state = self.getState(frame=frame)
    ...
    def getState(self, frame:typing.Optional[int]=None) -> typing.Optional[object]:
        """Get a reference to the visible state associated with the specified frame index.
        TODO use as delegate for hasStateForFrame
        
        Parameters:
        ----------
        frame: int or None; 
            When None, return (a reference to) the state associated with self.currentFrame
            
            When an int, it must be >= 0.
            
        Returns:
        -------
        A DataBag object (state) which can be:
        
        * the unique state with ubiquitous visibility (i.e., has z_frame None)
            - this will always be returned, regardless of the value of the 
                'frame' parameter
                
        OR:
            
        * the state visible in 'frame'; this is either:
            - the state with z_frame == frame
            
            or
            
            - the state explicitly invisible in another frame, i.e. with 
                z_frame != -1 *` frame -1
        ...
        if frame < 0:
            raise ValueError("'frame' must be >= 0; got %d instead" % frame)
            
        self._check_states_()
            
        if len(self._states_) == 1: # trivial
            if self.__class__.isStateVisible(self._states_[0], frame):
    ...
    def setFrameForState(self, state:DataBag, new_frame:typing.Optional[int]=None,
        ...
        frndx = [s.z_frame for s in self._states_ if isinstance(s.z_frame, int) and s.z_frame >=0 ]
        
        avoided_frames = [-s.z_frame-1 for s in self._states_ if isinstance(s.z_frame, int) and s.z_frame < 0]
        
        if any([len(v)>0 for v in (frndx, avoided_frames)]):
            if new_frame is None:
                max_frame = max(frndx + avoided_frames)
            else:
                max_frame = max(frndx + avoided_frames + [new_frame])
                
        else:
            if new_frame is None:
                max_frame = 0 # irrelevant for ubiquitous state
                
            else:
                max_frame = new_frame
            
        if new_frame is None:
            # the state is set to become ubiquitous; 
            # therefore all other states must be discarded
            state.z_frame = None
            self._states_[:] = [state]
            
        elif isinstance(new_frame, int): # a new frame is assigned to state
            if new_frame >= 0: # state set to become a single-frame state, while:
                if state.z_frame is None: # initially it is an ubiquitous state
                    # this should be the ONLY state
                    state.z_frame = new_frame # done
                    # also make sure there is not gremlin state left behind
                    self._states_[:] = [state]
                    
                else: # initially is either a single-frame or frame-avoiding
                    if state.z_frame >= 0: # state is initially a single-frame state
                        # check to see if there is another state visible at new frame
                        # this can be a frame-avoiding state (avoiding a different frame)
                        # or just another single-frame state
                        #
                        # if there is one, THAT state needs to be discarded and 
                        # replaced with the (modified) state
                        # 
                        # since there is only one copy of the state to be modified,
                        # we just change its z_frame value
                        
                        # 1) search for existing single-frame state at new frame
                        old_state = self.__class__.findStateWithZFrame(self._states_, new_frame)
                        
                        if old_state is None: # found no existing single-frame state at new_frame
                            # check if there is a frame-avoiding state that is 
                            # visible in new_frame
                            sa = [o for o in self._states_ if o.z_frame < 0]
                            
                            if len(sa): # found a frame-avoiding state 
                                # NOTE: theoretically there should be only one
                                fa_state = sa[0]
                                
                                avoided_frame = -fa_state.z_frame - 1
                                
                                #if fa_state.z_frame == -new_frame - 1:
                                if avoided_frame == new_frame:
                                    # fa_state avoids the new_frame which we want
                                    # to attach to the state to be modified 
                                    #
                                    # This is OK: self._states_ should only have two
                                    # states: the fa_state (unmodifed) that avoids
                                    # new frame, and the (now modified) state which 
                                    # is linked to new frame - no clash here.
                                    #
                                    # NOTE: HOWEVER: this contingency should only
                                    # when the state to be modified, which is a 
                                    # single-frame state, is already linked to 
                                    # the acoided frame (otherwise it could NOT
                                    # have existed in self._states_)
                                    # 
                                    # So, this branch only gets executed when
                                    # state.z_frame already equals new_frame (i.e.
                                    # it changes nothing, except for consolidating
                                    # the one frame - one visible state rule)
                                    #
                                    state.z_frame = new_frame
                                    self._states_ = [fa_state, state] # done
                                    
                                else: # frame_clash
                                    # this is the most likely to happen;
                                    # the state is initially linked to the frame
                                    # avoided by fs_state (otherwise it would not
                                    # have existed in self._states_); new_frame 
                                    # is one of the  frames where fa_state is visible.
                                    # 
                                    # Therefore we replicate fa_state to generate
                                    # a set of single-state frames visible in all 
                                    # frames EXCEPT new frame and its originally 
                                    # avoided frame (which may or may not have 
                                    # an associated state)
                                    #
                                    
                                    coverted_states = list()
                                    
                                    # Is there a state at the avoided frame?
                                    other_state = self.__class__.findStateWithZFrame(self._states_,
                                                                                     avoided_frame)
                                    
                                    if other_state is not None: # state found at avoided_frame
                                        converted_states.append(other_state)
                                    ...
                                    # replicate fa_state to a set of single-frame
                                    # states visible everywhere except the avoided_frame
                                    for k in range(nFrames):
                                        if k not in (avoided_frame, new_frame):
                                            s = fa_state.copy()
                                            s.z_frame = k
                            ...
                            else: # no frame-avoding state found
                                # just set the state's z_frame to new_frame and we're done
                                # self._states_ still hold only ONE reference to 
                                # state
                                state.z_frame = new_frame # done
                                        
                        else: # a single-frame state at new_frame is found:
                            # discard the old state at new frame
                            # replace that state with this state
                            # this effectively means we get rid of the (old)
                            # state that exists at new frame, ending up with one
                            # one state less than before.
                            
                            # NOTE: since both state and old_state are single-frame states
                            # technicaly self._states_ should contain NO
                            # frame-avoiding or ubiquitous states
                            old_ndx = index_of(self._states_, old_state, key = lambda x: x.z_frame)
                            ndx = index_of(self._states_, state, key = lambda x: x.z_frame)
                            
                            # where was the state argument located?
                            #initial_ndx = index_of(self._states_, state, key = lambda x: x.z_frame)
                            
                            state.z_frame = new_frame # set this NOW, and not before to avoid messing up the index_of result
                            
                            # now replace the old state
                            if old_ndx is not None: # NOTE: having found an old_state, this is technically always True
                                self._states_[old_ndx] = state
                                # remove the reference to the modifed state from
                                # the previous location to avoid storing two
                                # references to the same object
                                if ndx is not None: # NOTE: since state belongs ot self._states_ this is technically always True
                                    self._states_.pop([ndx])
                                
                    else: # state is initially a frame-avoiding state
                        # by rule (II) there can be at most one other state which
                        # is single-frame and linked to the frame initially 
                        # avoided by state
                        #
                        
                        avoided_frame = -state.z_frame - 1
                        
                        
                        # two things can happen:
                        # a) new_frame is the frame being avoided
                        # b) new_frame is the frame where state is already visible
                        #
                        if new_frame == avoided_frame: # new_frame is the avoided frame
                            if isinstance(nFrames, int) and nFrames <= max_frames:
                                nFrames = max_frames + 1
                            ...
                            # check if there already is a visible state at the avoided_frame
                            old_state = self.__class__.findStateWithZFrame(self._states_, new_frame)
                            
                            if old_state is None: # no single-frame state exists at the avoided frame
                                # by rule (II) then state is expected to be the ONLY
                                # state in self._states_
                                # Therefore we need to replicate state such that
                                # it becomes visible in all frames including the
                                # new_frame.
                                #
                                # It is tempting to set its z_frame to None, but 
                                # this is not what is being asked here
                                
                                for k in range(nFrames):
                                    if k != new_frame: # new_frame IS avoided_frame
                                        s = state.copy()
                                        s.z_frame = k
                                        converted_states.append(s)
                                        
                            else: # there already is an old state at new frame
                                # this needs replacing => we end up with one 
                                # state less than initially
                                for k in range(nFrames):
                                    s = state.copy()
                                    s.z_frame = k
                                    converted_states.append(s)
                        ...
                        else: # new_frame is one of the frames where state was
                            # initially visible; since state is frame_avoiding
                            # there should be no other state associated with 
                            # new frame;
                            # but there might be a state associated with the 
                            # avoided frame
                            ...
                            old_state = self.__class__.findStateWithZFrame(self._states_, avoided_frame)
                            
                            converted_states = list()
                        
                            if old_state is not None:
                                converted_states.append(old_state)
            ...
            else: # state is to become a frame_avoiding state # TODO
                if state.z_frame is None: # state is initially an ubiquitous state
                    # hence it should be unique
                    # we just set its frame to the new value and we're done
                    state.z_frame = new_frame
                    self._states_[:] = [state]
                    
                elif state.z_frame >= 0: # state is initially a single-frame state
                    ...
                    if len(fa_states):
                        ...
                        # technically state is initially the only single-frame state
                        ndx = index_of(self._states_, state, key = lambda x: x.z_frame)
                        
                        if ndx is not None: # remove this state
                            self._states_.pop([ndx])
                            
                        # we now want state to replace the fa_state
                        old_ndx = index_of(self._states_, fa_state, key = lambda x: x.z_frame)
                        
                        if old_ndx is not None:
                            state.z_frame = new_frame
                            self._states_[old_ndx] = state
                            
                    else: # no frame-avoiding states
                        # figure out which frame will be avoided
                        avoided_frame = -new_frame - 1
                        
                        # is there an old state at the avoided frame?
                        old_state = self.__class__.findStateWithZFrame(self._states_, avoided_frame)
                        
                        state.z_frame = new_frame
                        
                        if old_state is not None: # ther already is a state there
                            self._states_[:] = [state, old_state]
                            
                        else: # no state at the (new) avoided frame
                            # therefore, our state will become the only one
                            # (frame-avoiding at new_frame, visible elsewhere)
                            self._states_[:] = [state]
                        
                else: #state is initally a frame-avoiding state
                    # check if the new avoided frame is the same as its old
                    # avoided frame; if they are, then there's nothing to do
                    if state.z_frame != new_frame:
                        # even if there is an old state at the previously 
                        # avoided frame, changing this state z_frame to 
                        # a new avoided frame will overwrite the visible state
                        # elsewhere
                        old_avoided_frame = -state.z_frame - 1
                        old_state = self.__class__.findStateWithZFrame(self._states_, old_avoided_frame)
                        
                        if old_state is not None:
                            ndx = index_of(self._states_, old_state, key = lambda x: x.z_frame)
    ...
    def setState(self, state:DataBag, frame:typing.Optional[int], nFrames:typing.Optional[int]=None):
        ...
        # (II)  one frame-avoding state and a single-frame state (for the avoided frame)
        # (III) many single-frame states
        self._check_states_()
    ...
    def frameIndices(self, values:typing.Optional[typing.Iterable]):
        """FIXME Re-maps the frame indices in existing states as indicated by "values".
        
        NOTE: 2020-11-02 08:54:48
        The parameters 'values' must be an interable and should ideally have 
        ...
        values: One of:
                ...
                1.b) may be a sequence with all None elements
                1.c) may be an empty sequence
...
class Line(PlanarGraphics):
    ...
    _graphics_object_type_ = GraphicsObjectType.line # or point?
...
class Cursor(PlanarGraphics):
    ...
    type: GraphicsObjectType.crosshair_cursor, horizontal_cursor, vertical_cursor, point_cursor
...
class Path(PlanarGraphics):
            ...
            if len(args) == 1:
                if isinstance(args[0], Path):
                    ...
                    if all([isinstance(p, PlanarGraphics) for p in args[0]]):
                        # NOTE: build Path from iterable of PlanarGraphics
                        # passed  the single var-positional parameter
                    ...
                    for k in range(args[0].elementCount()):
                        if args[0].elementAt(k).type == QtGui.QPainterPath.MoveToElement:
                            ...
                            self._objects_.append(Move(element.get("x"), element.get("y"), 
                                                       frameindex=frameindex, 
                                                       currentframe=currentframe))
                            
                        elif args[0].elementAt(k).type == QtGui.QPainterPath.LineToElement:
                            element = args[0].elementAt(k)
                            self._objects_.append(Line(element.get("x"), element.get("y"),
                                                       frameindex=frameindex, 
                                                       currentframe=currentframe))
                            ...
                            self._objects_.append(Cubic(c1.get("x"), c1.get("y"), 
                                                        c2.get("x"), c2.get("y"), 
    ...
    def __call__(self, path=None, frame=None, closed=None, connected=False):
        ...
        if len(self):
            ...
            if closePath:
                ...
                if state and len(state):
                    path.lineTo(self._objects_[0].get("x"), 
                                self._objects_[0].get("y")) 
    ...
    def x(self, value):
        if len(self._objects_):
            states = self.getState(self.currentFrame)
            if len(states):
                old_x = min([s.get("x") for s in states])
                ...
                for s in states:
                    s.set("x", s.get("x") + delta_x)
                    
                    if "cx" in s.__class__._planar_descriptors_:
                        s.set("cx", s.get("cx") + delta_x)
                        
                    if "c1x" in s.__class__._planar_descriptors_:
                        s.set("c1x", s.get("c1x") + delta_x)
                        
                    if "c2x" in s.__class__._planar_descriptors_:
                        s.set("c2x", s.get("c2x") + delta_x)
    ...
    def y(self, value):
        if len(self._objects_):
            ...
            if len(states):
                old_y = min([s.get("y") for s in states])
                delta_y = value - old_y
                ...
                for s in states:
                    s.set("y", s.get("y") + delta_y)
                    
                    if "cy" in s.__class__._planar_descriptors_:
                        s.set("cy", s.get("cy") + delta_y)
                        
                    if "c1y" in s.__class__._planar_descriptors_:
                        s.set("c1y", s.get("c1y") + delta_y)
                        
                    if "c2y" in s.__class__._planar_descriptors_:
                        s.set("c2y", s.get("c2y") + delta_y)
    ...
    def closed(self, value):
        ...
        if value:
            self.append(Line(self._objects_[0].get("x"), 
                             self._objects_[0].get("y")))
            
        else:
            # remove last element (break-up path)
            if self._objects_[-1].get("x") == self._objects_[0].get("x") and self._objects_[-1].get("y") == self._objects_[0].get("y"):
    ...
    def adoptPainterPath(self, p):
        ...
        for k in range(p.elementCount()): # does not parse CurveToDataElement objects
            if p.elementAt(k).type == QtGui.QPainterPath.MoveToElement:
                element = p.elementAt(k)
                self.append(Move(element.get("x"), element.get("y"), 
                                 frameindex=frameindex, currentframe=currentframe))
                
            elif p.elementAt(k).type == QtGui.QPainterPath.LineToElement:
                element = p.elementAt(k)
                self.append(Line(element.get("x"), element.get("y"), 
                                 frameindex=frameindex, currentframe=currentframe))
                ...
                c1 = p.elementAt(k+1)
                c2 = p.elementAt(k+2)
                self.append(Cubic(x=c1.get("x"), y=c1.get("y"), 
                                  c1x=c2.get("x"), c1y=c2.get("y"), 
                                  c2x=element.get("x"), c2y=element.get("y"), 
                                  frameindex=frameindex, currentframe=currentframe))
...
class GraphicsObject(QtWidgets.QGraphicsObject):
    ...
    def __init__(self, 
        ...
        if self.__backend__ is not None:
            ...
            if self not in self.__backend__.frontends:
    ...
    def __parametric_constructor__(self, parameters, pos, frameindex=None, currentframe=0):
        """Called when there is no PlanarGraphics backend.
    ...
    def __parse_parameters__(self, parameters, pos, frameindex = None, currentframe = 0):
        ...
        2.2) for shaped objects (i.e. antythgin but cursors), use the planar 
            graphics passed in 'parametsrs' as backend then generate a cached
            cached path from the state descriptor associated with the current frame
        ...
        if isinstance(parameters, (Cursor, Ellipse, Rect, Path, Text, str)):
            if isinstance(parameters, (Cursor, Ellipse, Rect, Path)):
                if isinstance(parameters, Path):
                    if len(parameters) == 0: # avoid empty paths
                        ...
                        # convert a 1-element Path into a PlanarGraphics primitive
    ...
    def __updateCachedPathFromBackend__(self):
        ...
        if isinstance(self.__backend__, (Ellipse, Rect)):
            self.__cachedPath__ = Path(Move(self.__backend__.get("x"), 
                                            self.__backend__.get("y")),
                                       Line(self.__backend__.get("x") + self.__backend__.get("w"),
                                            self.__backend__.get("y") + self.__backend__.get("h")))
    ...
    def shape(self):
        ...
        if self.objectType & GraphicsObjectType.allCursorTypes:
            ...
            if self.__backend__ is not None and self.__backend__.hasStateForFrame(self._currentframe_) and self.__backend__() is not None:
                ...
                if self._objectType == GraphicsObjectType.line:
                    p0 = self.mapFromScene(QtCore.QPointF(self.__backend__[0].get("x"), 
                                                          self.__backend__[0].get("y")))
                    
                    p1 = self.mapFromScene(QtCore.QPointF(self.__backend__[1].get("x"), 
                                                          self.__backend__[1].get("y")))
                ...
                if self.editMode:
                    ...
                    if len(self.__cachedPath__) > 1:
                        for k, element in enumerate(self.__cachedPath__):
                            pt = self.mapFromScene(QtCore.QPointF(element.get("x"), 
                                                                  element.get("y")))
        ...
        try:
            ...
            if self.objectType & GraphicsObjectType.allCursorTypes:
                ...
                if self._objectType == GraphicsObjectType.vertical_cursor:
                    ...
                    labelPos = self.mapFromScene(QtCore.QPointF(self.__backend__.get("x") - self._labelRect.width()/2, 
                                                                self._labelRect.height()))

                elif self._objectType == GraphicsObjectType.horizontal_cursor:
                    lines = [self._hline, self._vwbar]
                    labelPos = self.mapFromScene(QtCore.QPointF(0, self.__backend__.get("height")/2 - self._labelRect.height()/2))
                    ...
                    rects = [self._crect]
                    labelPos = self.mapFromScene(QtCore.QPointF(self.__backend__.get("x") - self._labelRect.width()/2,
                                                                self.__backend__.get("y") - self._labelRect.height()))
                ...
                if self._buildMode: # this only makes sense for ROIs
                    ...
                    for k, element in enumerate(self.__cachedPath__):
                        painter.drawEllipse(element.get("x") - self._pointSize,
                                            element.get("y") - self._pointSize,
                                            self._pointSize * 2., 
                                            self._pointSize * 2.)
                    ...
                    if self.__backend__ is not None:
                        if self._objectType == GraphicsObjectType.ellipse:
                            r_ = self.mapRectFromScene(self.__backend__.get("x"),
                                                       self.__backend__.get("y"),
                                                       self.__backend__.get("w"),
                                                       self.__backend__.het("h"))
                            ...
                            r_ = self.mapRectFromScene(self.__backend__.get("x"),
                                                       self.__backend__.get("y"),
                                                       self.__backend__.get("w"),
                                                       self.__backend__.get("h"))
                            
                            painter.drawRect(r_)
                                                                            
                        elif self._objectType == GraphicsObjectType.point:
                            p_ = self.mapFromScene(self.__backend__.get("x"),
                                                   self.__backend__.get("y"))
                            ...
                            if self.objectType & GraphicsObjectType.path:
                                for k, element in enumerate(self.__cachedPath__):
                                    if isinstance(element, Quad):
                                        pt = self.mapFromScene(QtCore.QPointF(element.get("x"), element.get("y")))
                                        cp = self.mapFromScene(QtCore.QPointF(element.get("cx"), element.get("cy")))
                                        ...
                                        pt  = self.mapFromScene(QtCore.QPointF(element.get("x"), element.get("y")))
                                        cp1 = self.mapFromScene(QtCore.QPointF(element.get("c1x"), element.get("c1y")))
                                        cp2 = self.mapFromScene(QtCore.QPointF(element.get("c2x"), element.get("c2y")))
                                ...
                                p0 = self.mapFromScene(QtCore.QPointF(self.__cachedPath__[0].get("x"), 
                                                                      self.__cachedPath__[0].get("y")))
                                
                                p1 = self.mapFromScene(QtCore.QPointF(self.__cachedPath__[1].get("x"),
                                                                      self.__cachedPath__[1].get("y")))
        ...
        if change == QtWidgets.QGraphicsItem.ItemPositionChange and self.scene():
            ...
            if self.objectType & GraphicsObjectType.allCursorTypes:
                ...
                if self._objectType == GraphicsObjectType.vertical_cursor:
                    ...
                    elif newPos.x() > self.__backend__.get("width"):
                        newPos.setX(self.__backend__.get("width"))
                ...
                # horizontal cursors
                    ...
                    elif newPos.y() > self.__backend__.get("height"):
                        newPos.setY(self.__backend__.get("height"))
                ...
                # crosshair cursors
                    ...
                    if newPos.x() > self.__backend__.get("width"):
                        newPos.setX(self.__backend__.get("width"))

                    if newPos.y() <= 0.0:
                        newPos.setY(0.0)
                        
                    if newPos.y() > self.__backend__.get("height"):
                        newPos.setY(self.__backend__.get("height"))
                        
                    self._deltaPos = (newPos - QtCore.QPointF(self.__backend__.get("x"), 
                                                              self.__backend__.get("y")))
                ...
                # here; 
                self.__backend__.set("x", newPos.x())
                self.__backend__.set("y", newPos.y())
                
            elif self._objectType & GraphicsObjectType.allShapeTypes:
                #non-cursor types
                if isinstance(self.__backend__, Path): # use the x and y property setters
                    self.__backend__.x = value.x()
                    self.__backend__.y = value.y()
                    
                else:
                    self.__backend__.set("x", value.x())
                    self.__backend__.set("y", value.y())
        ...
        if self.editMode: # this is ALWAYS False for cursors
            ...
            for k, p in enumerate(self.__cachedPath__):
                if isinstance(p, Quad):
                    ...
                    dc1 = QtCore.QLineF(evt.pos(), self.mapFromScene(QtCore.QPointF(p.get("cx"), p.get("cy")))).length()
                    ...
                    d = QtCore.QLineF(evt.pos(), self.mapFromScene(p.point())).length() # d = length of line between event pos and point pos
                    dc1 = QtCore.QLineF(evt.pos(), self.mapFromScene(QtCore.QPointF(p.get("c1x"), p.get("c1y")))).length()
                    dc2 = QtCore.QLineF(evt.pos(), self.mapFromScene(QtCore.QPointF(p.get("c2x"), p.get("c2y")))).length()
                    #print("Cubic")
            ...
            if self.editMode and self.__cachedPath__ is not None and len(self.__cachedPath__) \
                ...
                if isinstance(element, Move):
                    ...
                    if self._c_activeControlPoint == 0:
                        ...
                        self.__cachedPath__[self._c_activePoint] = Quad(cx=element.get("cx"), cy=element.get("cy"),
                                                                        x=epos.x(), y=epos.y())
                    
                elif isinstance(element, Cubic):
                    if self._c_activeControlPoint == 0:
                        self.__cachedPath__[self._c_activePoint] = Cubic(c1x=epos.x(), c1y=epos.y(),
                                                                         c2x=element.get("c2x"), c2y=element.get("c2y"),
                                                                         x=element.get("x"), y=element.get("y"),)
                        
                    elif self._c_activeControlPoint == 1:
                        self.__cachedPath__[self._c_activePoint] = Cubic(c1x=element.get("c1x"), c1y=element.get("c1y"),
        ...
        if self.objectType & GraphicsObjectType.allCursorTypes:
            ...
            if self._objectType == GraphicsObjectType.vertical_cursor:
                ...
                if evt.key() == QtCore.Qt.Key_Right:
                    ...
                    if newX > self.__backend__.get("width")-1:
                        moveX = self.__backend__.get("width")-1 - self.pos().x()
                    ...
                    if newY > self.__backend__.get("height")-1:
                        moveY = self.__backend__.get("height")-1 - self.pos().y
                ...
                elif self.pos().x() > self.__backend__.get("width")-1:
                    y = self.pos().y()
                    self.setPos(self.__backend__.get("width")-1, y)
                    
                if self.pos().y() < 0:
                    x = self.pos().x()
                    self.setPos(x, 0)
                    
                elif self.pos().y() > self.__backend__.get("height")-1:
                    x = self.pos().x()
                    self.setPos(x, self.__backend__.get("height")-1)
    ...
    def __updateBackendFromCachedPath__(self):
        ...
        if isinstance(self.__backend__, (Ellipse, Rect)) and len(self.__cachedPath__) >= 2:
            if self.hasStateDescriptor:
                self.__backend__.set("x", self.__cachedPath__[0].get("x"))
                
                self.__backend__.set("y", self.__cachedPath__[0].get("y"))
                
                self.__backend__.set("w", self.__cachedPath__[1].get("x") - self.__cachedPath__[0].get("x"))
                
                self.__backend__.set("h", self.__cachedPath__[1].get("y") - self.__cachedPath__[0].get("y"))
            ...
            try:
                for k, element in enumerate(self.__cachedPath__):
                    self.__backend__[k].set("x", element.get("x"))
                    self.__backend__[k].set("y", element.get("y"))
    ...
    def currentBackendFrame(self, value):
        ...
        self.__backend__.currentFrame=float(value)
        self.__backend__.updateLinkedObjects()
