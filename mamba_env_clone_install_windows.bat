...
set env_yaml=%mydir%\mamba_environment_exported_25c05.yml
rem  goto install_own_console_styles
:create_env
rem  set default_env_path="c:\scipyenv"
rem  set /P env_path="Enter the full path name of the new environment (no spaces, please, default is: %default_env_path%): "
rem  if [%env_path%] equ [] set env_path=%default_env_path%
echo Creating mamba environment from clone
call mamba env create --file %env_yaml%
