...
def remove_events(event, segment, byLabel=True):
    """Removes an event from the neo.Segment "segment"
...
def is_same_as(e1, e2, rtol = 1e-4, atol =  1e-4, equal_nan = True):
    ...
    if any([not isinstance(e, neo.Event) or "Event" not in type(e).__name__ for e in (e1,e2)]):
        return False
    
    # because importing triggerprotocols creates dependency loop
    #if all([isinstance(e, TriggerEvent) for e in (e1,e2)]):
    if all(["Event" in type(e).__name__ for e in (e1,e2)]):
