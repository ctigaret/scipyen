...
This is the axonio module from neo.io (python3 version) copied here as a different module
in order to try and fix troubles with AxonIO imports under python3
...
NOTE: Modified by Cezar Tigaret on 2017-03-04 11:22:38 to comply with python3 
NOTE: distinction between float and integer divsion
"""

#NOTE:2017-03-04 09:11:47 CMT
...
class AxonIO(BaseIO):
    ...
    def read_block(self, lazy=False, cascade=True):
        ...
        # NOTE: 2017-03-04 09:12:27
        #print(header)        # NOTE: OK so header is a dictionary;
        
        version = header['fFileVersionNumber']# NOTE: OK so header is a dictionary;
        ...
        if (mode == 1) or (mode == 2) or (mode == 5) or (mode == 3):
            ...
            for j in range(episodArray.size):
                ...
                print("j=", j, " seg=", seg, " length=", length, " pos=", pos)
                ...
                if not lazy:
                    ...
                    # NOTE: 2017-03-04 10:13:07
                    # NOTE: in python3 the '/' operator indicates floating point division
                    # NOTE: (python 2) type(6/1) => type 'int'
                    # NOTE: (python 2) type(6//1) => type 'int'
                    # NOTE: (python 3) type(6/1) => float
                    # NOTE: (python 3) type(6//1) => int
                    # NOTE: and the result is coerced to a float, however the resize()
                    # NOTE: function takes integers values as arguments; 
                    # NOTE: therefore we can either cast the variables
                    # NOTE: ourselves (e.g. int(subdata.size/nbchannel)) or 
                    # NOTE: directly use integer division operator '//' as below
    ...
    def read_protocol(self):
        ...
        # NOT: again, nSam expected to be an int, so in python3 we need to use //
        nSam = header['protocol']['lNumSamplesPerEpisode']//nADC  # samples/ep
        ...
        for epiNum in range(nEpi):
            ...
            for DACNum in range(nDAC):
                ...
                # NOTE: 2017-03-04 19:26:29 in python 3 unit is of type <class 'bytes'>
                # NOTE: but should be str
                unit = str(header['listDACInfo'][DACNum]['DACChUnits'])
                # NOTE: 2017-03-04 19:39:31 python 3 doesn't like consructing what
                # NOTE:  is effectively a unit quantity by using the pq.Quantity constructor
                # NOTE: the correct way I think is simply constructing a UnitQuantity
                signal = np.ones(nSam) *\
                    header['listDACInfo'][DACNum]['fDACHoldingLevel'] *\
                    pq.UnitQuantity(unit)
                #signal = np.ones(nSam) *\
                    #header['listDACInfo'][DACNum]['fDACHoldingLevel'] *\
                    #pq.Quantity(1.0, unit)
                anaSig = AnalogSignal(signal, sampling_rate=sampling_rate,
                ...
                if DACNum in header['dictEpochInfoPerDAC']:
                    ...
                    for epochNum, epoch in iteritems(epochInfo):
                        ...
                        dif = i_end-i_begin
                        # NOTE: 2017-03-04 19:41:05 see NOTE at line 590 & 593
                        anaSig[i_begin:i_end] = np.ones(len(range(dif))) *\
                            pq.UnitQuantity(unit) * (epoch['fEpochInitLevel'] +
