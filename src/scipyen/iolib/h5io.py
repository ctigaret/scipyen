...
def group2neoContainer(g:h5py.Group, target_class:type, cache:dict = {}):
    ...
    if "annotations" in g:
        annotations = fromHDF5(g["annotations"], cache)
...
def group2neoSignal(g:h5py.Group, target_class:type, cache:dict = {}):
    ...
    if isinstance(segment_entity, h5py.Group):
        ...
        # in the next call, fromHDF5 will either:
        ...
        segment = fromHDF5(segment_entity, cache)
    ...
    if isinstance(annotations_group, h5py.Group):
        ...
        annotations = fromHDF5(annotations_group)
    else:
        annotations = dict()
...
def group2neo(g:h5py.Group, target_class:type, cache:dict = {}):
    ...
    if neo.core.dataobject.DataObject in mro:
        ...
        if isinstance(signal_entity, h5py.Group):
            signal = fromHDF5(signal_entity, cache)
...
def makeAxisScale(obj, dset:h5py.Dataset, axesgroup:h5py.Group, dimindex:int,
    ...
    if isinstance(obj, (neo.IrregularlySampledSignal, IrregularlySampledDataSignal,
        ...
        axis_dset = toHDF5(obj.times, axesgroup,
                                     name = axis_dset_name,
...
@objectToEntity.register(pd.DataFrame)
@objectToEntity.register(pd.Series)
def _(obj:typing.Union[pd.DataFrame, pd.Series], 
      obj_attrs:dict, group:h5py.Group, name:typing.Optional[str]=None,
      target_name:typing.Optional[str]=None,
      compression:typing.Optional[str]="gzip",
      chunks:typing.Optional[bool]=None,
...
@objectToEntity.register(vigra.VigraArray)
@objectToEntity.register(neo.core.dataobject.DataObject)
def _(obj:typing.Union[vigra.VigraArray, neo.core.dataobject.DataObject], 
      obj_attrs:dict, group:h5py.Group, name:typing.Optional[str]=None,
      target_name:typing.Optional[str]=None,
      compression:typing.Optional[str]="gzip",
      chunks:typing.Optional[bool]=None,
...
@objectToEntity.register(neo.ChannelView)
def _(obj:neo.ChannelView, 
      obj_attrs:dict, group:h5py.Group, name:typing.Optional[str]=None,
      target_name:typing.Optional[str]=None,
      compression:typing.Optional[str]="gzip",
      chunks:typing.Optional[bool]=None,
...
@objectToEntity.register(enum.Enum)
def _(obj:enum.Enum, 
      obj_attrs:dict, group:h5py.Group, name:typing.Optional[str]=None,
      target_name:typing.Optional[str]=None,
      compression:typing.Optional[str]="gzip",
...
@objectToEntity.register(taxonbridge.Taxon)
def _(obj:taxonbridge.Taxon, 
      obj_attrs:dict, group:h5py.Group, name:typing.Optional[str]=None,
      target_name:typing.Optional[str]=None,
      compression:typing.Optional[str]="gzip",
      chunks:typing.Optional[bool]=None,
...
@objectToEntity.register(bgbridge.BrainGlobeAtlas)
...
def toHDF5(obj, group:h5py.Group, name:typing.Optional[str]=None,
    ...
    way to initialize it, using the data stored in the HDF5 entity tree.
    ...
    the type provides its own "toHDF5" method — usually, an instance 
    ...
    own "toHDF5" method, try to see is the caller of this function has
    supplied a custom function for this purpose (passed as the keyword 
    "toHDF5", and expecting the same parameters as this function); if 
    ...
    if entity_factory_method is None:
        entity_factory_method = kwargs.pop("toHDF5", None)
        
    # 3. a toHDF5 method or function is available from (1) or (2) above
    #   → use it 
...
def _(obj:pathlib.Path, group, attrs, name, compression, chunks, track_order, 
    ...
    try:
        ...
        # replace NULLs with 0:
        # o = obj.replace("\0", "0")
        if len(data.strip().strip(b"\x00".decode())) == 0: # empty string
            dset = group.create_dataset(name, data = h5py.Empty("f"), track_order=track_order)
        else:
...
def _(obj:neo.core.dataobject.DataObject, group, attrs, name, compression, chunks, 
    ...
    #     else:
    #         labels_dset = toHDF5(labels, group, 
    #                                     name ="labels",
    #                                     compression = compression,
    #                                     chunks = chunks)
    ...
    if isinstance(annotations, dict) and len(annotations):
        ...
        if isinstance(cached_entity, h5py.Group):
            ...
            annot_group = toHDF5(annotations, group, "annotations", 
                                        compression=compression, chunks=chunks,
                                        track_order = track_order)
...
def _(obj:neo.core.container.Container, group, attrs, name, compression, chunks, 
    ...
    if hasattr(obj, "annotations"):
        ...
        annotations_entity = toHDF5(annotations, grp, "annotations",
                                                    compression = compression, 
                                                    chunks = chunks,
                                                    track_order = track_order,
                                                    entity_cache = entity_cache,
...
def _(entity:h5py.Group, target_class:type, attrs:dict, cache:dict=dict()):
    ...
    if target_class == vigra.VigraArray:
        ...
        # elif any(k in mro for k in (list, NeoObjectList)):
            ...
            item_types = list()
            for k in entity.keys():
                # print(f"entity key {k} in {target_class.__name__}")
                o = fromHDF5(entity[k], cache)
                items.append(o)
