"""
CEDSignal2RawIO is a class for reading data files recorded with CED Signal2 
software.
...
alnumpunct = alnum + string.punctuation

class CEDSignal2RawIO(BaseRawWithBufferApiIO):
    """
...
def parse_cfs_data(filename):
    with open(filename, "rb") as fid:
        # NOTE: 2025-03-22 22:42:29
        # StructFile methods in addition to those of BufferedReader: 
        #   read_f(fmt, offset=None) 
        f = StructFile(fid)
        
        # f_file_signature = f.read(8)
        f_file_signature = f.read_f("8s", 0)
        # print(f"f_file_signature =  {f_file_signature}")
        if not (f_file_signature[0].startswith(b'CEDFILE')):
            return
        
        f_name = f.read()
        
        header=dict()
        for key, offset, fmt in CEDHeaderDescription:
            val = f.read_f(fmt, offset=offset)
            if len(val) == 1:
                if ("s") in fmt:
                    try:
                        header[key] = val[0].decode()
                    except:
                        header[key] = val[0]
                else:
                    header[key] = val[0]
                if key == "fFileSignature":
                    header["fFileVersionNumber"] = string.punctuation.index(header["fFileSignature"][-1]) + 1
            else:
                if "c" in fmt:
                    vl = list()
                    for v in val:
...
CEDHeaderDescription = [
    ("fFileSignature", 0x00, "8s"),
    ("sFileName", 0x09, "13c"),
    ("iFileSize", 0x16, "1I"),
    ("sFileTime", 0x1a, "8s"),
    ("sFileDate", 0x22, "8s"),
    ("iNDataChannels", 0x2a, "1h"),
    ("iNFileVariables", 0x2c, "1h"),
    ("iNDataVariables", 0x2e, "1h"),
    ("iFileHeaderSize", 0x30, "1h"),
    ("iDataHeaderSize", 0x32, "1h"),
    ("iLastDataSectionHeaderOffset", 0x34, "1I"),
    ("iNDataSections", 0x38, "2H"),
    ("iDiskBlockSizeRounding", 0x3a, "2H"),
    ("sFileComment", 0x3c, "73c"),
    ("iPointerTableOffset", 0x86, "1I"),
    ]
