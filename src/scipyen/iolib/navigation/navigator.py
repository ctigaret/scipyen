...
import typing, pathlib, functools, os, itertools, sys, traceback # noqa
from functools import singledispatch, singledispatchmethod
from urllib.parse import urlparse, urlsplit # noqa
from collections import namedtuple, deque # noqa
...
from core.prog import (safewrapper, print_styled) # noqa
...
class UrlNavigator: pass # fwd decl # noqa
...
def __pathToLocation__(o:str, state:typing.Optional[QtCore.QByteArray]=None) -> LocationData:
    path = pathlib.Path(o)
...
def __pathToLocation__(o:pathlib.Path, state:typing.Optional[QtCore.QByteArray]=None) -> LocationData: # noqa
    return LocationData(QtCore.QUrl(o.as_uri()), state)

@pathToLocation.register(QtCore.QUrl)
def __pathToLocation__(o:QtCore.QUrl, state:typing.Optional[QtCore.QByteArray]=None) -> LocationData: # noqa
...
class UrlComboBox(QtWidgets.QComboBox):
    ...
    def removeUrl(self, url:QtCore.QUrl, checkDefaultURLs:bool):
        ...
        with qtutils.SignalBlocker(self):
            self.setDefaults()
...
class UrlNavigatorMenu(QtWidgets.QMenu):
    ...
    sig_mouseLeave = Signal(name="sig_mouseLeave")
    sig_mouseEnter = Signal(name="sig_mouseEnter")
    sig_closed = Signal(name="sig_closed")
...
class PlacesButton(UrlNavigatorButtonBase):
    ...
    def slot_openPlacesMenu(self):
        ...
        # print(f"{self.__class__.__name__}.slot_openPlacesMenu: parent -> {self.parent()}")
        if (
...
class _UrlNavigator_(QtCore.QObject):
    ...
    def __init__(self, url:QtCore.QUrl, qq: UrlNavigator):
        ...
        self._closestPlace_:typing.Optional[dutils.DEPlace] = None
...
#         elif ((button & QtCore.Qt.MiddleButton) or ((button & QtCore.Qt.LeftButton) and (modifiers & QtCore.Qt.ControlModifier)) or ((button & QtCore.Qt.LeftButton) and (modifiers & QtCore.Qt.ShiftModifier))):
    ...
    def slotPathBoxChanged(self, text:str): # WARNING 2025-01-19 09:30:32 possible BUG
        ...
        if len(text) == 0:
            ...
            with qtutils.SignalBlocker(self._pathBox_):
...
class UrlNavigator(QtWidgets.QWidget):
    ...
    def __init__(self, url:typing.Optional[QtCore.QUrl]=None,
        ...
        super().__init__(parent=parent)
        self._newLook_ = False
        self._backgroundEnabled_ = False
    ...
    def backgroundEnabled(self) -> bool:
        return self._backgroundEnabled_

    @backgroundEnabled.setter
    def backgroundEnabled(self, enabled:bool):
        self._backgroundEnabled_ = enabled
    ...
    def newLook(self, val:bool):
        ...
        for btn in list(filter(lambda x: issubclass(type(x), UrlNavigatorButtonBase), self.children())):
            btn.newLook = self._newLook_

        self.backgroundEnabled = self._newLook_
    ...
    def paintEvent(self, evt:QtGui.QPaintEvent):
        ...
        option.state = QtWidgets.QStyle.State_None
        if self.hasFocus():
            option.palette.setColor(QtGui.QPalette.Window, self.palette().color(QtGui.QPalette.Highlight))

        if self.backgroundEnabled:
            if self._nav_p_._editable_:
                option.palette.setColor(QtGui.QPalette.Base, self.palette().alternateBase().color())
            self.style().drawPrimitive(QtWidgets.QStyle.PE_FrameLineEdit, option, painter, self)
