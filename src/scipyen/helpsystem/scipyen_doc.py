...
# # # Author of the python's pydoc module
# # # """
# # # 
...
def _is_bound_method(fn):
    ...
    """
    # NOTE: Copied from stock pydoc module
...
class ReSTDoc(pydoc.TextDoc):
    """Subclass of TextDoc to render reStructuredText"""
    # NOTE: still using the TextRepr
    ...
    def __init__(self):#, imagestempdir:typing.Optional[str|pathlib.Path]=None):
        ...
        # elif isinstance(imagestempdir, pathlib.Path) and imagestempdir.is_dir():
        #     self.imagesdir = imagestempdir.as_posix()
        # else:
    ...
    def render_latex(self, txt:str) -> str:
        ...
        for k, match in enumerate(matches):
            ...
            filepath = pathlib.Path(self.imagedir.name) / f"png{k}.png"
    ...
    def docclass(self, obj, name=None, mod=None, *ignored): # TODO 2025-12-30 13:44:38
        """Produce text documentation for a given class object."""
        realname = object.__name__
        name = name or realname
        bases = object.__bases__
        ...
        if name == realname:
            title = f"class **{realname}**"
        else:
            title = f"**{name}** = class {realname}"
        ...
        title = self.render_title(f"class **{realname}**" if name == realname else f"**{name}** = class {realname}")
        ...
        mro = deque(inspect.getmro(object))
        if len(mro) > 2:
            mrosection  = self.section("Method resolution order:", ", ".join([f"{makename(base)}" for base in mro]), 0)
            push(mrosection)
            # push("Method resolution order:")
            # for base in mro:
            #     push('    ' + makename(base))
        ...
        if subclasses:
            subcls = list()
            for subclassname in subclasses[:MAX_SUBCLASSES_TO_DISPLAY]:
                subcls.append(subclassname)
            if no_of_subclasses > MAX_SUBCLASSES_TO_DISPLAY:
                subcls.append(' ... and ' +
                     str(no_of_subclasses - MAX_SUBCLASSES_TO_DISPLAY) +
                     ' other subclasses')
                
            subclssection = self.section("Built-in subclasses:", ", ".join(subcls), 0)
            push(subclssection)
    ...
    def docroutine(self, object, name=None, mod=None, cl=None, homecls=None):
        ...
        decl = self.render_code(asyncqualifier + title + argspec + note)

        if skipdocs:
            return decl + '\n'
        else:
            doc = pydoc.getdoc(object) or ''
            synopsis, doc = pydoc.splitdoc(doc)
            doc = self.render_latex(doc)
            
            if len(synopsis.strip()):
                synopsis = self.render_synopsis(synopsis)
                return decl + '\n' + synopsis + "\n" + (doc and doc.rstrip() + '\n')
