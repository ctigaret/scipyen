...
from core.basescipyen import BaseResult
...
class RheobaseResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class RheobaseLatencyResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class PassivePropertiesResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class APTrainAnalysisResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class APAnalysisSweepResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

class APAnalysisSweepSeriesResult(BaseResult):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
def rheobase_latency(*args, **kwargs):
    ...
    if all(isinstance(a, BaseResult) and hasattr(a, "First_AP_latency") and isinstance(a.First_AP_latency, IrregularlySampledDataSignal) for a in args):
...
#
#     β  = float(np.squeeze(vsag10_90[-1])) - α
#
#     x0  = float(vsag10_90.times[0]) # WRONG!
...
def extract_AP_train( vm: neo.AnalogSignal, im: neo.AnalogSignal | tuple,
    ...
    method: signalprocessing.BoxcarDetectionMethod, one of
        "BoxcarDetectionMethod.state_levels" (default) or
        "BoxcarDetectionMethod.kmeans".
...
def detect_AP_waveforms_in_train(sig, iinj,
    ...
    # ap_results = dict()
    ap_results = APTrainAnalysisResult()
...
def analyse_AP_step_injection_series(data: typing.Union[ # noqa
    ...
    * for current injection time interval:
    ...
    ret = APAnalysisSweepSeriesResult()
    ...
    try:
        ...
        for kseg, seg_res in enumerate(ret.Current_injection_steps):
            if isinstance(seg_res.AP_analysis.AP_train, neo.SpikeTrain) and len(seg_res.AP_analysis.AP_train):
                val = getattr(seg_res.AP_analysis, "AP_onset_Vm", None)
            ...
            ap_freq = getattr(seg_res.AP_analysis, "Mean_AP_Frequency", None)
        ...
        if not test_for_rheobase_latency(ret, minsteps):
            ...
            try:
                ...
                ret.rheobase_latency_analysis = RheobaseLatencyResult({"Latencies": latencies, "Result": rheo_result})
...
def frequency_isi(results: BaseResult, isi_start: int = 0, isi_span: int = 1):
    ...
    dom, vals = zip(*[(d.AP_analysis.Injected_current, calculate_isi(d.AP_analysis.AP_train.times)) for d in results.Current_injection_steps])

    ret = IrregularlySampledDataSignal(dom, vals, name = "ISI0_Frequency")
    return ret


def frequency_isi0(results: BaseResult):
    r"""Particular case of frequency_isi for first two APs.
    """
    return frequency_isi(results, isi_start=0, isi_span=1)
...
def test_for_rheobase_latency(data: APAnalysisSweepSeriesResult, minsteps=3):
    ...
    try:
        ok = (isinstance(data, APAnalysisSweepSeriesResult)
              and len([s for s in data.Current_injection_steps if (s.AP_analysis.Number_of_APs > 0 and s.AP_analysis.AP_train is not None) ] ) >= minsteps)
...
def analyse_AP_step_injection_sweep(segment, VmSignal:typing.Union[int, str] = "Vm_prim_1",
    ...
    References:
    -----------
    For AP detection algorithm, please see: Naundorf et al (2006).
    ...
    if passive_analysis:
        ...
        passive = PassivePropertiesResult(passive)
    ...
    for key, value in ap_results.__dict__.items():
    ...
    result.Afterspike_potentials = types.SimpleNamespace()
...
#
#     left_sweep: float or scalar Quantity: domain interval between the start of
#         the event waveform and the detected start of the event
#
#     Var-keyword parameters:
#     =======================
...
#
#         if "channel_names" in x.array_annotations and channel in range(x.array_annotations.length):
#             ch_name = str(x.array_annotations["channel_names"][channel])
#         else:
#             ch_name = f"{x.name}_channel_{channel}"
#
#         if "channel_ids" in x.array_annotations and channel in range(x.array_annotations.length):
#             ch_id = int(x.array_annotations["channel_ids"][channel])
#         else:
#             ch_id = channel
#
...
#
#         # print(f"ephys.membrane.detect_Events_CBsliding: h shape = {h.shape}, x shape = {x.shape}")
#
#         N = h.shape[0]
#
#         M = x.shape[0]
...
#
#         θN.array_annotate(channel_names=[f"{ch_name}_θN"])
#         θN.array_annotate(channel_ids=[ch_id])
#
...
#                                           θN, threshold, peakfunc, left_sweep,
#                                           **kwargs)
#         else:
...
#
#     return result   # None, or dict channel:int ↦ detection:dict
#                     #           "stamps"                ↦ pq.Quantity array
...
def calculate_template_scale_offset(x, h):
    ...
    if not all(datatypes.is_vector(v) for v in (x,h)):
...
def slide_detect(x:np.ndarray, h:np.ndarray, padding:bool=True, data_cache = None, **kwargs) -> typing.NamedTuple:
    ...
    """
    # print(f"ephys.membrane.slide_detect: h shape = {h.shape}, x shape = {x.shape}")

    # if any(v.ndim > 1 and vfor v in (x,h)):
    if not all(datatypes.is_vector(v) for v in (x,h)):
        raise TypeError("Expecting two 1D signals")
    ...
    for k in time_range:
        y = xx[k:k+N]
        # print(f"ephys.membrane.slide_detect; y.shape = {y.shape}")
...
def extract_event_waveforms(x:typing.Union[neo.AnalogSignal, DataSignal],
    ...
    • waves: sequence of "event" waveforms (as AnalogSignal or DataSignal obejcts)
        each array annotated according to the example below (where 'time' refers
    ...
    if isinstance(left_sweep, float):
        left_sweep = left_sweep * x.times.units

    elif isinstance(left_sweep, (pq.Quantity, np.ndarray)):
        assert left_sweep.size == 1, "'left_sweep' must be a scalar"
        if isinstance(left_sweep, np.ndarray):
            left_sweep = left_sweep[0] * x.times.units
        else:
            if left_sweep.units != x.times.units:
                if unitsConvertible(left_sweep, x.times.units):
                    left_sweep = left_sweep.rescale(x.times.units)
                else:
                    raise TypeError(f"'left_sweep' units ({left_sweep.units}) are incompatible with the signal domain ({x.times.units})")

    else:
        if left_sweep is not None:
            raise TypeError(f"'left_sweep' has wrong type ({type(left_sweep).__name__})")

        left_sweep = 0 * x.times.units
    ...
    if left_sweep != 0:
        if events_starts[0] - left_sweep < x.t_start:
            scipwarn(f"'left_sweep' too large to the left; will be readjusted")
            left_sweep = events_starts[0] - x.t_start

        elif len(event_starts)> 1:
            if event_starts[0] - left_sweep >= event_starts[1]:
                scipwarn(f"'left_sweep' too large to the right; will be set to 0")
                left_sweep = 0 * x.times_units
        elif event_starts[0] - left_sweep >= x.t_stop:
            scipwarn(f"'left_sweep' too large to the right; will be set to 0")
            left_sweep = 0 * x.times.units
...
def detect_Events(x:typing.Union[neo.AnalogSignal, DataSignal],
    ...
    detection: A mapping of key:int  ↦ value:mapping (see below)
        ...
        "threshold" ↦ float | None;, default None.
            ...
            root-mean-square of the detection criterion signal generated by the
            sliding template detection method.
...
#         return detect_Events_CBsliding(x, detection, raw_signal, **kwargs)
    ...
    for channel, channel_detection_mapping in detection.items():
        ...
        if useCBsliding:
            ...
            # θ and θN (normalized veriosn of θ) are an "analog" signals wrapping,
            # respectively, the dxc and the dxc_n vectors;
        ...
        if ret:
            stamps, waves, fitted_waves = ret
            channel_detection["stamps"] = stamps
            channel_detection["waves"]  = waves
            channel_detection["fitted_waves"]  = fitted_waves
            channel_detection["θ"] = θ
            channel_detection["θN"] = θN
            channel_detection["detection_datetime"] = datetime.datetime.now()
            channel_detection["detection_options"] = channel_detection_mapping
...
def make_event_detection_trainlist(event_detection_result:dict,
    ...
    for channel, channel_result in event_detection_result.items():
        ...
        wave_arr_ann = channel_result["waves"][0].array_annotations
        for w in channel_result["waves"][1:]:
        ...
        per_channel_spike_trains.append(spike_train)
...
def fit_event_wave(w:typing.Union[neo.AnalogSignal, DataSignal], kw:int,
    ...
    try:
        ...
        # wrap the fitted curve in an AnalogSignal or DataSignal
        # (i.e. the same object class as w)
        fw = type(w)(fw, units = w.units, t_start = w.t_start,
                        sampling_rate = w.sampling_rate, description=f"event_{kw}")

        fit_amplitude = sigp.waveform_amplitude(fw)

        # create a TWO-CHANNEL signal:
        # channel 0 is the mEPSC waveform,
        # channel 1 is the fitted curve
        fw = neoutils.concatenate_signals(w, fw)
        fw.name = w.name
        fw.description = f"{w.name} with fitted model {fresult.ModelFunction}"
        fw.annotations.update(w.annotations)
        fw.annotations["wave_index"] = kw
        fw.annotations["event_fit"] = datatypes.namespace2dict(fresult)
        if useThresholdOnRsquared:
            accept = bool(fw.annotations["event_fit"]["Coefficients"]["GoF"]["Rsq"] >= rsqThreshold)
        else:
            accept = True
        fw.annotations["event_fit"]["Accept"] = accept
        fw.segment = w.segment
        fw.array_annotate(amplitude = np.array([w_amplitude, fit_amplitude]), channel_names=np.array(["wave", "fit"]))
        return fw
    except Exception as e:
        traceback.print_exc()
...
def parse_event_detection_spiketrain(train:neo.SpikeTrain) -> tuple:
    if not isinstance(train, neo.SpikeTrain) or train.annotations.get("source", None) != "Event_detection":
        return
    channel_events = dict()
