...
#                 self.workspaceModel.currentItemName = self.workspaceModel.item(row,0).text()
    ...
    def _configureUI_(self):
        ...
        self._recentDirectoriesMenu_ = navigator.UrlNavigatorMenu("Recent Working Directories", self)
        self._recentDirectoriesMenu_.mouseButtonClicked.connect(self.slot_recentDirActivated)
        # self.recentDirectoriesMenu.triggered.connect(self.slot_recentDirectorySelected)
        self._recentDirectoriesMenu_.setIcon(QtGui.QIcon.fromTheme("folder-open-recent"))
        
        # self._recentDirectoriesMenu_.setAttribute(QtCore.Qt.WidgetAttribute.WA_NoSystemBackground, True)
        
        self.menuFile.insertMenu(self.actionReload_Plugins, self._recentDirectoriesMenu_)
        ...
        self.tbChDir.setMenu(self._recentDirectoriesMenu_)
        ...
        self.selDirBtn.setMenu(self._recentDirectoriesMenu_)
...
#     @property
#     def recentDirectoriesMenu(self) -> QtWidgets.QMenu:
#         r"""Do not use yet: trying to do away with the flickering of the menu upon closing"""
#         if len(self.recentDirectories) == 0:
#             return QtWidgets.QMenu()
#         
#         if isinstance(self._recentDirectoriesMenu_, QtWidgets.QMenu) and qtutils.isQObjectAlive(self._recentDirectoriesMenu_):
#             self._recentDirectoriesMenu_.close()
#             self._recentDirectoriesMenu_.deleteLater()
#             self._recentDirectoriesMenu_ = None
#             
#         self._recentDirectoriesMenu_ = navigator.UrlNavigatorMenu(parent=self)
#         # menu = navigator.UrlNavigatorMenu(parent=w)
#         
...
#         
#         self.initRecentDirectoriesMenu(self._recentDirectoriesMenu_, 0, maxItems)
#         # self._recentDirectoriesMenu_.popup(pos)
#         self.update()
#         return self._recentDirectoriesMenu_
    
#     def initRecentDirectoriesMenu(self, menu:QtWidgets.QMenu, startIndex:int, maxItems:int):
...
#         dirNames = self._recentDirectories[startIndex:lastIndex]
#         actions = list()
#         if self._maxRecentDirectories > 0:
#             actions.append(QAction(QtGui.QIcon.fromTheme("edit-clear-history"),
#                                         "Clear Recent Directories List"))
#             actions.append(QAction())
#             actions[1].setSeparator(True)
#         
#         dirActions = list(map(lambda x: QAction(guiutils.csqueeze(x.replace('&', '&&'), 60), self), dirNames))
#         if self._currentDir_ in dirNames:
#             currentIndex = dirNames.index(self._currentDir_)
#             font = QtGui.QFont(subDirsActions[currentIndex].font())
#             font.setBold(True)
#             dirActions[currentIndex].setFont(font)
#             
#         for k, i in enumerate(range(startIndex, lastIndex)):
#             dirActions[k].setData(i)
#             dirActions[k].setText(dirNames[k])
#             
#         actions.extend(dirActions)
#         
...
#         if nDirs > maxIndex:
#             menu.addSeparator()
#             dirsMenu = navigator.UrlNavigatorMenu("More", menu)
#             self.initRecentDirectoriesMenu(dirsMenu, maxIndex, maxItems)
#             menu.addMenu(dirsMenu)
            
    def _refreshRecentDirectoriesMenu_(self):
        self._recentDirectoriesMenu_.clear()

        if len(self.recentDirectories) > 0:
            if self._maxRecentDirectories > 10:
                clearDirAction = self._recentDirectoriesMenu_.addAction(QtGui.QIcon.fromTheme("edit-clear-history"),
                    "Clear Recent Directories List")
                clearDirAction.triggered.connect(self._clearRecentDirectories_)
                self._recentDirectoriesMenu_.addSeparator()
                
            self._initRecentDirsMenu_(self._recentDirectoriesMenu_, 0)
    ...
    def _initRecentDirsMenu_(self, menu: QtWidgets.QMenu, startIndex:int):
        ...
        for k,i in enumerate(range(startIndex, lastIndex)):
            dirName = dirsNames[k]
            dirIndex = self._recentDirectories.index(dirName)
            dirsActions[k].setData(dirIndex)
            # dirsActions[k].setData(i)
            # dirsActions[k].setData(dirsNames[k])
    ...
    @Slot(QtCore.QUrl, bool)
    @safewrapper
    def slot_chDirUrl(self, val:QtCore.QUrl, refreshMenu:bool=True):
        # print(f"{self.__class__.__name__}.slot_chDirUrl({val})")
        path = desktoputils.urlToPath(val)
        s = path.as_posix()
        self.slot_chDirString(s, refreshMenu)

    @Slot(str)
    @safewrapper
    def slot_chDirString(self, val, refreshMenu:bool=True):
        ...
        self.slot_changeDirectory(target, refreshMenu)
    ...
    def slot_recentDirActivated(self, action:QAction, button:QtCore.Qt.MouseButton):
        ...
        if path.exists():
            url = QtCore.QUrl(path.as_uri())
            self.navigator.setLocationUrl(url)
            self.navigator.urlChanged.emit(url, False)
        else:
            p = pathlib.Path(path)
            while not p.exists():
                if p == p.parent:
            ...
            if p.exists():
                ...
                self.errorMessage("Navigation", f"Chosen recent directory is inacessible:\n{txt}")
                
    @Slot(QAction)
    def slot_recentDirectorySelected(self, action:QAction):
        from gui import guiutils
        result = action.data() 
        path = pathlib.Path(self._recentDirectories[result]).absolute() #.resolve()   # the path to the subdirectory pointed to by the action
        if path.exists():
    ...
    @Slot()
    @safewrapper
    def slot_changeDirectory(self, targetDir:str=None, refreshMenu:bool=False):
