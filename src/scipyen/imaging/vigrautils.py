...
def kernel2array(value:typing.Union[vigra.filters.Kernel1D, vigra.filters.Kernel2D], compact:bool=False):
    ...
    for a 2D kernel (N x M samples):
        when compact is False:
            tuple (X, Y, KRN) with:
                X, Y: 2D numpy arrays with size (N,M) - together, these are the
                    *meshgrid* with the coordinates of kernel's samples,
                    from kernel.lowerRight() to kernel.upperLeft()

                KRN: 2D numy array with shape (N,M) - kernel sample values

                See also np.meshgrid, np.mgrid

        when compact is True:
            3D numpy array with shape (N,M,3) where
...
def _(value: vigra.filters.Kernel1D, compact: bool = False,
      ogrid: bool = False):
    ...
    if ogrid:
        try:
            x = np.ogrid[value.left():value.right()+1]
            return x, y
...
def _(value: vigra.filters.Kernel2D, compact: bool = False,
      ogrid: bool = False):
    ...
    if ogrid:
        try:
            yy, xx = np.ogrid[value.upperLeft()[0]:value.lowerRight()[1]+1, value.upperLeft()[1]:value.lowerRight()[0]+1]

            for kx, x_ in enumerate(xx.flatten()): # from left to right
                for ky, y_ in enumerate(yy.flatten()): # from top to bottom
                    y[kx, ky] = value[x_, y_]
        ...
        xx = np.linspace(value.upperLeft()[0], value.lowerRight()[0], value.width(),
                        dtype=np.dtype(int))

        yy = np.linspace(value.upperLeft()[1], value.lowerRight()[1], value.height(),
                        dtype=np.dtype(int))
