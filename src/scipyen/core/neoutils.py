...
def segment_stop(data:neo.Segment):
    """Returns the maximum of t_stop for all signals and spiketrains in a segment.
    ...
    return max([s.t_stop for s in data.analogsignals] + 
...
events:list                                     neo.Event,
    ...
    if len(args) == 1:
        ...
        if isinstance(args[0], str):
            try:
                blocks = get_workspace_neo_blocks(args[0], sortby=sortby,ascending=ascending) # sorting at NOTE: 2023-06-30 12:17:15
            ...
            if all(isinstance(a, str) for a in args[0]):
                try:
                    blocks = get_workspace_neo_blocks(args[0], sortby=sortby,ascending=ascending) # sorting below see NOTE: 2023-06-30 12:17:15
            ...
            raise ValueError("Expecting a sequence of blocks or symbols (str) bound to blocks n the workspace, a block or a single str (symbol) bound to a block in the workspace")
    ...
    if isinstance(blocks, collections.abc.Sequence) and all(isinstance(a, neo.Block) for a in blocks):
