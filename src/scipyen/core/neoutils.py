...
r"""Various utilities for handling objects and data structures in the neo package.
...
def sweep_duration(data: neo.Segment):
    return max(
        s.duration
        for s in data.analogsignals
        + data.irregularlysampledsignals
        + list(st for st in data.spiketrains)
    )


def segment_start(data: neo.Segment):
    r"""Returns the minimum of t_start for all signals and spiketrains in a segment.

    Avoids any events and epochs that fall outside the signals' time domain.

    NOTE: When the segment belongs to a block generated by reading data recorded
    on disk, the t_start depends on how the software used to record the data
    was configured. Therefore it may represent a relative time from the start
    of the block, or the start of the experiment.

    See also segment_stop
    """

    return min(
        [s.t_start for s in data.analogsignals]
        + [s.t_start for s in data.spiketrains]
        + [min(s.times) for s in data.irregularlysampledsignals]
    )


def segment_stop(data: neo.Segment):
    r"""Returns the maximum of t_stop for all signals and spiketrains in a segment.

    Avoids any events and epochs that fall outside the signals' time domain.

    NOTE: When the segment belongs to a block generated by reading data recorded
    on disk, the t_stop depends on how the software used to record the data
    was configured. Therefore it may represent a relative time from the start
    of the block, or the start of the experiment.
    ...
    return max(
        [s.t_stop for s in data.analogsignals]
        + [s.t_stop for s in data.spiketrains]
        + [max(s.times) for s in data.irregularlysampledsignals]
    )
...
def _(data, t=0):
    ...
    isigs = [
        set_relative_time_start(make_neo_object(s), t)
        for s in data.irregularlysampledsignals
    ]
        ...
        try:
            ...
            if isinstance(value, pq.Quantity):
                try:
                    ...
                    raise ValueError(
                        "Could not merge array annotations due to different units"
                    )
                merged_array_annotations[key] = (
                    np.append(value, other_value) * value.units
                )
            else:
...
def make_neo_object(obj, /, **kwargs):
    r"""Generic (copy) constructor for neo objects.
...
def _(obj, /, **kwargs):
    ...
    sig_named_params = dict(
        (param_name, getattr(obj, param_name, param_value[0]))
        for param_name, param_value in signature.named.items()
        if hasattr(obj, param_name)
    )  # and isinstance(getattr(obj, param_name), param_value[1]))

    factory_pos_params = tuple(
        getattr(obj, param_name)
        for param_name, param_type in signature.positional.items()
        if hasattr(obj, param_name) and isinstance(getattr(obj, param_name), param_type)
    )
    ...
    factory_kwarg_params = (
        getattr(obj, sig_kwargs_param)
        if hasattr(obj, sig_kwargs_param)
        and isinstance(getattr(obj, sig_kwargs_param, None), dict)
        else dict()
    )
...
def _(obj, /, **kwargs):
    ...
    sig_named_params = dict(
        (param_name, kwargs.pop(param_name, getattr(obj, param_name, param_value[0])))
        for param_name, param_value in signature.named.items()
        if hasattr(obj, param_name)
    )

    factory_pos_params = tuple(
        kwargs.pop(param_name, getattr(obj, param_name, None))
        for param_name, param_type in signature.positional.items()
        if hasattr(obj, param_name) and isinstance(getattr(obj, param_name), param_type)
    )
    ...
    if isinstance(obj, neo.core.basesignal.BaseSignal):
        ...
        # obj._quantity_attr names the parameter for the actual numeric data
        # going into the signal
        attr_named_params = dict(
            (p[0], kwargs.pop(p[0], getattr(obj, p[0], None)))
            for p in attr_params
            if p[0] != obj._quantity_attr
        )
...
def combine_time_ranges(time_ranges):
    r"""This is neo.AnalogSignal._concatenate_time_ranges()
...
def invert_time_ranges(time_ranges):
    r"""This is neo.AnalogSignal._invert_time_ranges()"""
    i = 0
    new_ranges = list()
...
def fuse_irregular_signals(
    *args,
    func: typing.Optional[typing.Callable] = np.nanmean,
    name: typing.Optional[str] = None,
) -> typing.Union[IrregularlySampledDataSignal, neo.IrregularlySampledSignal]:
    r"""Fusion of irregularly sampled (data) signals.
    ...
    assert all(
        isinstance(v, (neo.IrregularlySampledSignal, IrregularlySampledDataSignal))
        for v in args
    ), (
        "Expecting only neo.IrregularlySampledSignal or IrregularlySampledDataSignal objects"
    )
    ...
    assert all(
        lambda x: unitsConvertible(x[0].times.units, x[1].times.units)
        for x in pairwise(args)
    ), "Signals must have domains with compatible units"

    assert all(
        lambda x: unitsConvertible(x[0].units, x[1].units) for x in pairwise(args)
    ), "Signals must have compatible units"

    assert all(lambda x: x[0].ndim == x[1].ndim for x in pairwise(args)), (
        "Signals must have the same dimensions (i.e., number of axes)"
    )

    assert all(x.shape[1] == 1 for x in args), (
        "Only 1D (i.e. single-channel) signals are supported"
    )
    ...
    return args[0].__class__(
        dd, vv, units=signal_units, time_units=domain_units, name=name
    )
...
def assign_to_signal(
    ...
    r"""Assigns values in src to values in dest, for the specified channel, or all channels
    ...
    if isinstance(src, neo.AnalogSignal):
        if src.t_start < dest.t_start:
            raise ValueError(
                "Source signal starts (%s) before destination signal (%s)"
                % (src.t_start, dest.t_start)
            )

        if src.t_stop > dest.t_stop:
            raise ValueError(
                "Source signal stops (%s) after destination signal (%s)"
                % (src.t_stop, dest.t_stop)
            )

        if src.units != dest.units:
            raise TypeError(
                "Mismatch between destination unts (%s) and source units (%s)"
        ...
        raise TypeError(
            "source expected to be an AnalogSignal or a scalar python quantity of same units as destination"
        )
...
def assign_to_signal_in_epoch(
    ...
    r"""Assigns values in src to values in dest, within an epoch, for the specified channel, or all channels"""

    if not isinstance(dest, neo.AnalogSignal):
        raise TypeError(
            "dest expectyed to be an AnalogSignal; got %s instead"
            % (type(dest).__name__)
        )

    if not isinstance(epoch, neo.Epoch):
        raise TypeError(
            "epoch expected to be a neo.Epoch; got %s instead" % (type(epoch).__name__)
        )
    ...
    if isinstance(src, neo.AnalogSignal):
        ...
        if any(
            [
                (epoch.times[k] + epoch.durations[k]) > dest.t_stop
        ...
        if any(
            [
                (epoch.times[k] + epoch.durations[k]) > src.t_stop
                for k in range(len(epoch))
            ]
        ):
        ...
        for k in range(len(epoch)):
            src_ndx = np.where(
                (dest.times >= epoch.times[k])
                & (dest.times < (epoch.times[k] + epoch.durations[k]))
            )

            dest_ndx = np.where(
                (src.times >= epoch.times[k])
                & (src.times < (epoch.times[k] + epoch.durations[k]))
            )
...
def get_signal_names_indices(
    ...
    r"""Returns a list of analog signal names in data.
    ...
    for k in range(len(signals)):
        if (
            sig_indices_names[k][1] is None
            or not isinstance(sig_indices_names[k][1], str)
            or len(sig_indices_names[k][1].strip()) == 0
        ):
...
def __get_container_collection_attribute__(
    container, attrname, container_index=None, multiple=True
):
...
def __container_lookup__(
    container: neo.container.Container,
    ...
    **kwargs,
    ...
    if isinstance(container, neo.container.Container):
        ...
        ret = dict(
            (cname, t)
            for cname, t in zip(member_collection_names, map(pfun0, member_collections))
            if len(t) > 0
        )
        ...
        if len(ret) == 0:
            ...
            child_container_names = [
                neo_child_container_name(c) for c in direct_containers
            ]

            child_container_collections = [
                getattr(container, cname, None) for cname in child_container_names
            ]

            child_container_collections2 = [
                __get_container_collection_attribute__(
                    container, cname, kwargs.get(cname, None)
                )
                for cname in child_container_names
            ]

            pfun = partial(
                __collection_lookup__,
                index_obj=index_obj,
            ...
            ret = dict(
                (cname, d)
                for cname, d in zip(
                    child_container_names, map(pfun, child_container_collections)
                )
                if len(d) > 0
            )
    ...
    raise TypeError(
        f"Expecting a neo.Container; got {type(container).__name__} instead."
    )


def __collection_lookup__(
    seq: typing.Sequence,
    index_obj: typing.Union[str, int, tuple, list, np.ndarray, range, slice],
    contained_type: neo.baseneo.BaseNeo,
    ...
    **kwargs,
    ...
    if seq_index is None:
        return dict(
            (k, d)
            for k, d in enumerate(map(pfun, seq))
            if len(d) > 0 and any([len(val) > 0 for key, val in d.items()])
        )

    elif isinstance(seq_index, (tuple, list, range)) and all(
        ...
        return dict(
            (k, d)
            for k, d in zip(map(pfun, seq), k_)
            if len(d) > 0 and any([len(val) > 0 for key, val in d.items()])
        )
...
def is_empty(
    x: typing.Union[
        ...
        neo.core.dataobject.DataObject,
        typing.Sequence[
            typing.Union[neo.core.container.Container, neo.core.dataobject.DataObject]
        ],
    ],
    ignore: typing.Optional[typing.Union[typing.Sequence[type], type]] = None,
):
    r"""Checks whether x contains any data.
    Parameters:
    ==========
    ...
    if ignore is not None:
        if not isinstance(ignore, (tuple, list)) or not all(
            (isinstance(t, type) for t in ignore)
        ):
            raise TypeError(
                "'ignore' expected to be a type, a sequence of types;, or None got %s"
                % ignore
            )
    ...
    if isinstance(x, neo.core.container.Container):
        if ignore is None:
            ...
            container_children = tuple(
                c
                for c in x.container_children_recur
                if not isinstance(c, tuple(ignore))
            )
            data_children = tuple(
                c for c in x.data_children_recur if not isinstance(c, tuple(ignore))
            )
    ...
    elif isinstance(x, (tuple, list)) and all(
        (
            isinstance(
                xx, (neo.core.container.Container, neo.core.dataobject.DataObject)
            )
            for xx in x
        )
    ):
        ...
        raise TypeError(
            "Expecting a neo Container, DataObject or a sequence of these; got %s instead"
            % type(x).__name__
        )
...
def neo_lookup(
    *args: typing.Union[
        neo.core.container.Container, typing.Sequence[neo.core.container.Container]
    ],
    ...
    **kwargs,
    ...
    r"""Enhanced filtering of child data objects inside neo containers.
    ...
    if isinstance(data_obj_type, (tuple, list, deque)) and all(
        (isinstance(d, type) and neo.core.dataobject.DataObject in inspect.getmro(d))
        for d in data_obj_type
    ):
        signal_collection_names = tuple(
            map(_container_name, (d.__name__ for d in data_obj_type))
        )
    else:
        raise ValueError(
            f"'data_obj_type' expected to be a descendant of neo.core.dataobject.DataObject, or a sequence (deque, list, tuple) of such types; got {data_obj_type.__name__} instead"
        )
    ...
    if isinstance(src, neo.core.container.Container):
        ...
        raise TypeError(
            "'src' expected to be a descendant of neo.core.container.Container, or a sequence (deque, list, tuple) of such, or of data objects"
        )

    container_names = tuple(
        map(lambda x: _container_name(type(x).__name__), containers)
    )
        ...
        for l, signal_collection_name in enumerate(signal_collection_names):
            ...
            if collection is not None:
                ...
                for child_container_name in container._child_containers:
                    ccdict = neo_lookup(
                        getattr(container, child_container_name),
                        data_obj_type=data_obj_type,
                        op=op,
                        indices=indices,
                        indices_only=indices_only,
...
def neo_use_lookup_index(
    *args: typing.Union[neo.container.Container, typing.Sequence], ndx: dict
):
    r"""Access data objects using an indexing dictionary returned by neo_lookup.
    ...
    if not isinstance(ndx, dict):
        raise TypeError(
            f"'ndx' expected to be a dict; got {type(ndx).__name__} instead"
        )
    ...
    if isinstance(src, neo.core.container.Container):
        ...
        isinstance(src, (tuple, list, deque))
        and all(isinstance(s, neo.core.baseneo.BaseNeo) for s in src)
    ):
        raise TypeError(
            "'src' expected ot be a neo container or a sequence of neo objects"
        )
    ...
    for kdata, data in enumerate(src):
        ...
        if data_container_name in ndx.keys():
            ...
            if isinstance(data_subindex, dict) and all(
                isinstance(k, int) for k in data_subindex
            ):
                dsub = data_subindex[kdata]

    if isinstance(
        src, (tuple, list)
    ):  # NOTE: 2020-03-23 23:41:16 sequence of containers or data objects
        if isinstance(ndx, (tuple, list, deque, range)) and all(
            [isinstance(k, int) for k in ndx]
        ):  # sequence of ints
            ...
            for key, index in ndx.items():
                if isinstance(key, int):
                    ...
                    ret += [
                        v for v in ret_ if all([not is_same_as(v, v_) for v_ in ret])
                    ]

                else:
                    raise IndexError(
                        "Unexpected index:%s at key:%s = %s, for src:%s"
                        % (
                            type(index).__name__,
                            type(key).__name__,
                            key,
            ...
            raise TypeError(
                "Unexpected indexing type %s for %s object"
                % (type(ndx).__name__, type(src).__name__)
            )
        ...
        if isinstance(ndx, dict):
            ...
            if (
                src_container_name in ndx.keys()
            ):  # and all (isinstance(k, int) for k in ndx[src_container_name].keys()):
            ...
            for (
                key,
                index,
            ) in ndx.items():  # iterate through the ndx dict's key/value pairs
                if isinstance(key, str):  # works on Container's attribute in 'key'
                    collection = getattr(
                        src, key, None
                    )  # sequence of containers or data objects

                    if collection is None:
                        raise AttributeError(
                            "%s is an invalid attribute of %s"
                    ...
                    if isinstance(index, (dict, tuple, list)):
                        ret += [
                            v
                            for v in neo_use_lookup_index(collection, index)
                            if v not in ret
                        ]  # enters at NOTE: 2020-03-23 23:41:16

                    else:
                        raise KeyError(
                            "Unexpected indexing structure type %s for %s object"
                            % (type(index).__name__, type(collection).__name__)
                        )
...
def normalized_index(
    src: neo.core.container.Container, # neo.Block, neo.Segment, neo.Group
    ...
    if isinstance(src, neo.Block):
        getsingle = lambda x: x[0] if isinstance(x, tuple) and len(x) == 1 else x
        return list(map(lambda seg: getsingle(normalized_index(seg, index, childtype, silent)), src.segments))
    ...
    if childtype in (neo.AnalogSignal, DataSignal):
        ...
        if not all(
            [
                (hasattr(i, "image_data") and hasattr(i, "name"))
                for i in signal_collection
            ]
        ):
...
def get_index_of_named_signal(
    ...
    if isinstance(src, (tuple, list)):
        if all(isinstance(s, neo.Segment) for s in src):
            ...
            and all(isinstance(s_, neo.core.dataobject.DataObject) for s_ in s)
            for s in src
        ):
    ...
    if is_block or is_segments_list:
        ...
        if isinstance(names, str):
            ...
            if silent:
                return [
                    utilities.silentindex(
                        [i.name for i in getattr(j, signal_collection)],
                        names,
                        multiple=False,
                    )
                    for j in data
            ...
            return [
                [i.name for i in getattr(j, signal_collection)].index(names)
                for j in data
            ]
            ...
            if np.all([isinstance(i, str) for i in names]):
                ...
                return [
                    [
                        [i.name for i in getattr(j, signal_collection)].index(k)
                        for k in names
                    ]
                    for j in data
                ]
    ...
    # elif isinstance(src, neo.core.Segment):
        ...
        if isinstance(names, str):
            if silent:
                return utilities.silentindex(
                    [i.name for i in objectList], names, multiple=False
                )
            ...
            if np.all([isinstance(i, str) for i in names]):
                if silent:
                    return [
                        utilities.silentindex(
                            [i.name for i in objectList], j, multiple=False
                        )
                        for j in names
                    ]
            ...
            if np.all([isinstance(i, str) for i in names]):
                if silent:
                    return [
                        [
                            utilities.silentindex(
                                [i.name for i in objectList], k, multiple=False
                            )
                            for k in names
                        ]
                        for objectList in src
                ...
                return [
                    [[i.name for i in objectList].index(k) for k in names]
                    for objectList in src
                ]
    else:
        raise TypeError(
            "First argument must be a neo.Block object, a list of neo.Segment objects, or a neo.Segment object; got %s instead"
            % type(src).__name__
        )


def epoch_has_interval(
    epoch: typing.Union[neo.Epoch, DataZone],
    interval_name: typing.Union[str, np.str_, bytes],
) -> bool:
    if not isinstance(epoch, (neo.Epoch, DataZone)):
        raise TypeError(
            f"'epoch' expected to be a neo.Epoch or DataZone; got {type(epoch).__name__} instead"
        )
    ...
    if isinstance(interval_name, bytes):
        ...
        raise TypeError(
            f"'interval_name' expected a str, np.str_ or bytes; got {type(interval_name).__name__} instead"
        )
...
def get_workspace_neo_blocks(
    *args,
    sortby: typing.Optional[typing.Union[str, typing.Callable]] = None,
    ascending: bool = False,
):
    r"""Helper to get lookup neo.Blocks in the workspace by using globs or regexps"""
    ...
    if len(args) == 1:
        try:
            if isinstance(sortby, str):
                return workspacefunctions.getvars(
                    args[0],
                    var_type=(neo.Block,),
                    sort=True,
    ...
    elif isinstance(args[0], collections.abc.Sequence) and all(
...
def get_domain_index(data, t):
    r"""Returns the sample index nearest to the domain scalar value `t`"""
    u = data.times.units
...
def get_sample_at_domain_value(data, x):
    r"""Returns the signal sample value at (or around) domain value x
...
def get_time_slice(
    ...
    r"""Returns a time slice from a neo.Block or neo.Segment object.
    ...
    if isinstance(t1, pq.Quantity):
        ...
        if not isinstance(_d_, pq.time.UnitTime):
            raise ValueError(
                "The t1 quantity must be in time unit; got %s instead"
                % type(_d_).__name__
            )
        ...
        raise TypeError(
            "When given, t1 must be a real scalar, time python quantity, or None; got %s instead"
            % type(t1).__name__
        )
    ...
    if isinstance(t0, neo.Epoch):
        if t0.size > 1:
            warnings.warn(
                f"The epoch {t0} has more than one interval; the first interval will be used"
            )
        elif t0.size == 0:
        ...
        if not isinstance(_d_, pq.time.UnitTime):
            raise ValueError(
                "The t0 quantity must be in time unit; got %s instead"
                % type(_d_).__name__
            )
        ...
        raise TypeError(
            "t0 must be a neo.Epoch, real scalar or python quantity in time units; got %s instead"
            % type(t0).__name__
        )
    ...
    if t1 is None:
        if isinstance(window, pq.Quantity):
            ...
            if not isinstance(_d_, pq.time.UnitTime):
                raise ValueError(
                    "The window quantity must be in time units; got %d instead"
                    % type(_d__).__name__
                )
            ...
            raise TypeError(
                "When given, window must be a time python quantity or a real scalar; got %s instead"
    ...
    if isinstance(data, neo.Block):  # yes, this function calls itself = recursion
        ...
        if len(data.analogsignals) > 0:
            if analog_index is None:  # from ALl signals
                ...
                ret.analogsignals = [
                    data.analogsignals[analog_index].time_slice(t0, t1)
                ]

            elif isinstance(analog_index, (list, tuple, range)):
                if all([isinstance(s, str) for s in analog_index]):
                    analog_index = [
                        get_index_of_named_signal(data, s) for s in analog_index
                    ]

                ret.analogsignals = [
                    data.analogsignals[k].time_slice(t0, t1) for k in analog_index
                ]

            elif isinstance(analog_index, slice):
                ndx = analog_index.indices(len(data.analogsignals))
                ret.analogsignals = [
                    data.analogsignals[k].time_slice(t0, t1) for k in ndx
                ]
        ...
        if len(data.irregularlysampledsignals) > 0:
            if irregular_index is None:  # from ALl signals
                ret.irregularlysampledsignals = [
                    a.time_slice(t0, t1) for a in data.irregularlysampledsignals
                ]

            elif isinstance(irregular_index, (str, int)):
                if isinstance(irregular_index, str):
                    irregular_index = get_index_of_named_signal(
                        data, irregular_index, stype=neo.IrregularlySampledSignal
                    )

                ret.irregularlysampledsignals = [
                    data.irregularlysampledsignals[irregular_index].time_slice(t0, t1)
                ]
                ...
                if all([isinstance(s, str) for s in irregular_index]):
                    irregular_index = [
                        get_index_of_named_signal(
                            data, s, stype=neo.IrregularlySampledSignal
                        )
                        for s in irregular_index
                    ]

                ret.irregularlysampledsignals = [
                    data.irregularlysampledsignals[k].time_slice(t0, t1)
                    for k in irregular_index
                ]

            elif isinstance(irregular_index, slice):
                ndx = irregular_index.indices(len(data.irregularlysampledsignals))
                ret.irregularlysampledsignals = [
                    data.irregularlysampledsignals[k].time_slice(t0, t1) for k in ndx
                ]
        ...
        if len(data.spiketrains) > 0:
            if spiketrain_index is None:  # ALL spike trains
                ...
                if isinstance(spiketrain_index, str):
                    spiketrain_index = get_index_of_named_signal(
                        data, spiketrain_index, stype=neo.SpikeTrain
                    )

                ret.spiketrains = [
                    data.spiketrains[spiketrain_index].time_slice(t0, t1)
                ]

            elif isinstance(spiketrain_index, (list, tuple, range)):
                if all([isinstance(s, str) for s in spiketrain_index]):
                    spiketrain_index = [
                        get_index_of_named_signal(data, s, stype=neo.SpikeTrain)
                        for s in spiketrain_index
                    ]

                ret.spiketrains = [
                    data.spiketrains[k].time_slice(t0, t1) for k in spiketrain_index
                ]
        ...
        if len(data.events) > 0:
            if event_index is None:
                ...
                if isinstance(event_index, str):
                    event_index = get_index_of_named_signal(
                        data, event_index, stype=neo.Event
                    )
                ...
                if all([isinstance(s, str) for s in event_index]):
                    event_index = [
                        get_index_of_named_signal(data, s, stype=neo.Event)
                        for s in event_index
                    ]
        ...
        if len(data.epochs) > 0:
            if epoch_index is None:
                ...
                if isinstance(epoch_index, str):
                    event_index = get_index_of_named_signal(
                        data, epoch_index, stype=neo.Event
                    )
        ...
        raise TypeError(
            "Expecting data to be a neo.Block or neo.Segment; got %s instead"
            % (type(data).__name__)
        )
...
def splice_signals(*args, times=None):
    r"""Splice-merge signals along the time axis.
    ...
    if all(isinstance(s, (neo.AnalogSignal, DataSignal)) for s in args):
        ...
        if times is None:
            ...
            if times[0] > args[0].t_start:
                raise ValueError(
                    "the 'times' vector starts after that the first signal"
                )
        ...
        return type(args[0])(
            y, t_start=tt[0], units=args[0].units, sampling_rate=args[0].sampling_rate
        )

    elif all(
        isinstance(s, (neo.IrregularlySampledSignal, IrregularlySampledDataSignal))
        for s in args
    ):
        if len(args) == 1:
            return args[0]  # no splice
...
def concatenate_signals(
    *args,
    ...
    def __get_attrs__(s):
        yield from (
            __get_default_attr__(getattr(s, attr[0], None), attr[1])
            for attr in (
                ("description", ""),
                ("name", ""),
                ("file_origin", ""),
    ...
    if len(args) == 1:
        if isinstance(args[0], (tuple, list)):
            ...
            raise TypeError(
                "Expecting a sequence (tuple, or list); got %s instead"
    ...
    # mixed types !!!
    if all([isinstance(sig, neo.AnalogSignal) for sig in signals]) or all(
        [isinstance(sig, DataSignal) for sig in signals]
    ):
        ...
        if not all(sig_shapes[0] == s for s in sig_shapes):
            raise MergeError(
                "Signals do not have identical shapes on non-concatenating axes"
            )

        # this is needed for any concatenation axis!
        if not ignore_domain:
            if not all(
                [sig.times.units] == signals[0].times.units for sig in signals[1:]
            ):
                raise MergeError(
                    "Cannot concatenate signals having different domain units "
                )

            if not all(
                [
                    np.isclose(
                        sig.sampling_rate.magnitude, signals[0].sampling_rate.magnitude
                    )
                    for sig in signals[1:]
                ]
            ):
                raise MergeError(
                    "Cannot concatenate signals having different sampling rates"
                )
            ...
            if axis > 0 or axis == -1:
                if not all([sig.t_start == signals[0].t_start for sig in signals[1:]]):
                    raise MergeError(
                        "Cannot merge channels of signals that have different domains "
                    )
        ...
        if ignore_units:
            ...
            if not all(
                [unitsConvertible(sig.units, signals[0].units) for sig in signals[1:]]
            ):
                raise MergeError(
                    "There are signals with non-convertible units; this may become an error in the future"
                )

            signal_data = [signals[0].magnitude] + [
                sig.rescale(units_0).magnitude
                if sig.units != units_0
                else sig.magnitude
                for sig in signals[1:]
            ]
        ...
        descr, names, files, annots, aannots = tuple(
            (zip(*tuple(tuple(__get_attrs__(s)) for s in signals)))
        )
        ...
        if axis == 1:  #  concatenation on the channel axis (axis 1) a.k.a "merging"
            ...
            if force_contiguous:
                ...
                if ignore_domain:
                    ...
                    t_stop = (
                        sum(s.duration.magnitude.item() for s in signals)
                        * signals[0].times.units
                    )
                else:
                    combined_time_ranges = combine_time_ranges(
...
def copy_with_data_subset(obj, **kwargs):
    r"""Copy data from a source neo container to a new container of the same type.
            ...
            for child_class_name, child_container in group._container_lookup.items():
                ...
                if child_class_name != "ChannelView":
                    data = list(
                        chain(
                            *[
                                s.list_children_by_class(child_class_name)
                    ...
                    objects.extend(
                        [
                            o
                            for o in child_container
                            if any(is_same_as(o, o_) for o_ in data)
                        ]
                    )
                    ...
                    for channelview in group.channelviews:
                        if isinstance(channelview.obj, neo.core.basesignal.BaseSignal):
                            data = list(
                                chain(
                                    *[
                                        s.list_children_by_class(
                                            type(channelview.obj).__name__
                                        )
                                        for s in new_segments
                                    ]
                                )
                            ...
                            if any(is_same_as(channelview.obj, o_) for o_ in data):
                                new_channelview = neo.ChannelView(
                                    channelview.obj,
...
def _(obj, **kwargs):
    ...
    indexing = dict(
        (_container_name(s), kwargs.pop(_container_name(s), None))
        for s in obj._data_child_objects
    )
...
def concatenate_blocks(*args, **kwargs):
    ...
    if len(args) == 1:
        ...
        if isinstance(args[0], str):
            try:
                blocks = get_workspace_neo_blocks(
                    args[0], sortby=sortby, ascending=ascending
                )  # sorting at NOTE: 2023-06-30 12:17:15
            ...
            if all(isinstance(a, str) for a in args[0]):
                try:
                    blocks = get_workspace_neo_blocks(
                        args[0], sortby=sortby, ascending=ascending
                    )  # sorting below see NOTE: 2023-06-30 12:17:15

                except Exception as e:
                    print(
                        "String argument did not resolve to a list of neo.Block objects"
                    )
                    traceback.print_exc()
                    return

            elif all(isinstance(a, neo.Block) for a in args[0]):
                blocks = args[0]

        elif isinstance(args[0], neo.Block):
            blocks = args[0]
        else:
            raise ValueError(
                "Expecting a sequence of blocks or symbols (str) bound to blocks n the workspace, a block or a single str (symbol) bound to a block in the workspace"
            )
        ...
        if all(isinstance(a, str) for a in args):
            ...
            if len(not_found):
                raise KeyError(
                    f"the following objects do not exist in the workspace: {not_found}"
                )
            ...
            if len(wrong_types):
                raise TypeError(
                    f"The following workspace objects are of the wrong type; expecting {neo.Block.__name__}"
                )

            blocks = [ws[a] for a in args]

        elif all(isinstance(a, neo.Block) for a in args):
            blocks = args
    ...
    if isinstance(blocks, neo.Block):
        # nothing to do here: return a copy of 'args'
        return copy_with_data_subset(blocks, **kwargs)

    if isinstance(blocks, collections.abc.Sequence) and all(
        isinstance(a, neo.Block) for a in blocks
    ):
        ...
        if isinstance(sortby, str):
            try:
                if isinstance(sortby, str):
                    blocks = sorted(blocks, key=lambda x: getattr(x, sortby))

                elif isinstance(sortby, typing.Callable):
                    blocks = sorted(blocks, key=sortby)
        ...
        firstBlockDatetime = blocks[0].rec_datetime
        ...
        for k, block in enumerate(blocks):
            ...
            try:
                block_src_copy = copy_with_data_subset(block, **kwargs)
            except:
                print(
                    f"*****\nCannot copy block {k} (named: {getattr(block, 'name', None)}) with data subset\n*****\n"
                )
                raise
            ...
            deltaSeconds = (
                block_src_copy.rec_datetime - firstBlockDatetime
            ).total_seconds() * pq.s

            for ks, seg in enumerate(block_src_copy.segments):
                seg.file_origin = block_src_copy.file_origin
                seg.rec_datetime = (
                    block_src_copy.rec_datetime
                )  # not quite correct, is it ?!?
                # if ks == 0:
                #     seg.rec_datetime = block_src_copy.rec_datetime # not quite correct, is it ?!?
                # else:
                #     seg_start = tfloat(segment_start(seg).magnitude)
                ...
                if len(seg.events):
                    ...
                    for event in seg.events:
                        if checkTimeUnits(event.times):
                            ...
                            evt = event.__class__(
                                times=event.times + deltaSeconds,
                                labels=event.labels,
                                units=event.units,
                                name=event.name,
            ...
            if len(block_src_copy.groups):
                for group in block_src_copy.groups:
                    ...
                    for (
                        child_class_name,
                        child_container,
                    ) in group._container_lookup.items():
                        ...
                        if child_class_name != "ChannelView":
                            data = list(
                                chain(
                                    *[
                                        s.list_children_by_class(child_class_name)
                                        for s in block_src_copy.segments
                                    ]
                                )
                            )
                            objects.extend(
                    ...
                    if len(objects):
                        ...
                        if hasattr(group, "channelviews") and len(group.channelviews):
                            for channelview in group.channelviews:
                                if isinstance(
                                    channelview.obj, neo.core.basesignal.BaseSignal
                                ):
                                    data = list(
                                        chain(
                                            *[
                                    ...
                                    if any(
                                        ...
                                        target_group.channelviews.append(
                                            newchannel_view
                                        )

    else:
        raise TypeError(
            "Expecting a neo.Block or a sequence of neo.Block objects, got %s instead"
            % type(args).__name__
        )
...
def get_events(
    *src: typing.Union[neo.Block, neo.Segment, typing.Sequence],
    ...
    triggers: typing.Optional[
        typing.Union[bool, str, int, type, typing.Sequence]
    ] = None,
    match: str = "==",
    clear: bool = False,
):
    r"""Returns a collection of neo.Events embedded in data.
    ...
    *src: neo.Block, neo.Segment, sequence (tuple, list) of neo.Block, sequence
    ...
    if not isinstance(match, str):
        raise TypeError(
            "match rule expected a str; got %s instead" % type(match).__name__
        )

    if match in ("strict", "s", "=="):
        typefilter = (
            lambda x, y: x.type == y
            if isinstance(y, TriggerEventType)
            else any([x.type == y_ for y_ in y])
        )
        ...
        # y must be composite
        typefilter = (
            lambda x, y: (x.type == y or x.type.is_component_of(y))
            if isinstance(y, TriggerEventType)
            else any([(x.type == y_ or x.type.is_component_of(y_)) for y_ in y])
        )
        ...
        # y must be composite
        typefilter = (
            lambda x, y: x.type.is_component_of(y)
            if isinstance(y, TriggerEventType)
            else any([x.type.is_component_of(y_) for y_ in y])
        )
        ...
        # will be excluded
        typefilter = (
            lambda x, y: x.type.is_primitive_of(y)
            if isinstance(y, TriggerEventType)
            else any([x.type.is_primitive_of(y_) for y_ in y])
        )
        ...
        typefilter = (
            lambda x, y: (x.type == y or x.type.includes(y))
            if isinstance(y, TriggerEventType)
            else any([(x.type == y_ or x.type.includes(y_)) for y_ in y])
        )
        ...
        typefilter = (
            lambda x, y: x.type.includes(y)
            if isinstance(y, TriggerEventType)
            else any([x.type.includes(y_) for y_ in y])
        )

    elif match in ("related", "rel", "r", "sim", "~"):
        typefilter = (
            lambda x, y: (
                x.type == y or x.type.is_component_of(y) or x.type.includes(y)
            )
            if isinstance(y, TriggerEventType)
            else any(
                [
                    (x.type == y_ or x.type.is_component_of(y_) or x.type.includes(y_))
                    for y_ in y
                ]
            )
        )
        ...
        typefilter = (
            lambda x, y: (x.type.is_component_of(y) or x.type.includes(y))
            if isinstance(y, TriggerEventType)
            else any([(x.type.is_component_of(y_) or x.type.includes(y_)) for y_ in y])
        )
        ...
        typefilter = (
            lambda x, y: x.type != y
            if isinstance(y, TriggerEventType)
            else all([x.type != y_ for y_ in y])
        )

    elif match in ("unrelated", "ur", "/="):
        typefilter = (
            lambda x, y: (
                x.type != y and not x.type.includes(y) and not y.includes(x.type)
            )
            if isinstance(y, TriggerEventType)
            else all(
                [
                    (
                        x.type != y_
                        and not x.type.includes(y_)
                        and not y_.includes(x.type)
                    )
                    for y_ in y
                ]
            )
    ...
    if isinstance(triggers, bool):
        filtfn = (
            partial(filter, lambda x: isinstance(x, TriggerEvent))
            if triggers
            else partial(filterfalse, lambda x: isinstance(x, TriggerEvent))
        )
        ...
        filtfn = partial(
            filter, lambda x: isinstance(x, TriggerEvent) and typefilter(x, triggers)
        )

    elif isinstance(triggers, (tuple, list)):
        triggers = [
            TriggerEventType.type(t)
            for t in triggers
            if isinstance(t, (TriggerEventType, int, str))
        ]

        filtfn = partial(
            filter,
            lambda x: isinstance(x, TriggerEvent)
            and any([typefilter(x, tr) for tr in triggers]),
        )
    ...
    if clear:  # function is used to clear events
        if isinstance(src, neo.Block):
            ...
            raise TypeError(
                "Expecting a sequence of neo.Block or neo.Segments (no type mixing)"
            )
        ...
        if isinstance(src, neo.Block):
            if as_dict:
                return {
                    "block_0": dict(
                        [
                            ("segment_%d" % k, [e for e in filtfn(s.events)])
                            for k, s in enumerate(src.segments)
                        ]
                    )
                }
            else:
                ret = [
                    [e for e in filtfn(s.events)] for s in src.segments
            ...
            if as_dict:
                ...
                return [
                    e for e in filtfn(src.events)
                ]  # by default only a flattened list
            ...
            if all([isinstance(v, neo.Block) for v in src]):
                            ...
                            (
                                ...
                                dict(
                                    [
                                        (
                                            "segment_%d" % k,
                                            [e for e in filtfn(s.events)],
                                        )
                                        for k, s in enumerate(b.segments)
                                    ]
                                ),
                            )
                            for kb, b in enumerate(src)
                        ]
                    )
                else:
                    ...
                    if flat:
                        ...
                        return [
                            ...
                            for e in chain.from_iterable(
                                [
                                    [e for e in filtfn(s.events)]
                                    for s in chain(*[b.segments for b in src])
                                ]
                            )
                        ]
                    else:  # ragged nested sequence
                        return [
                            [[e for e in filtfn(s.events)] for s in b.segments]
                            for b in src
                        ]
                ...
                if as_dict:
                    ...
                    if flat:
                        return [
                            e
                            for e in chain(
                                *[[o for o in filtfn(s.events)] for s in src]
                            )
                        ]
                    else:  # ragged nested sequence
                ...
                raise TypeError(
                    "Expecting a uniformly typed sequence of neo.Block or neo.Segment objects"
                )
...
def check_ephys_data_collection(x: typing.Any, mix: bool = False):
    r"""Checks if x is a collection of electrophysiology data.
    ...
    if isinstance(x, (tuple, list)):
        if mix:
            ...
            return any(
                [
                    all([isinstance(x_, e_type) for x_ in x])
                    for e_type in ephys_data_collection
                ]
            )
...
def check_ephys_data(x: typing.Any, mix: bool = False):
    r"""Checks if x is a electrophysiology data type.
    ...
    if isinstance(x, (tuple, list)):
        if mix:
            ...
            return any(
                [all([isinstance(x_, e_type) for x_ in x]) for e_type in ephys_data]
            )
...
def get_non_empty_events(sequence: (tuple, list)):
    ...
    if not all([isinstance(e, (neo.Event, TriggerEvent)) for e in sequence]):
        raise TypeError(
            "Expecting a sequence containing only neo.Event or datatypes.TriggerEvent objects"
        )
...
def clear_spiketrains(
    data: typing.Union[
        neo.Segment, neo.Block, typing.Sequence[typing.Union[neo.Segment, neo.Block]]
    ],
):
    r"""Removes ALL SpikeTrain objects from the data"""
...
def remove_spiketrain(
    segment: neo.Segment,
    index: typing.Union[int, str, typing.Sequence[int], typing.Sequence[str]],
):
    r"""Remove the SpikeTrain at specified index from the segment's spiketrains.
    ...
    if len(trains):
        stl2 = neo.core.spiketrainlist.SpikeTrainList(
            items=trains
        )  # will set up channel ids
...
def remove_events(event, segment, byLabel=True):
    ...
    if not isinstance(segment, neo.Segment):
        raise TypeError(
            "segment expected to be a neo.Segment; got %s instead"
    ...
    if isinstance(event, neo.Event):
        if event in segment.events:  # find event reference stored in events list
            ...
            evs = [
                (k, e) for (k, e) in enumerate(segment.events) if e.is_same_as(event)
            ]
            ...
            if len(evs):
                ...
                keep_events = [
                    segment.events[k] for k in all_events_ndx if k not in evndx
                ]
        ...
        evs = [
            ...
            if isinstance(e, TriggerEvent) and e.type & event
        ]
...
def _(
    ...
    if comparator not in (operator.eq, isclose):
        raise TypeError(
            f"'comparator' expected to be operator.eq or utilties.isclose; got {comparator} instead"
        )
    ...
    if ret:
        ...
        ret &= reduce(
            ...
            (
                sim_func(x_, y_)
                for x_, y_ in (
                    (getattr(a, attr[0]), getattr(b, attr[0]))
...
def _(
    ...
    if ret:
        ...
        ret &= reduce(
            ...
            (
                sim_func(x_, y_)
                for x_, y_ in (
                    (getattr(a, attr[0]), getattr(b, attr[0])) for attr in data_attrs
                )
            ),
        )
    ...
    if ret and len(a.data_children):
        ret &= reduce(
            operator.and_,
            (neo_sim_func(x_, y_) for x_, y_ in zip(a.data_children, b.data_children)),
        )
    ...
    if ret and len(a.container_children):
        ret &= reduce(
            ...
            (
                neo_sim_func(x_, y_)
                for x_, y_ in zip(a.container_children, b.container_children)
            ),
        )
    ...
    if ret:
        ret &= reduce(
            ...
            (
                ...
                for x_, y_ in (
                    (getattr(a, attr[0]), getattr(b, attr[0]))
                    for attr in a._recommended_attrs
                )
            ),
        )
...
def is_in(x: neo.core.dataobject.DataObject, container: collections.abc.Sequence):
    r"""Testing for the existence of a neo DataObject in a Python Sequence.
    ...
    if not isinstance(container, collections.abc.Sequence):
        raise TypeError(
            f"Expecting a Python sequence; got {type(container).__name__} instead"
        )
...
def is_likely_in(
    x: neo.core.dataobject.DataObject, container: collections.abc.Sequence
):
    if not isinstance(x, neo.core.dataobject.DataObject):
        raise TypeError(f"Expecting a DataObject; got {type(x).__name__} instead")
...
def lookup(
    signal, value, channel=0, rtol=1e-05, atol=1e-08, equal_nan=False, right=False
):
    r"""Lookup signal values for given domain value(s).
    ...
    if not isinstance(
        ...
        raise TypeError(
            "signal expected to be a signal; got %s instead" % type(signal).__name__
        )

    if not isinstance(value, (numbers.Number, tuple, list)):
        raise TypeError(
            "value expected to be a float, or sequence of one or two floats; got %s instead"
            % type(value).__name__
        )
    ...
    if isinstance(value, (tuple, list)):
        if len(value) < 1 or len(value) > 2:
            raise TypeError(
                "When a tuple, value must contain at most two elements; got %d instead"
                % len(value)
            )
    ...
    if not isinstance(channel, int):
        raise TypeError(
            "channel expected to be an int; got %s instead" % type(channel).__name__
        )

    if channel < 0 or channel >= signal.shape[1]:
        raise ValueError(
            "channel index %d out of range for a signal with %d channels"
            % (channel, signal.shape[1])
        )

    if not isinstance(rtol, numbers.Number):
        raise TypeError(
            "rtol expected to be a float; got %s instead" % type(rtol).__name__
        )

    if not isinstance(atol, numbers.Number):
        raise TypeError(
            "atol expected to be a float; got %s instead" % type(atol).__name__
        )

    if not isinstance(equal_nan, bool):
        raise TypeError(
            "equal_nan expected to be a bool; got %s instead" % type(equal_nan).__name__
        )
    ...
    if isinstance(value, (tuple, list)):
        ...
        ndx = np.isclose(
            np.array(domain), value, atol=atol, rtol=rtol, equal_nan=equal_nan
        )
...
def inverse_lookup(
    ...
    r"""Look-up for domain values given a nominal signal value.
    ...
    if isinstance(value, (tuple, list)):
        if len(value) < 1:
            raise TypeError(
                "When a tuple, value must contain at least one element; got %d instead"
                % len(value)
            )
    ...
    if isinstance(value, (tuple, list)):
        ...
        ndx = np.isclose(
            signal_values, value, atol=atol, rtol=rtol, equal_nan=equal_nan
        )
...
def intersect_annotations(A, B):
    ...
    for key in set(A.keys()) & set(B.keys()):
        ...
        assert type(v1) == type(v2), "type({}) {} != type({}) {}".format(
...
def merge_annotation(a, b):
    ...
    assert type(a) == type(b), "type({}) {} != type({}) {}".format(
        a, type(a), b, type(b)
    )
...
def irregularsignal2epoch(sig, name=None, labels=None):
    ...
    if not isinstance(sig, neo.IrregularlySampledSignal):
        raise TypeError(
            "Expecting a neo.IrregularlySampledSignal; got %s instead"
            % type(sig).__name__
        )

    if not unitsConvertible(sig.units, sig.times.units):
        raise TypeError(
            "Signal was expected to have time units; it has %s instead" % sig.units
        )
    ...
    if isinstance(labels, str) and len(labels.strip()):
        ...
        if labels.shape != sig.times.shape:
            raise TypeError(
                "'labels' array has wrong shape (%s); shloud have %s"
                % (labels.shape, sig.times.shape)
            )

    elif labels is not None:
        raise TypeError(
            "'labels' expected to be a str, numpy array of string dtype, or None; got %s instead"
            % type(labels).__name__
        )

    if not isinstance(name, (str, type(None))):
        raise TypeError(
            "'name' expected to be None or a string; got %s instead"
            % type(name).__name__
        )
    ...
    ret = neo.Epoch(
        times=sig.times, durations=sig.magnitude * sig.units, name=name, labels=labels
    )
...
def aggregate_signals(
    *args, name_prefix: str, collectSD: bool = True, collectSEM: bool = True
    ...
    r"""Returns signal mean, SD, SEM, and number of signals in args.
    ...
    if (
        ...
        and all([isinstance(a, (neo.AnalogSignal, Datasignal)) for a in args[0]])
    ):
        args = args[0]
...
def average_irregular_signals(*args, fun=np.mean, name: typing.Optional[str] = None):
    ...
    if (
        ...
        and all(
            [
                isinstance(
                    a, (neo.IrregularlySampledSignal, IrregularlySampledDataSignal)
                )
                for a in args[0]
            ]
        )
    ...
    assert all(isinstance(a, neo.IrregularlySampledSignal) for a in args) or all(
        isinstance(a, IrregularlySampledDataSignal) for a in args
    ), (
        "This function only supports all neo.IrregularlySampledSignal OR all IrregularlySampledDataSignal objects (no mixing of types)"
    )
...
def average_signals(*args, fun=np.mean, name: typing.Optional[str] = None):
    ...
    if (
        ...
        and all(
            [
                isinstance(a, (neo.core.analogsignal.AnalogSignal, DataSignal))
                for a in args[0]
            ]
        )
    ):
    ...
    assert all(isinstance(a, neo.AnalogSignal) for a in args) or all(
        isinstance(a, DataSignal) for a in args
    ), (
        "This function only supports all neo.AnalogSignal OR all DataSignal objects (no mixing of types)"
    )
...
def __applyRecDateTime__(sgm, blk):
    r"""Applies the block's rec_datetime to all its contained segments"""
...
def average_segments(*args, **kwargs) -> typing.List[neo.Segment]:
    ...
    if m is None or m == 0:
        if n is None:
            ranges_avg = [
                range(0, len(args))
            ]  # take the average of the whole segments list
        else:
            if n == 1:
                return args
        ...
        ranges_avg = [
            range(k, k + n) for k in range(0, len(args), m)
        ]  # this will result in as many segments in the data block
            ...
            if k == range_avg.start:
                ...
                if analog_index is None:
                    ...
                    try:
                        ...
                        print(
                            f"Analog index {analog_index} is invalid for segment {k} (named: {args[k].name})"
                        )
                        raise
                    ...
                    if len(ss) < len(selected_signals):  # this shouldn't really happen
                        raise ValueError(
                            f"The selection of analogsignals ({analog_index}) in segment {k} (named: {args[k].name}) has generated only {len(ss)} when {len(selected_signals)} were expected.\n\n"
                        )

                    if len(ss) > len(selected_signals):  # this shouldn't really happen
                        warnings.warn(
                            f"The selection of analogsignals ({analog_index}) in segment {k} (named: {args[k].name}) has generated {len(ss)} but only the first {len(selected_signals)} will be used ",
                            category=RuntimeWarning,
                        )

                for i in range(len(selected_signals)):
                    try:
                        selected_signals[i] += __resample_to_add__(
                            selected_signals[i], ss[i]
                        )
                    except:
    ...
    **kwargs:
    ...
    for k in range(nSegs):
        ...
        if min(nSigs) != max(nSigs):
            raise ValueError(
                "Corresponding segments must have the same number of signals"
            )

        segment = average_segments(
            [block.segments[k] for block in args], analog_index=analog_index
        )
...
def average_segments_in_block(data, **kwargs):
    ...
    if segments is not None:
        if isinstance(segments, (tuple, list)) and all(
            isinstance(k, numbers.Integral) and k >= 0 and k < len(data.segments)
            for k in segments
        ):
    ...
    if isinstance(new_segs, (tuple, list)) and all(
        isinstance(s, neo.Segment) for s in new_segs
    ):
    ...
    if data.name is None or (isinstance(data.name, str) and len(data.name) == 0):
        ...
        if (
            data.file_origin is not None
            and isinstance(data.file_origin, str)
            and len(data.file_origin) > 0
        ):
...
def average_blocks(*args, **kwargs) -> neo.Block:
    ...
    def __get_blocks_by_name__(*arg):
        ...
        if len(arg) == 1:
            ret_ = workspacefunctions.getvars(
                arg[0], glob=True, var_type=neo.Block, ws=ws
            )

        elif len(arg) > 1:
            ret_ = workspacefunctions.getvars(
                *arg, glob=True, var_type=neo.Block, ws=ws
            )
        else:
    ...
    if len(blocks) == 1:
        if segment_index is None:
            ...
            ret.annotations["Source"] = {
                "name": blocks[0].name,
                "segments": segment_index,
            }

        elif isinstance(segment_index, (tuple, list)) and all(
            isinstance(v, int) and v < len(blocks[0].segments) for v in segment_index
        ):
        ...
        elif isinstance(segment_index, range) and segment_index.stop <= len(
            blocks[0].segments
        ):
            ret.segments = [blocks[0].segments[k] for k in segment_index]
            ret.annotations["Source"] = {
        ...
        elif isinstance(segment_index, slice) and segment_index.stop <= len(
            blocks[0].segments
        ):
    ...
    for r, range_avg in enumerate(ranges_avg):
        ...
        if segment_index is None:
            ...
            if segment_index < 0:
                raise ValueError(
                    f"Segment index must be >= 0; instead, got {segment_index}"
                )

            out_of_range = [
                k for k in range_avg if segment_index >= len(blist[k].segments)
            ]
            if len(out_of_range):
                raise ValueError(
                    f"Segment index {segment_index} out of range for block {k} ({blist[k].name})"
                )
            ...
            isinstance(v, int) for v in segment_index
        ):
            ...
            for v in segment_index:
                ...
                if len(out_of_range):
                    raise ValueError(
                        f"Segment index {v} out of range for block {k} ({blist[k].name})"
                    )
            ...
            slist = list(
                itertools.chain.from_iterable(
                    [
                        average_segments(
                            [blist[k].segments[v] for k in range_avg],
                            signals=analog_index,
                        )
                        for v in segment_index
                    ]
                )
            ...
            out_or_range = [
                k for k in range_avg if segment_index.stop > len(blist[k].segments)
            ]
            if len(out_of_range):
                raise ValueError(
                    f"Last segment index in {segment_index} is out of range for block {k} ({blist[k].name})"
                )
            ...
            slist = [
                average_segments(
                    [blist[k].segments[v] for k in range_avg], signals=analog_index
                )
                for v in srange
            ]
            seg_list.extend(slist)
...
def average_blocks_new(*args, **kwargs):
    ...
    if len(args) == 1:
        if isinstance(args[0], str):
            try:
                args = get_workspace_neo_blocks(
                    args[0], sortby=sortby, ascending=ascending
                )  # sorting at NOTE: 2023-05-22 16:04:11
            ...
            try:
                args = get_workspace_neo_blocks(
                    args[0], sortby=sortby, ascending=ascending
                )  # sorting below see NOTE: 2023-05-22 16:04:11
    ...
    if isinstance(args, collections.abc.Sequence) and all(
        isinstance(a, neo.Block) for a in args
    ):
        ...
        if isinstance(segments, int):  # ⇒ list of single segment indices, one per block
            if segments not in range(-nSegs, nSegs):
                raise ValueError(
                    f"Invalid segment index {segments} for {nSegs} {pluralize('segment', nSegs)} in individual Blocks"
                )

            segments = list(
                chain.from_iterable(
                    map(lambda x: normalized_index(x.segments, segment), args)
                )
            )

        elif isinstance(segments, (tuple, list)) and all(
            isinstance(x, int) for x in segments
        ):  # ⇒ sequence of ints
        ...
        ret = neo.core.Block(
            ...
            file_origin=ret_file_origin,
            file_datetime=ret_file_datetime,
            rec_datetime=ret_rec_datetime,
            **ret_annotations,
        )
        ...
        for range_avg in ranges_avg:
            ...
            for bk in range_avg:
                ...
                block_segments = [
                    copy_with_data_subset(block.segments[sk], **kw)
                    for sk in segment_index
                ]
                ...
                for sk in range(len(block_segments)):
                    if sk == 0:
                        seg.analogsignals[:] = [
                            make_neo_object(s) for s in block_segments[sk].analogsignals
                        ]

                    else:
                        for sig_ndx, sig in enumerate(block_segments[sk].analogsignals):
                            seg.analogsignals[sig_ndx] += make_neo_object(
                                __resample_to_add__(signals[sig_ndx, sig])
                            )
...
def parse_acquisition_metadata(
    ...
    r"""TODO Parses metadata from electrophysiology acquisition data.
...
def plot_neo(
    ...
    **kwargs,
    ...
    r"""Wrapper to matplotlib.pyplot for `neo` signal objects.
    ...
    if (
        hasattr(obj, "array_annotations")
        and len(obj.array_annotations)
        and "channel_names" in obj.array_annotations
    ):
