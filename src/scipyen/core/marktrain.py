...
def check_has_dimensions_domain(*values, units) -> None:
    """
    ...
    errmsgs = []
    for value in values:
        if value.units != units and not unitsConvertible(value, units):
            dim = value.dimensionality.simplified
            errmsgs.append(f"value {value} has dimensions {dim}, not [{units}]")
    if errmsgs:
        raise ValueError("\n".join(errmsgs))
...
def _check_waveform_dimensions(marktrain) -> None:
    """
    ...
    Parameters
    ----------
    marktrain: neo.core.MarkTrain
        The neo.core.MarkTrain to check for waveforms
    ...
    if not marktrain.size:
        return

    waveforms = marktrain.waveforms

    if (waveforms is None) or (not waveforms.size):
        return

    if waveforms.shape[0] != len(marktrain):
        raise ValueError(
            f"Marktrain length ({len(marktrain)}) does not match to number of "
...
class MarkTrain(DataObject):
    r"""neo.SpikeTrain unconstrained ot the time domain.
TBH, one *could* use DataMark with an array of 1D domain coordinates instead...
"""
    _parent_objects = ("Segment",)
    _parent_attrs = ("segment",)
    _quantity_attr = "times"
    _necessary_attrs = (("times", pq.Quantity, 1), ("t_start", pq.Quantity, 0), ("t_stop", pq.Quantity, 0))
    _recommended_attrs = (
    ...
    def __new__(
        ...
        **annotations,
        ...
        This is called whenever a new :class:`MarkTrain` is created from the
        ...
        # marktrain times always need to be 1-dimensional
        if len(obj.shape) > 1:
    ...
    def __init__(
        ...
        **annotations,
        ...
        """
        Initializes a newly constructed :class:`MarkTrain` instance.
        """
        # This method is only called when constructing a new MarkTrain,
    ...
    def _repr_pretty_(self, pp, cycle):
        ...
        pp.text(
            f"{self.__class__.__name__} containing {self.size} spikes{waveforms}; "
    ...
    def rescale(self, units, dtype=None):
        """
        Return a copy of the :class:`MarkTrain` converted to the specified units
    ...
    def __array_finalize__(self, obj):
        """
        This is called every time a new :class:`MarkTrain` is created.

        It is the appropriate place to set default values for attributes
        for :class:`MarkTrain` constructed by slicing or viewing.
    ...
    def __repr__(self):
        """
        Returns a string representing the :class:`MarkTrain`.
        """
        return f"<MarkTrain({super().__repr__()}, [{self.t_start}, {self.t_stop}])>"

    def sort(self):
        """
        Sorts the :class:`MarkTrain` and its :attr:`waveforms`, if any,
        by time.
        """
        ...
        # a dependency on the slicing functionality of MarkTrain.
        super().sort()
    ...
    def __add__(self, time):
        """
        Shifts the time point of all marks by adding the amount in
        ...
        check_has_dimensions_domain(time, self.spikes.units)
        if isinstance(time, MarkTrain):
            raise TypeError("Can't add two mark trains")
    ...
    def __sub__(self, time):
        ...
        In general, it is not possible to subtract two MarkTrain objects (raises ValueError).
        However, if `time` is itself a MarkTrain of the same size as the MarkTrain,
        ...
        """
        spikes = self.view(pq.Quantity)
        check_has_dimensions_domain(time)
        if isinstance(time, MarkTrain):
            if self.size == time.size:
                return spikes - time
            else:
                raise TypeError("Can't subtract mark trains with different sizes")
        else:
            new_times = spikes - time
            if time.size > 1:
                t_start = min(self.t_start, np.min(new_times))
    ...
    def __getitem__(self, i):
        ...
        except AttributeError:  # If Quantity was returned, not MarkTrain
            pass
        return obj
    ...
    def _copy_data_complement(self, other, deep_copy=False):
        """
        Copy the metadata from another :class:`MarkTrain`.
        ...
        for attr in ("left_sweep", "sampling_rate", "name", "file_origin", "description", "annotations"):
    ...
    def duplicate_with_new_data(self, signal, t_start=None, t_stop=None, waveforms=None, deep_copy=True, units=None):
        """
        Create a new :class:`MarkTrain` with the same metadata
        ...
        new_st = MarkTrain(signal, t_start=t_start, t_stop=t_stop, waveforms=waveforms, units=units)
    ...
    def time_slice(self, t_start, t_stop):
        """
        Creates a new :class:`MarkTrain` corresponding to the time slice of
        the original :class:`MarkTrain` between (and including) times
    ...
    def time_shift(self, t_shift):
        """
        Shifts a :class:`MarkTrain` to start at a new time.

        Parameters
        ----------
        t_shift: Quantity (time)
            Amount of time by which to shift the :class:`MarkTrain`.

        Returns
        -------
        marktrain: :class:`MarkTrain`
            New instance of a :class:`MarkTrain` object starting at t_shift later than the
            original :class:`MarkTrain` (the original :class:`MarkTrain` is not modified).
        """
        # We need new to make a new MarkTrain
        ...
        # the length of the MarkTrain does not change.
    ...
    def merge(self, *others):
        """
        Merge other :class:`MarkTrain` objects into this one.

        The times of the :class:`MarkTrain` objects combined in one array
        and sorted.

        If the attributes of the :class:`MarkTrain` objects are not
        ...
        for other in others:
            if isinstance(other, neo.io.proxyobjects.SpikeTrainProxy):
                raise MergeError(
                    ...
                    "into regular MarkTrain objects, please load them first."
                )
            elif not isinstance(other, MarkTrain):
                raise MergeError("Cannot merge, only MarkTrain" "can be merged into a MarkTrain.")
            ...
            if self.segment != other.segment:
                raise MergeError("Cannot merge these signals as they belong to" " different segments.")
        ...
        for name in ("name", "description", "file_origin"):
            ...
            # check if self is already a merged marktrain
            ...
            for other in others:
                ...
                # check if the other marktrain is already a merged marktrain
        ...
        merged_annotations = merge_annotations(*(st.annotations for st in all_spiketrains))
    ...
    def _merge_array_annotations(self, others, sorting=None):
        ...
        for key in keys:
            try:
                ...
                other_ann = np.concatenate([deepcopy(other.array_annotations[key]) for other in others])
    ...
    def times(self):
        """
        Returns the :class:`MarkTrain` as a quantity array.
        """
