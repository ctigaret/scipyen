...
class AttributeAdapter(ABC):
    r"""Abstract Base Class as a callable for pre- and post-validation"""
...
class AttributeSpecification:
    r"""The non-keyword fields are set to match neo attribute specifications"""

    name: str  # decriptor name
    types: typing.Union[
        type, typing.Tuple[type], typing.Callable
    ]  # descriptor types or predicates
    ndim: typing.Union[int, dict] = 0  # ndim (neo model), dtype
    dtype: typing.Union[np.dtype, dict] = dataclasses.field(
        default_factory=lambda: np.dtype(float)
    )
    ...
    def __post_init__(self):
        ...
        if not (isinstance(self.types, type)) and not (
            isinstance(self.types, tuple)
            and all(isinstance(t_, type) for t_ in self.types)
        ):
        ...
        if self.default is dataclasses.MISSING:
            if self._allow_none:
                ...
                if isinstance(self.types, type):
                    type_ = self.types
                elif isinstance(self.types, tuple) and all(
                    isinstance(t_, type) for t_ in self.types
                ):
                    type_ = self.types[0]
...
class DescriptorValidatorABC(ABC):
    ...
    def __set__(self, obj, value) -> None:
        r"""Assigns a new value to the private attribute accessed by the descriptor.
...
class BaseDescriptorValidator(DescriptorValidatorABC):
    ...
    def make_private_name(name: str) -> str:
        r"""Find out what private name this would operate on"""
        return f"_{name}_"
...
class DescriptorTypeValidator(BaseDescriptorValidator):
    ...
    def validate(self, value):
        if not isinstance(value, tuple(self.types)):
            raise TypeError(
                f"For {self.private_name} one of {self.types} was expected; got {type(value).__name__} instead"
            )
...
=========
    ...
    """
        ...
        if len(dcriteria) and all(
            isinstance(k, type) and isinstance(v, dict) for k, v in dcriteria.items()
        ):
    ...
    def validate(self, value):
        ...
        if not self.allow_none:
            ...
            if len(self.dcriteria):
                ...
                criteria = tuple(
                    v
                    for k, v in self.dcriteria.items()
                    if is_type_or_subclass(value, k)
                )
                criterion = dict()
    ...
    def _check_value_special_criteria_(self, value, crit: dict):
        ...
        if isinstance(value, (str, bytes, bytearray, deque, list, tuple, dict)):
            ...
            if isinstance(c, int) and c >= 0:
                if len(value) != c:
                    raise AttributeError(
                        f"Unexpected length of {type(value).__name__}; expecting {c}, got {len(value)}"
                    )

            if isinstance(value, (deque, list, tuple, dict)):
                c = crit.get("types", tuple())

                if isinstance(c, type) or (
                    isinstance(c, tuple)
                    and len(c)
                    and all(isinstance(c_, type) for c_ in c)
                ):
                    if isinstance(value, (deque, list, tuple)):
                        if not all(isinstance(e, c) for e in value):
                            raise AttributeError(
                                f"Collection's elements have unexpected types. Expecting {c}, got {tuple(type(e).__name__ for e in value)}"
                            )

                    if isinstance(value, dict):
                        if not all(isinstance(v, c) for v in value.values()):
                            raise AttributeError(
                                f"Mapping's values have unexpected types. Expecting {c}, got {tuple(type(v).__name__ for v in value.values())}"
                            )
                ...
                if isinstance(value, dict):
                    ...
                    if isinstance(c, type) or (
                        ...
                        if not all(isinstance(k, c) for k in v_keys):
                            raise AttributeError(
                                f"Mapping's keys have unexpected types. Expecting {c}, got {tuple(type(v).__name__ for v in v_keys)}"
                            )
                    ...
                    if isinstance(c, tuple) and len(c):
                        ...
                        if v_keys != c_keys():
                            raise AttributeError(
                                f"Mapping's keys ({v_keys}) do not match the criterion ({c_keys})"
                            )
                    ...
                    if isinstance(c, dict) and len(c):
                        ...
                        for k in v_keys:
                            if isinstance(c[k], type) or (
                                isinstance(c[k], tuple)
                                and len(c[k])
                                and all(isinstance(c_, type) for c_ in c[k])
                            ):
                                if not isinstance(value[k], c[k]):
                                    raise AttributeError(
                                        f"Key {k} expected to be mapped to a type in {c[k]}; instead got {type(value[k]).__name__}"
                                    )
        ...
        if isinstance(value, np.ndarray):
            ...
            if isinstance(c, int) and c >= 0:
                if value.ndim != c:
                    raise AttributeError(
                        f"Expecting an array with {c} dimensions; instead, got {value.ndim}"
                    )
            ...
            if (
                isinstance(c, tuple)
                and len(c)
                and all(isinstance(c_, int) and c_ >= 0 for c_ in c)
            ):
                if value.shape != c:
                    raise AttributeError(
                        f"Expecting an array with shape {c}; instead, got {value.shape}"
                    )
            ...
            if isinstance(c, np.dtype):
                if not np.issubdtype(value.dtype, c):
                    raise AttributeError(
                        f"Expecting an array with dtype inheriting from {c}; got {value.dtype} instead"
                    )

            elif isinstance(c, tuple) and len(c) and all(isinstance(c_, np.dtype)):
                if not any(np.issubdtype(value.dtype, c_) for c_ in c):
                    raise AttributeError(
                        f"Expecting an array with dtype inheriting from one of {c}; got {value.dtype} instead"
                    )
            ...
            if isinstance(value, pq.Quantity):
                ...
                if isinstance(c.pq.Quantity):
                    if not unitsConvertible(value, c):
                        raise AttributeError(
                            f"Value units {value.units} are incompatible with {c}"
                        )
            ...
            if isinstance(value, vigra.VigraArray):
                ...
                if isinstance(c, vigra.AxisTags):
                    if value.AxisTags != c:
                        raise AttributeError(
                            f"Expecting c{axistags}; got {value.axistags} instead"
                        )
        ...
        if isinstance(value, (pd.Series, pd.DataFrame)):
            ...
            if (
                ...
                if value.shape != c:
                    raise AttributeError(
                        f"Expecting {type(value).__name__} with shape {c}; instead, got {value.shape}"
                    )
            ...
            if isinstance(c, type) and issubclass(c, pd.Index):
                if not isinstance(value.index, c):
                    raise AttributeError(
                        f"Invalid index type ({type(value.index).__name__}); expecting {c}"
                    )
            ...
            if isinstance(value, pd.Series):
                ...
                if isinstance(c, tuple) and len(c) and all(isinstance(c, np.dtype)):
                    if not any(np.issubdtype(value.dtype, d) for d in c):
                        raise AttributeError(
                            f"Invalid dtype ({value.dtype}); expecting {c}"
                        )
            ...
            if isinstance(value, pd.DataFrame):
                ...
                if isinstance(c, type) and issubclass(c, pd.Index):
                    if not isinstance(value.columns, c):
                        raise AttributeError(
                            f"Invalid columns type ({type(value.columns).__name__}); expecting {c}"
                        )
...
class Timer(object):
    r"""Recipe 13.13 "Making a Stopwatch Timer" in Python Cookbook 3rd Ed. 2013"""
...
def signature_as_dict(
    ...
    if isinstance(sig, Signature):
        if not isinstance(name, str) or len(name.strip()) == 0:
            raise ValueError(
                f"With Signature objects, 'funcname' is REQUIRED; got {name} instead"
            )

        if not isinstance(qualname, str) or len(qualname.strip()) == 0:
            raise ValueError(
                f"With Signature objects, 'qualname' is REQUIRED; got {qualname} instead"
            )

        if not isinstance(module, str) or len(module.strip()) == 0:
            raise ValueError(
                f"With Signature objects, 'modname' is REQUIRED; got {module} instead"
            )
    ...
    if not isinstance(sig, Signature):
        raise TypeError(
            f"Expecting a Signature object, a function, or a method; got {type(sig).__name__} instead"
        )
    ...
    return Bunch(
        ...
        qualname=qualname,
        module=module,
        positional=posonly_params,
        named=named_params,
        varpos=varpos_params,
        kwonly=kwonly_params,
        varkw=varkw_params,
...
def makeSignature(dct: Bunch) -> Signature:
    ...
    for p, val in dct.named.items():
        parameters.append(
            Parameter(
                p, Parameter.POSITIONAL_OR_KEYWORD, default=val[0], annotation=val[1]
            )
        )
    ...
    for p, val in dct.kwonly.items():
        parameters.append(
            Parameter(p, Parameter.KEYWORD_ONLY, default=val[0], annotation=val[1])
        )
...
def signature_as_str(
    ...
    r"""Turns a signature dict into an executable str.
    ...
    if isinstance(f, (types.FunctionType, inspect.Signature)):
        ...
        raise TypeError(
            f"Expecting a function, a function Signature, or a traitlets.Bunch; got {type(f).__name__} instead"
        )
...
def scipywarn(message, category=None, stacklevel=1, source=None, out=None):
    ...
    if not (isinstance(category, type) and issubclass(category, Warning)):
        raise TypeError(
            "category must be a Warning subclass, not '{:s}'".format(
...
def showwarning(message, category, filename, lineno, file=None, line=None):
    r"""To replace stock Python warnings.showwarning"""
    if file is None:
        file = sys.stderr
        if file is None:
            return
...
def iter_attribute(iterable: typing.Iterable, attribute: str, silentfail: bool = True):
    r"""Iterator accessing the specified attribute of the elements in 'iterable'.
...
def filter_type(iterable: typing.Iterable, klass: typing.Type):
    r"""Iterates elements of 'iterable' that are of type specified by 'klass'
...
def filterfalse_type(iterable: typing.Iterable, klass: typing.Type):
    r"""The negated version of filter_type.
...
def filter_attr(
    ...
    **kwargs,
    ...
    r"""Filter an iterable using predicates applied to attributes of its elements.
    ...
    if not isinstance(
        ...
        raise TypeError(
            f"'op' expected to be a function, buitlin, lambda or method; got {op} instead"
        )
    ...
    if len(inspect.getfullargspec(op).args) != 2:
        raise TypeError(
            f"'op' parameter expected to be a binary function, i.e., with call syntax op(a,b)"
        )
    ...
    def _tf_(x, key, f):
        ...
        if exclude:
            return (
                (
                    not f(operator.attrgetter(key)(x))
                    if _check_dotted_attr_(x, key)
                    else not f(getattr(x, key, None))
                )
                if inspect.isfunction(f)
                else not operator.attrgetter(key)(x) == f
                if _check_dotted_attr_(x, key)
                else f != getattr(x, key, None)
            )
        else:
            return (
                (
                    f(operator.attrgetter(key)(x))
                    if _check_dotted_attr_(x, key)
                    else f(getattr(x, key, None))
                )
                if inspect.isfunction(f)
                else operator.attrgetter(key)(x) == f
                if _check_dotted_attr_(x, key)
                else f == getattr(x, key, None)
            )
    ...
    if len(kwargs) == 0:
        ...
        if indices:
                ...
                yield from (
                    ...
                    for i in filter(
                        lambda x: functools.reduce(
                            op, (_tf_(x[1], k, f) for k, f in kwargs.items())
                        ),
                        enumerate(iterable),
                    )
                )
            ...
            yield from filter(
                lambda x: functools.reduce(
                    op, (_tf_(x, k, f) for k, f in kwargs.items())
                ),
                iterable,
            )
...
def filterfalse_attr(iterable: typing.Iterable, **kwargs):
    r"""'Negative' form of filter_attr.
...
def filter_attribute(
    ...
    r"""Iterates elements in 'iterable' for which 'attribute' satisfies 'predicate'.
    ...
    return filter(
        lambda x: predicate(
            getattr(x, attribute, None) if silentfail else getattr(x, attribute), value
        ),
        iterable,
    )


def filterfalse_attribute(
    iterable: typing.Iterable,
    attribute: str,
    value: typing.Any,
    predicate: typing.Callable[..., bool] = lambda x, y: x == y,
    ...
    r"""The negated version of filter_attribute.
    DEPRECATED
    ...
    if with_properties:
        return [
            ...
            for i in inspect.getmembers(
                obj, lambda x: inspect.isdatadescriptor(x) or isinstance(x, property)
            )
        ]
    else:
        return [
            i[0] for i in inspect.getmembers(obj, lambda x: inspect.isdatadescriptor(x))
        ]
...
def get_methods(obj):
    ...
    return [
        i[0] for i in inspect.getmembers(obj, lambda x: inspect.isfunction(x, property))
    ]
...
def class_def(data):
    ...
    ss = (
        "("
        + ", ".join([f"{p.__module__}.{p.__name__}" for p in parent_types(data)])
        + ")"
    )
...
def __check_array_attribute__(rt, param) -> None:
    ...
    if rt["default_value"] is not None:
        if not isinstance(rt["default_value"], np.ndarray):
            raise ValueError(
                f"Type of the default value type {type(rt['default_value']).__name__} is not a numpy ndarray"
            )
    ...
    if isinstance(param, collections.abc.Sequence):
            ...
            if rt["default_value_dtypes"] is None:
                if isinstance(rt["default_value"], np.ndarray):
                    if rt["default_value"].dtype not in tuple(param):
                        raise ValueError(
                            f"dtype of the default value type ({type(rt['default_value']).dtype}) is not in {param}"
                        )
            ...
            if rt["default_value_shape"] is None or rt["default_value_ndim"] is None:
                if isinstance(rt["default_value"], np.ndarray):
                    if rt["default_value"].shape != tuple(param):
                        raise ValueError(
                            f"Default value has wrong shape ({rt['default_value'].shape}); expecting {param}"
                        )
        ...
        if rt["default_value_dtypes"] is None:
            if isinstance(rt["default_value"], np.ndarray):
                if (
                    rt["default_value"].dtype != param
                    and rt["default_value"].dtype.kind != param.kind
                ):
                    raise ValueError(
                        f"Wrong dtype of the default value ({rt['default_value'].dtype}); expecting {param}"
                    )
        ...
        if rt["default_value_ndim"] is None:
            if isinstance(rt["default_value"], np.ndarray):
                if rt["default_value"].ndim != param:
                    raise ValueError(
                        f"Wrong dimensions for the default value ({rt['default_value'].ndim}); expecting {param}"
                    )

            rt["default_value_ndim"] = param

    if issubclass(rt["default_value_type"], vigra.VigraArray):
        if rt["default_value"] is not None and not isinstance(
            rt["default_value"], vigra.VigraArray
        ):
            raise TypeError(
                f"Wrong default value type ({type(rt['default_value']).__name__}; expecting a vigra.VigraArray"
            )
            ...
            if rt["default_array_order"] is None:
                if isinstance(rt["default_value"], vigra.VigraArray):
                    if rt["default_value"].order != param:
                        raise ValueError(
                            f"Default value has wrong array order ({rt['default_value'].order}); expecting {param} "
                        )
            ...
            if rt["default_axistags"] is None:
                if isinstance(rt["default_value"], vigra.VigraArray):
                    if rt["default_value"].axistags != param:
                        raise ValueError(
                            f"Default value has wrong axistags ({rt['default_value'].axistags}); expecting {param} "
                        )
    ...
    if issubclass(rt["default_value_type"], pq.Quantity):
        if rt["default_value"] is not None and not isinstance(
            rt["default_value"], pq.Quantity
        ):
            raise TypeError(
                f"Wrong default value type ({type(rt['default_value']).__name__}; expecting a Python Quantity"
            )
        ...
        if isinstance(param, pq.Quantity):
            if rt["default_value_ndim"] is None:
                if isinstance(
                    rt["default_value"], pq.Quantity
                ) and not unitsConvertible(rt["default_value"].units, param.units):
                    raise ValueError(
                        f"Default value has wrong units ({rt['default_value'].units}); expecting {param} "
                    )
...
def resolve_object(modName, objName):
    ...
    if modName is None:
        ...
        if len(owner.strip()) == 0:
            return (
                dataclasses.MISSING
            )  # no owner type/module specified - no way to resolve that
...
def _(modname: str):
    ...
    m_rev_path = list(
        itertools.accumulate(reversed(modname.split(".")), lambda x, y: ".".join(y, x))
    )
...
def parse_module_class_path(x: str) -> typing.Union[type, types.ModuleType]:
    ...
    if len(obj) > 1:
        raise RuntimeError(
            f"Ambiguous module.class specification {a} - are there duplicates?"
        )

    if len(obj) == 0:
        raise RuntimeError(
            f"The module.class specification {a} is not found. HAs it been defined and imported at all?"
        )
...
class with_doc:
    ...
    def indent_lines(self, docstr: str, times: int = 1):
        ...
        indlines = list(
            map(lambda x: f"{self.indent * self.factor * times}{x}", doclines)
        )
...
::
    from x.y import z as q

where ``q`` is an alias to object ``z`` defined in (sub)module ``x.y``
...
"""
    ...
    if not isinstance(obj, (types.ModuleType, types.FunctionType, types.MethodType, type)):
        return False
    ...
    if oname in name:
        ...
        parts_ok, parts = shell._find_parts(name)
        return oname in parts
...
def object_find(oname=str, namespaces=None,
    ...
    if len(candidates):
        ...
        msg = f"<b>No Python documentation found for '{oname}'</b><br>Check the spelling; you may need to enter a valid dotted path e.g. 'package.module.object.member' <br>" if len(foundinfos) == 0 else ""
