...
def simplify_2d_shape(xy: np.ndarray, max_points: int = 5, k: int = 3):
    ...
    # but never both)
    unq_xy_ndx = np.where(
        (xd1[:, 0] != 0) & (xd1[:, 1] != 0) | (xd2[:, 0] != 0) & (xd2[:, 1] != 0)
    )[0]
    ...
    if len(nadj_unq_xy_ndx):
        ...
        if len(long_stretch_end_indirect_ndx):
            ...
            curves = [
                xy[adjacent_runs[0, k] : adjacent_runs[1, k]]
                for k in range(adjacent_runs.shape[1])
            ]

            # parametric 1D cubic spline representation (NOT bivariate spline !)
            splines = [
                interpolate.splprep([curve[:, 0], curve[:, 1]]) for curve in curves
            ]
...
def generate_bin_width(adcres: float = 15, adcrange: float = 10, adcscale: float = 1):
    r"""Define a histogram bin width according to the ADC used to collect the data.
...
def is_positive_waveform(x: np.ndarray):
    r"""A positive waveform has the majority of its samples >= 0
    ...
    if not isinstance(x, np.ndarray):
        raise TypeError(
            f"Expecting a np.ndarray object or a derived type; got {type(x).__name__} instead"
        )

    if len(x.shape) > 1:
        if x.ndim > 2 or x.shape[1] > 1:
            raise TypeError(
                f"Expecting a vector; got an array with shape {x.shape} instead"
            )
...
def scale_waveform(x: np.ndarray, α, β):
    r"""Scales waveform by a rational factor (α/β).
    ...
    if isinstance(x, pq.Quantity):
        ...
        xx = (
            x.magnitude
        )  # → this is a REFERENCE ; its changes will be reflected in x !!!
...
def normalise_waveform(
    x: np.ndarray,
    axis: typing.Optional[int] = None,
    rng: typing.Optional[typing.Union[float, np.ndarray]] = None,
):
    r"""Waveform normalization.
...
def data_range(x: np.ndarray, **kwargs):
    r"""The difference between a signal max and min values.
...
def waveform_amplitude(x: np.ndarray, method: str = "direct", axis=None):
    r"""Calculates the amplitude of a waveform.
...
def shorth_estimator(x: np.ndarray):
    r"""Shorth estimator for step-like waveforms.
    See IEEE Std 181-2011:
        IEEE Standard for Transitions, Pulses, and Related Waveforms
        Section 5.2.2
    """
...
def split_histogram(counts, f):
    ...
    if isinstance(f, float):
        if f > 0 and f < 1:
            ...
            raise ValueError(
                "A single level fraction must be between 0 and 1; got %f instead" % (f)
            )
        ...
        if len(f) == 1:
            if f[0] > 0 and f[0] < 1:
                ...
                raise ValueError(
                    "A single level fraction must be between 0 and 1; got %f instead"
                    % (f[0])
                )
            ...
            if any([j < 0 for j in f]):
                raise ValueError(
                    "Level fractions cannot have negative values. Got %s" % str(f)
                )
            ...
            if np.sum(f) != 1:
                if np.sum(f) < 1:  # augument if sum less than one
                    ...
                    raise ValueError(
                        "Level fractions must sum to 1; instead, they sum to %f"
                        % (np.sum(f))
                    )
        ...
        raise ValueError(
            "Level fractions must be specified as a single scalar between 0 and 1 (inclusive) or as a sequence of scalars that must add up to 1"
        )
...
def state_levels(x: np.ndarray, **kwargs):
    r"""Calculate states from a 1D waveform.
    See IEEE Std 181-2011:
        IEEE Standard for Transitions, Pulses, and Related Waveforms
    ...
    if isinstance(bins, int):
        if bins < 1:
            raise ValueError(
                "When specified, 'bins' must be > 1; got %d instead" % bins
            )
    ...
    if isinstance(moment, str):
        if moment == "mean":
            ...
            sLevels = [
                edges[where(counts[r] == np.max(counts[r]))[0][0]] for r in ranges
            ]

        else:
            raise ValueError(
                "Moment specified by an invalid string (%s); expecting 'mean' or 'mode'"
                % (moment)
            )
        ...
        raise TypeError(
            "Moment must be specified by a string ('mean' or 'mode') or a unary function; got %s instead"
...
def remove_dc(
    ...
    if x.ndim > 2:
        raise ValueError(
            f"Expecting an array of maximum 2D; instead, got an array with {x.ndim} dimensions"
        )
    ...
    if xx.ndim == 2:
        if isinstance(channel, int) and channel not in range(-x.shape[1], x.shape[1]):
            raise ValueError(
                f"Channel index {channel} outside range ({-x.shape[1]}, {x.shape[1]})"
            )

        elif isinstance(channel, (tuple, int)) and all(
            isinstance(c, int) for c in channel
        ):
            if any(c not in range(-x.shape[1], x.shape[1])):
                raise ValueError(
                    f"At leqsty one channel index in {channel} is outside the range ({-x.shape[1]}, {x.shape[1]})"
                )

        elif channel is not None:
            raise TypeError(
                f"Channel expected to be an int or None; got {type(channel).__name__} instead"
            )
        ...
        if isinstance(value, (pq.Quantity, np.ndarray)):
            ...
            elif isinstance(channel, (tuple, list)) and len(value) not in (
                1,
                len(channel),
            ):
                raise ValueError(
                    f"Mismatch between number of values and specified channels"
                )

            elif channel is None and value.size not in (1, x.shape[1]):
                raise ValueError(
                    f"Mismatch between number of values and signal channels"
                )
            ...
            if isinstance(value, pq.Quantity) and isinstance(x, pq.Quantity):
                if isinstance(x, pq.Quantity):
                    if not scq.unitsConvertible(value, x):
                        raise TypeError(
                            f"Value units {value.units} are incompatible with signal units {x.units}"
                        )
            ...
            if isinstance(channel, (tuple, list)) and len(value) not in (
                1,
                len(channel),
            ):
                raise ValueError(
        ...
        if channel is None:
            for k in range(xx.shape[1]):
                if isinstance(value, float):
                    ...
                    if value.size != 1:
                        raise TypeError(
                            "When channel is not specified, value must be a scalar"
                        )
...
def minmax(x: np.ndarray, **kwargs):
    r"""Returns maxmin(x, max_first=False)
...
def argmaxmin(x: np.ndarray, **kwargs):
    r"""CAUTION: x must not contain NaNs.
...
def sem(x: np.ndarray, **kwargs):
    ...
    if isinstance(x, (pd.DataFrame, pd.Series)):
        ...
        ret = np.std(x, ddof=ddof, axis=axis, out=None, keepdims=keepdims) / np.sqrt(
            sz - ddof
        )


def nansem(x: np.ndarray, **kwargs):
    r"""SEM for data containing np.nan
...
def detrend(x: typing.Union[neo.AnalogSignal, DataSignal], **kwargs):
    ...
    func = functools.partial(
        scipy.signal.detrend, axis=axis, bp=bp, type=detrend_type, overwrite_data=False
    )

    if detrend_type.lower() == "linear":
        if (
            bp is None
            or isinstance(bp, typing.Sequence)
            and all(isinstance(v, int) for v in bp)
        ):
            ret = func(x.magnitude)

        else:
            raise TypeError(
                f"Unexpected 'bp' value ({bp}) for {detrend_type} detrending"
            )
        ...
        if isinstance(bp, pq.Quantity):
            if bp.size == 1 and scq.unitsConvertible(bp, x):
                ...
                raise TypeError(
                    f"Wrong constant value in bp: {bp} for {detrend_type} detrending"
                )
...
def parse_step_waveform_signal(sig,
    ...
    **kwargs):
    ...
    if not isinstance(sig, neo.AnalogSignal):
        raise TypeError(
            "Expecting an analogsignal; got %s instead" % type(sig).__name__
        )

    if sig.ndim == 2 and sig.shape[1] > 1:
        raise ValueError(
            "Expecting a signal with one channel, instead got %d" % sig.shape[1]
        )
    ...
    if method == BoxcarDetectionMethod.state_levels:
        ...
        centroids, cnt, edg, rng = state_levels(
            sig_flt.magnitude,
            levels=levels,
            adcres=adcres,
            adcrange=adcrange,
    ...
    if isinstance(sig, (neo.AnalogSignal, DataSignal)):
        if isinstance(new_sampling_period, pq.Quantity):
            if not scq.unitsConvertible(new_sampling_period, sig.sampling_period):
                raise TypeError(
                    "new sampling period units (%s) are incompatible with those of the signal's sampling period (%s)"
                    % (new_sampling_period.units, sig.sampling_period.units)
                )
        ...
        new_times, new_step = np.linspace(
            sig.t_start.magnitude,
            sig.t_stop.magnitude,
            num=new_axis_len,
            retstep=True,
            endpoint=False,
        )
        ...
        interpolator = pchip(
            sig.times.magnitude.flatten(),
            sig.magnitude.flatten(),
            axis=0,
            extrapolate=False,
        )
        ...
        if old_sampling_period is None:
            raise ValueError(
                "When signal is a generic array the old sampling period must be specified"
            )
...
def diff(sig, n=1, axis=-1, prepend=False, append=True):
    ...
    if not isinstance(axis, int):
        raise TypeError(
            "Axis expected to be an int; got %s instead" % type(axis).__name__
        )

    # first, squeeze out the signal's sigleton dimensions
    sig_data = np.array(
        sig
    ).squeeze()  # also copies the data; also we can use plain arrays
...
def gradient(
    ...
    r"""First order gradient through central differences.
    ...
    diffsig = np.array(
        sig
    )  # for a neo.AnalogSignal this also copies the signal's magnitude
    ...
    if diffsig.ndim == 2:
        ...
        raise TypeError(
            "'sig' has too many dimensions (%d); expecting 1 or 2" % diffsig.ndim
        )
    ...
    if isinstance(sig, DataSignal):
        ret = DataSignal(
            ...
            sampling_period=sig.sampling_period,
            name=sig.name,
            description="Gradient of %s over %d samples along axis %d"
            % (sig.name, n, axis),
        )
...
def ediff1d(
    ...
    to_begin: [numbers.Number, type(None)] = None,
):
    r"""Differentiates each channel of an analogsignal with respect to its time basis.
    ...
    if diffsig.ndim == 2:
        for k in range(diffsig.shape[1]):
            diffsig[:, k] = np.ediff1d(
                diffsig[:, k], to_end=to_end, to_begin=to_begin
            )  # to_end = to_end, to_begin=to_begin)
...
def forward_difference(
    ...
    r"""Calculates the forward difference along the time axis.
    ...
    if diffsig.ndim == 2:
        if n >= diffsig.shape[0]:
            raise ValueError(
                "'n' is too large (%d); should be n < %d" % (n, diffsig.shape[0])
            )
        ...
        if n == 0:
            ...
            for k in range(diffsig.shape[1]):
                diffsig[:, k] = __n_diff__(
                    diffsig[:, k], n=n, to_e=to_end, to_b=to_begin
                )  # to_end = to_end, to_begin=to_begin)
        ...
        if n >= len(diffsig):
            raise ValueError(
                "'n' is too large (%d); should be < %d" % (n, len(diffsig))
            )
...
def snr(x, axis=None, ddof=None, db=True):
    ...
    if not isinstance(ddof, (int, type(None))):
        raise TypeError(
            "ddof expected to be an int or None; got %sinstead" % ype(ddof).__name__
        )
...
def resample_poly(sig, new_rate, p=1000, window=("kaiser", 5.0)):
    ...
    if not isinstance(sig, neo.AnalogSignal):
        raise TypeError(
            "First parameter expected to be a neo.AnalogSignal; got %s instead"
            % type(sig).__name__
        )
    ...
    if isinstance(new_rate, numbers.Real):
        ...
        if new_rate.size > 1:
            raise TypeError(
                "Expecting new_rate a scalar quantity; got a shaped array %d" % new_res
            )
        ...
        if new_rate.units != sig.sampling_rate.units:
            if new_rate.units == sig.sampling_period.units:
                ...
                raise TypeError(
                    "Second parameter should have the same units as signal's sampling rate (%s); it has %s instead"
                    % (sig.sampling_rate.units, new_rate.units)
                )

    if new_rate <= 0:
        raise ValueError(
            "New sampling rate (%s) must be strictly positive !" % new_rate
        )
    ...
    if using_rate:
        ret = neo.AnalogSignal(
            resample(sig, up, p, window=window),
            units=sig.units,
            t_start=sig.t_start,
    ...
    if minVal is None:
        ...
        if isinstance(sig, (neo.AnalogSignal, pq.Quantity)):
            if not scq.unitsConvertible(minVal, sig):
                raise ValueError(
                    f"Units of minVal ({minVal.units}) are incompatible with signal units ({sig.units})"
                )
    ...
    if maxVal is None:
        ...
        if isinstance(sig, (neo.AnalogSignal, pq.Quantity)):
            if not scq.unitsConvertible(maxVal, sig):
                raise ValueError(
                    f"Units of maxVal ({maxVal.units}) are incompatible with signal units ({sig.units})"
                )
...
def correlate(in1, in2, **kwargs):
    r"""Calls scipy.signal.correlate(in1, in2, **kwargs).
    ...
    mode = kwargs.pop(
        "mode", "same"
    )  # let mdoe be "same" by default but allow it to be overridden
    ...
    if isinstance(in1, (neo.AnalogSignal, DataSignal)):
        ...
        if isinstance(
            in2, (neo.AnalogSignal, neo.IrregularlySampledSignal, DataSignal)
        ):
        ...
        ret = neo.IrregularlySampledSignal(
            corr, units=units, times=in1.times, name=name
        )
...
def detect_boxcar(
    ...
    minampli: typing.Optional[float] = 1.0,
    channel: typing.Optional[int] = None,
    up_first: bool = True,
    **kwargs,
) -> tuple:
    r"""Detection of boxcar or step (Heaviside) waveforms in a signal.
    ...
    if not isinstance(x, neo.AnalogSignal):
        raise TypeError(
            "Expecting a neo.AnalogSignal object; got %s instead" % type(x).__name__
        )
    ...
    if x.ndim == 2 and x.shape[1] > 1:
        if not isinstance(channel, int):
            raise ValueError(
                f"Expecting a signal with one channel, instead got {sig.shape[1]}, but 'channel' was not specified"
            )
    ...
    if not isinstance(box_size, int):
        raise TypeError(
            f"boxcar filter size must be an int; instead, got {type(box_size).__name__}"
        )
    ...
    try:
        ...
        if method == BoxcarDetectionMethod.state_levels:
            ...
            centroids, cnt, edg, rng = state_levels(sig_filt.magnitude, **kwargs)
        ...
        if all(v is not np.nan for v in (times_lo_hi, times_hi_lo)):
            if times_lo_hi.size == times_hi_lo.size:  # signal has boxcars only
                if times_lo_hi.size == 1:
                    upward = (
                        times_lo_hi[0] <= times_hi_lo[0]
                    )  # if up_first else times_lo_hi[0] > times_hi_lo[0]

                else:
                    upward = (
                        times_lo_hi <= times_hi_lo
                    )  # if up_first else times_lo_hi > times_hi_lo
                ...
                if minlen == 1:
                    ...
                    upward = (
                        times_lo_hi[0:minlen] <= times_hi_lo[0:minlen]
                    )  # if up_first else times_lo_hi[0:minlen] <= times_hi_lo[0:minlen]

                    lastup = (
                        times_lo_hi[-1] > times_hi_lo[-1]
                    )  # if up_first else times_lo_hi[-1] > times_hi_lo[-1]
