...
from gui.cursors import SignalCursor, DataCursor
...
if os.environ["QT_API"] == "pyside6":
    ...
    # from PySide6.QtUiTools import dloadUiType # -- A-HA!
...
axis: (optional)
    ...
    When None (the default), the function returns cursor parameters.

    When not None, signal_viewer MUST be specified, and axis indicates in which
    axis of the viewer vertical signal cursors will be generated by the function.

Var-keyword parameters:
----------------------
keep_units: bool, optional default is False
    When True, the numeric cursor parameters are python Quantity objects with the
    physical units of 'epoch'.

    This is used when creating a DataCursor; SignalCursor object do not associate
physical units.

units: pq.Quantity or None (default)
    Needed to rescale the epoch's coordinates, when creating a SignalCursor in
an axis where a Quantity array (e.g. a ``neo`` ``BaseSignal``) is plotted.

    WARNING: If left to the default, there is a possibility that the SignalCursor
will be constructed at coordinates outside the definition domain of the plotted
data.

    ATTENTION: if specified, ``units`` must ben convertible to the epoch's units.
...
.. caution::
    This *may* result in a SignalCursor at a coordinate outside the data domain
or the data definition domain.

    To avoid this, make sure to rescale the coordinates to the actual definiton
domain if the data.

"""
    import gui.signalviewer as signalviewer
    ...
    do_rescale = lambda x: x if units is None else x.rescale(units)

    if keep_units:
        ret = [(t + d/2. if d else t,
                d if d else 0*t.units,
                l if l else f"{epoch_name}_{k}") for (t, d, l, k) in itertools.zip_longest(do_rescale(epoch.times).flatten(),
                                                                                           do_rescale(epoch.durations).flatten(),
                                                                                           epoch.labels.flatten(),
                                                                                           range(epoch.size)
                                                                                           )
        ]
        ...
        ret = [(t + d/2. if d else t,
                d if d else 0,
                l if l else f"{epoch_name}_{k}") for (t, d, l, k) in itertools.zip_longest(do_rescale(epoch.times).flatten().magnitude,
                                                                                           do_rescale(epoch.durations).flatten().magnitude,
                                                                                           epoch.labels.flatten(),
                                                                                           range(epoch.size)
                                                                                           )
                ]

    # print(f"{epoch2cursors} -> ret: {ret}")

    signal_viewer = kwargs.pop("signal_viewer", None)

    # print(f"epoch2cursors: signal_viewer -> {type(signal_viewer).__name__}")

    # if not (signal_viewer, signalviewer.SignalViewer): # BUG 2026-05-19 16:11:39 bad resolution of class & type FIXME URGENTLY
    if type(signal_viewer).__name__ != "SignalViewer":
        # print(f"{epoch2cursors} no signal_viewer")
        return list(map(lambda x: DataCursor(*x), ret))
    ...
    if isinstance(axis, (pg.PlotItem, pg.GraphicsScene)):
        ...
        if isinstance(axis, pg.PlotItem):
            # print(f"{epoch2cursors} axis in viewer: {axis in signal_viewer.axes}")
